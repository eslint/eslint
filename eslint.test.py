...
      serializer =     NotebookSerializer(context);
...
      exportNotebook(notebook, serializer);
},
setNotebookMetadata:  (resource: vscode.Uri, metadata: Partial<NotebookMetadata>): Promise<boolean> => {
      document = vscode.workspace.notebookDocuments.find(doc => doc.uri.toString() === resource.toString());
   (!document) {
     false;
} 
...
        exportNotebook(notebook: vscode.NotebookData, serializer: NotebookSerializer): string {
return serializer.serializeNotebookToString(notebook);
} 

               desactivate() { } 

# Other questions, tiping loop 
...
    async def async_get_gainers_and_l



Read & run the code and make a file(eslint.test.py) and save in arc(datalncloud)
Export ci
        datalncloud
