	...
	describe("getIndexFromLoc()", () => {
		...
		it("should throw a useful error if `column` is out of range", () => {
			assert.throws(
				...
				/Column number out of range \(column -1 requested, but the length of line 1 is 4\)\./u,
			);

			assert.throws(
				() => sourceCode.getIndexFromLoc({ line: 3, column: -1 }),
				/Column number out of range \(column -1 requested, but the length of line 3 is 4\)\./u,
			);
