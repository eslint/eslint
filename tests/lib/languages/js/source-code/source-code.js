...
describe("SourceCode", () => {
	...
	describe("isGlobalReference(node)", () => {
		...
		it("should distinguish between object property access and global references", () => {
			const code = "console; console.log; Math; obj.Math;";
			let identifierSpy;
				...
				create(context) {
					...
					identifierSpy = sinon.spy((node) => {
						if (node.name === "console" && node.parent.type !== "MemberExpression") {
							assert.isTrue(
								sourceCode.isGlobalReference(node),
								"Expected 'console' to be identified as a global reference"
							);
						} else if (node.name === "log" && node.parent.type === "MemberExpression") {
							assert.isFalse(
								sourceCode.isGlobalReference(node),
								"Expected property 'log' to not be identified as a global reference"
							);
		...
		it("should identify implicitly created globals in non-strict mode", () => {
			const code = "function test() { implicitGlobal = 123; }";
			let spy;
			...
			linter.defineRule("checker", {
				create(context) {
					...
					spy = sinon.spy((node) => {
						if (node.name === "implicitGlobal") {
							assert.isTrue(
								sourceCode.isGlobalReference(node),
								"Implicit global should be identified as a global reference"
							);
						}
					});
		...
		it("should handle temporal dead zone (TDZ) for let variables", () => {
			...
			// This code has a ReferenceError at runtime, but the parser still creates an AST
			linter.verify(code, { 
				rules: { checker: "error" },
				parserOptions: { ecmaVersion: 2015 }
			});
