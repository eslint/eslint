	...
	describe("'ignores', 'files' of the configuration that the '--config' option provided should be resolved from CWD.", () => {
		...
		describe("if { basePath: '..', files: ['*.js'], ... } is present by '--config ../node_modules/myconf/eslint.config.js',", () => {
			...
			it("'lintFiles()' with '../foo/test.js' should use the files entry.", async () => {
				...
				// Expected to be an 'eqeqeq' error because the file is matched by `$CWD/../foo/*.js`.
				assert.deepStrictEqual(results, [
					{
						suppressedMessages: [],
						errorCount: 1,
		...
		describe("if { basePath: '..', files: ['**/*'], ignores: ['foo/*.js'], ... } is present by '--config bar/myconf/eslint.config.js',", () => {
			...
			it("'lintFiles()' with '../foo/test.js' should have no errors because no rules are enabled.", async () => {
				...
				// Expected to be no errors because the file is matched by `$CWD/../foo/*.js`.
				assert.deepStrictEqual(results, [
					{
						suppressedMessages: [],
						errorCount: 0,
		...
		describe("if { basePath: '..', ignores: ['foo/*.js'], ... } is present by '--config ../node_modules/myconf/eslint.config.js',", () => {
			const { prepare, cleanup, getPath } = createCustomTeardown({
				cwd: `${root}b3`,
				files: {
					"node_modules/myconf/eslint.config.js": `
			...
			it("'lintFiles()' with '../**/*.js' should lint '../node_modules/foo/test.js' but not '../foo/test.js'.", async () => {
				const engine = new ESLint({
					overrideConfigFile:
						"../node_modules/myconf/eslint.config.js",
					cwd: getPath("bar"),
				...
				assert.deepStrictEqual(files, [
					path.join(getPath(), "node_modules/foo/test.js"),
				]);
			});
		});
	});
