...
var fs = require("fs"),
    path = require("path"),
    patIsInside = require("path-is-inside");
...
/**
 * Check to see if the passed in directory path is within process.cwd
 * @param {string} dirPath The directory path to check for.
 * @returns {boolean} True if its inside process.cwd path
 * @private
 */
function isWithinCwd(dirPath) {
    return this.root ? patIsInside(dirPath, this.cwd) : true;
}
...
function FileFinder() {
    ...
    this.cache = {};
    this.root = false;
    // This is only for testing purposes. Should not be used inside the code.
    this.cwd = process.cwd();
}

/**
 * Set the value of root option
 * @param {boolean} val Value to set
 * @returns {void}
 */
FileFinder.prototype.setRoot = function(val) {
    this.root = !!val;
};
...
FileFinder.prototype.findInDirectoryOrParents = function (directory) {
    ...
    while (directory !== child && isWithinCwd.call(this, directory)) {
...
FileFinder.prototype.findAllInDirectoryAndParents = function (directory) {
    ...
    } while (!cache.hasOwnProperty(directory) && isWithinCwd.call(this, directory));
