...
class SuppressionsService {
	 ...
	 * @param {SuppressedViolations} [suppressionsSnapshot] Suppressions loaded from the suppressions file, used instead of reading the file again.
	 * @returns {Promise<void>}
	 */
	async suppress(results, rules, suppressionsSnapshot) {
		const suppressions = suppressionsSnapshot
			? structuredClone(suppressionsSnapshot)
			: await this.load();

		for (const result of results) {
			const relativeFilePath = SuppressionsService.getRelativeFilePath(
				result.filePath,
				this.cwd,
			);
	...
	/**
	 * Returns the IDs of the rules that are suppressed for a file.
	 * @param {SuppressedViolations} suppressions The suppressions.
	 * @param {string} filePath The file path.
	 * @param {string} cwd The current working directory.
	 * @returns {Set<string>} The IDs of the suppressed rules.
	 */
	static getSuppressedRuleIds(suppressions, filePath, cwd) {
		const relativeFilePath = SuppressionsService.getRelativeFilePath(
			filePath,
			cwd,
		);

		return new Set(Object.keys(suppressions[relativeFilePath] ?? {}));
	}
