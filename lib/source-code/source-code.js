...
class SourceCode extends TokenStore {
    ...
    getScope(currentNode) {
        ...
        const cachedScope = this[caches].get(cacheKey).get(currentNode);
        ...
        for (let node = currentNode; node; node = node.parent) {
            ...
            if (scope) {
                if (scope.type === "function-expression-name") {
                    this[caches].get(cacheKey).set(currentNode, scope.childScopes[0]);
                    return scope.childScopes[0];
                }

                this[caches].get(cacheKey).set(currentNode, scope);
                return scope;
            }
        }

        this[caches].get(cacheKey).set(currentNode, this.scopeManager.scopes[0]);
