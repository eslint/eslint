...
module.exports = (function() {
     ...
     * @param {string|{text:string, ast:object}} textOrParseResult Either the JavaScript text to verify or an object
     *      containing the original source text as well as the output from esprima. Note that the ast object must
     ...
     */
    api.verify = function(textOrParseResult, config, filename, saveState) {
        ...
        if (typeof textOrParseResult === "string") {
            text = textOrParseResult;
        } else {
            text = textOrParseResult.text || "";
        }
        ...
        if (textOrParseResult.ast) {
            // if a pregenerated ast was supplied, use it
            ast = textOrParseResult.ast;
        } else {
            // we only have text, so try parsing it
            ast = api.parse(text.replace(/^#!([^\r\n]+)/, function(match, captured) {
                shebang = captured;
                return "//" + captured;
            }), config);
        }
