...
function modifyRulesFromComments(text, config) {
    var rules = {};
    var matches = text.match(/\s*\/\*\s*eslint\s*(.*)?\*\//g);
    var insertRule = function(match) {
        forEachOption(match, function(name, value) {
            rules[name] = JSON.parse(value);
        });
    };
    if (matches) {
        for (var i=0, l=matches.length; i<l; i++) {
            var match = matches[i].replace(/\s*\/\*s*eslint\s*/, "").replace(/\*\//, "");
            insertRule(match);
        }
    }
    var cfgHelper = new configHelper();
    var newConfig = cfgHelper.mergeConfigs(config.rules, rules);
    return newConfig;
}
...
module.exports = (function() {
    ...
    api.verify = function(text, config, saveState) {
        ...
        config.rules = modifyRulesFromComments(text, config);
