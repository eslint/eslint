...
function validateLanguageOptions(languageOptions) {
    ...
    if (!isNonNullObject(languageOptions)) {
        throw new TypeError("Expected an object.");
    }

    if (languageOptions.ecmaVersion) {
        validateEcmaVersion(languageOptions.ecmaVersion);
    }

    if (languageOptions.sourceType) {
        validateSourceType(languageOptions.sourceType);
    }

    if (languageOptions.globals) {
        validateGlobals(languageOptions.globals);
    }

    if (languageOptions.parser) {
        validateParser(languageOptions.parser);
    }

    if (languageOptions.parserOptions) {
        if (!isNonArrayObject(languageOptions.parserOptions)) {
