...
class SourceCode extends TokenStore {
	...
	traverse() {
		...
		/*
		 * Parallel arrays encode step data without per-step object allocation.
		 * stepTypes encodes kind+phase in one byte:
		 *   1 = visit-enter, 2 = visit-exit, 3 = call-method (array args),
		 *   4 = call-method-2 (two positional args, no array allocation)
		 */
		const INITIAL_CAPACITY = 200000;
		let stepTypes = new Uint8Array(INITIAL_CAPACITY);
		const stepTargets = new Array(INITIAL_CAPACITY);
		const stepArgs = new Array(INITIAL_CAPACITY);
		const stepArgs2 = new Array(INITIAL_CAPACITY);
		let count = 0;

		let analyzer = {
			enterNode(node) {
				if (count >= stepTypes.length) {
					const newLen = stepTypes.length * 2;
					const old = stepTypes;

					stepTypes = new Uint8Array(newLen);
					stepTypes.set(old);
					stepTargets.length = newLen;
					stepArgs.length = newLen;
					stepArgs2.length = newLen;
				}
				stepTypes[count] = 1;
				stepTargets[count] = node;
				stepArgs[count] = null;
				count++;
			},
			leaveNode(node) {
			...
			emit(eventName, arg1, arg2, arg3) {
				...
				if (arg3 !== void 0) {
					stepTypes[count] = 3;
					stepTargets[count] = eventName;
					stepArgs[count] = [arg1, arg2, arg3];
				} else {
					stepTypes[count] = 4;
					stepTargets[count] = eventName;
					stepArgs[count] = arg1;
					stepArgs2[count] = arg2;
				}
				count++;
			},
		};
