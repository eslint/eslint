...
class SourceCode extends TokenStore {
	...
	isGlobalReference(node) {
		...
		// If the identifier is a property in a MemberExpression, it's not a global reference
		...
		// If the variable isn't found in any scope, it could be an implicit global
		if (variable === null) {
			const builtInGlobals = Object.getOwnPropertyNames(globalThis);
			
			// It's either a built-in global or an implicit global (undeclared variable)
			if (builtInGlobals.includes(node.name)) {
				return true;
			}
			
			/*
			 * For implicit globals (variables used without declaration)
			 * Look for this variable reference in the global scope's through references
			 */
			const globalScope = this.getScope(this.ast);
			const isImplicitGlobal = globalScope.through.some(ref => 
				ref.identifier.name === node.name
			);
			
			return isImplicitGlobal;
		}
