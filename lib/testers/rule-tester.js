        ...
        function runRuleForItem(ruleName, item) {
            ...
            if (schema) {
                ...
                if (ajv.errors) {
                    ...
                    // @todo This is lacking tests. Changing is-my-json-valid to Ajv did not trigger any failing tests.
                    // "error.field" is not a valid Ajv error property.
                    throw new Error([
                        `Schema for rule ${ruleName} is invalid:`
                    ].concat(ajv.errors.map(error => `\t${error.field}: ${error.message}`)).join("\n"));
                }
            }
