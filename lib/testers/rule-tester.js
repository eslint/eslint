                ...
                for (let i = 0, l = item.errors.length; i < l; i++) {
                    ...
                    if (typeof error === "string" || error instanceof RegExp) {
                        ...
                        if (error.messageId) {
                            ...
                            assert.equal(error.message, void 0, "Error should not specify both a message and a messageId.");
                        ...
                        if (error.type) {
                            assert.equal(message.nodeType, error.type, `Error type should be ${error.type}, found ${message.nodeType}`);
                        }

                        if (error.hasOwnProperty("line")) {
                            assert.equal(message.line, error.line, `Error line should be ${error.line}`);
                        }

                        if (error.hasOwnProperty("column")) {
                            assert.equal(message.column, error.column, `Error column should be ${error.column}`);
                        }

                        if (error.hasOwnProperty("endLine")) {
                            assert.equal(message.endLine, error.endLine, `Error endLine should be ${error.endLine}`);
                        }

                        if (error.hasOwnProperty("endColumn")) {
                            assert.equal(message.endColumn, error.endColumn, `Error endColumn should be ${error.endColumn}`);
                        }
                    } else {
