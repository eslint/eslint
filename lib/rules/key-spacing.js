    ...
    return {
        "ObjectExpression": function(node) {
            ...
            node.properties.forEach(function(property) {
                ...
                /*{
                var colon=context.getSource(property).substring(context.getSource(property.key).length,context.getSource(property).length-context.getSource(property.value).length);
                if ((spacingOption === 'always') && (colon===' : ')){
                    
                }else if((spacingOption==='never')&&(colon===':')){
                    
                }else if((spacingOption==='always')&&(colon.match('\'))){
                    context.report(node, '');
                }
                else if ((spacingOption === 'never') && (context.getSource(property).indexOf(' : ') >= 0)) {
                    context.report(node, 'problem b');
                }
            });
            */
        }
