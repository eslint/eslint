...
module.exports = {
    ...
    create(context) {
         ...
         * @param {Token} firstToken - first token of the node.
         * @returns {boolean} `true` if it can fix the node.
         */
        function canFix(node, firstToken) {

            /**
             * do not fix if firstToken is not "var"
             * https://github.com/eslint/eslint/issues/11441
             */
            if (firstToken !== "var") {
                return false;
            }
        ...
        function report(node) {
            context.report({
                ...
                fix(fixer) {
                    ...
                    return canFix(node, firstToken)
                        ? fixer.replaceText(firstToken, "let")
                        : null;
                }
            });
        }
