...
module.exports = {
    ...
    create(context) {
        ...
        function needsParentheses(node) {
            let exprStatNode = node;

            while (exprStatNode && exprStatNode.type !== "ExpressionStatement") {
                exprStatNode = exprStatNode.parent;
            }
            if (exprStatNode && node.range[0] === exprStatNode.range[0]) {
                return true;
            }
         ...
         */
        function check(node) {
            if (node.callee.name !== "Object" || node.arguments.length) {
                return;
            }
