...
const VALID_PARENT = /^(?:Program|(?:Switch|Block)Statement|SwitchCase)$/;
const LT = `(?:${astUtils.LINEBREAK_MATCHER.source})`;
...
function isIIFEStatement(node) {
    return (
        ...
        astUtils.isFunction(node.expression.callee)
    );
}
...
function verifyForNever(context, prevNode, nextNode, paddingLines) {
    ...
    context.report({
        node: nextNode,
        message: "Unexpected blank lines before this statement.",
        fix(fixer) {
    ...
    create(context) {
        ...
        function getPaddingType(prevNode, nextNode) {
            for (let i = configureList.length - 1; i >= 0; --i) {
                ...
                const matched =
                    match(prevNode, configure[1]) &&
                    match(nextNode, configure[2]);

                if (matched) {
                    return PaddingTypes[configure[0]];
                }
            }
        ...
        function verify(node) {
            if (!VALID_PARENT.test(node.parent.type)) {
                return;
            }
