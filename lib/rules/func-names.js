...
module.exports = {
    ...
    create(context) {
        let never;
        let asNeeded;
        let requireNamedGenerators;

        if (context.options[0] instanceof Object) {
            never = context.options[0].base === "never";
            asNeeded = context.options[0].base === "as-needed";
            requireNamedGenerators = !never;

            if (context.options[0].hasOwnProperty("generators")) {
                requireNamedGenerators = context.options[0].generators !== "never";
            }
        } else {
            never = context.options[0] === "never";
            asNeeded = context.options[0] === "as-needed";
            requireNamedGenerators = !never;
