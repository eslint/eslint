...
const variableDeclaratorKinds = ["var", "let", "const", "using", "await using"];
...
module.exports = {
	...
	create(context) {
		...
		if (typeof mode === "string") {
			for (const kind of variableDeclaratorKinds) {
				options[kind] = { uninitialized: mode, initialized: mode };
			}
		} else if (typeof mode === "object") {
			options.separateRequires = !!mode.separateRequires;
			for (const kind of variableDeclaratorKinds) {
				options[kind] = {
					uninitialized: mode.uninitialized ?? mode[kind],
					initialized: mode.initialized ?? mode[kind],
				};
			}
		}
		...
		function startBlock() {
			blockStack.push({
				...
				"await using": { initialized: false, uninitialized: false },
			});
		}
		 ...
		 * @param {string} statementType node.kind, one of: "var", "let", "const", "using" or "await using"
		...
		function getCurrentScope(statementType) {
			...
			} else if (variableDeclaratorKinds.includes(statementType)) {
				// All non-var declared variables are block scoped
				currentScope = blockStack.at(-1)[statementType];
			}
			return currentScope;
		}
