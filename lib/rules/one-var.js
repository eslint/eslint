...
module.exports = {
    ...
    create(context) {
        ...
        function joinDeclarations(declarations) {
            ...
            const currentIndex = (declaration.parent.parent.body || []).indexOf(declaration.parent);
            const previousNode = currentIndex === -1 ? null : declaration.parent.parent.body[currentIndex - 1];

            return fixer => {
                const prevSemi = sourceCode.getTokenBefore(declaration, { skip: 1 });
                const res = [];

                if (previousNode && previousNode.kind === declaration.kind) {
                    res.push(fixer.replaceText(prevSemi, ","));
                    res.push(fixer.replaceText(declaration.kind, ""));
                }
