...
module.exports = {
    ...
    create(context) {
         ...
         * @param  {VariableDeclaration} declaration The `VariableDeclaration` to join
         * @return {Function}                         The fixer function
         */
        function joinDeclarations([declaration]) {
            return fixer => {
                const type = sourceCode.getTokenBefore(declaration);
                const semiColon = sourceCode.getTokenAfter(declaration);
                const nextType = sourceCode.getTokenAfter(semiColon);
                const prevSemi = sourceCode.getTokenBefore(type);
                const prevType = sourceCode.getTokenBefore(type, {
                    filter(token) {
                        return ~["const", "var", "let"].indexOf(token.value) && token.type === "Keyword";
                    }
                });

                const res = [];
                if (sourceCode.getText(prevType) === sourceCode.getText(type)) {
         ...
         */
        function splitDeclarations(declaration, filter) {
            if (!filter) {
                filter = () => true;
            }
            return fixer => declaration.declarations.map(declarator => {
                const comma = sourceCode.getTokenAfter(declarator);
                const afterComma = sourceCode.getTokenAfter(comma, { includeComments: true });

                if (filter(declarator) && comma.value === ",") {
                    let spaceBefore = " ";
                    let spaceAfter = "";

                    if (afterComma.range[0] === comma.range[1]) {
                         ...
                         */
                        spaceAfter = " ";
                        return fixer.replaceText(comma, `;${spaceBefore}${declaration.kind}${spaceAfter}`);
                    }
                    if (afterComma.loc.start.line > comma.loc.end.line || afterComma.type === "Line" || afterComma.type === "Block") {
                         ...
                         */
                        spaceBefore = "\n";
                        let lastComments = afterComma;
                        const comments = [];

                        while (lastComments.type === "Line" || lastComments.type === "Block") {
                            comments.push(lastComments);
                            lastComments = sourceCode.getTokenAfter(lastComments, { includeComments: true });
                        }
                        let replacedText = "";

                        for (const comment of comments) {
                            replaceText += `${sourceCode.getText(comment)}\n`;
                        }
                        return fixer.replaceTextRange([comma.range[0], lastComments.range[0]], `;\n${replacedText}${declaration.kind} `);
                    }
