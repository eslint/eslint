...
module.exports = {
	...
	create(context) {
		...
		function joinDeclarations(declarations) {
			const currentDeclaration = declarations[0].parent;

			if (!isInStatementList(currentDeclaration)) {
				return () => [];
			}

			const body = currentDeclaration.parent.body;
			const currentIndex = body ? body.indexOf(currentDeclaration) : -1;
			const previousNode =
				currentIndex > 0 ? body[currentIndex - 1] : null;

			return fixer => {
				if (
					!previousNode ||
					previousNode.type !== "VariableDeclaration" ||
					previousNode.kind !== currentDeclaration.kind
				) {
					return [];
				}

				const startOfRemoval =
					previousNode.declarations.at(-1).range[1];
				const endOfRemoval = declarations[0].range[0];

				const range = [startOfRemoval, endOfRemoval];
