...
module.exports = {
    ...
    create(context) {
            ...
            return function *fix(fixer) {
                for (const declarator of declaration.declarations) {
                    ...
                    if (filter(declarator) && comma.value === ",") {
                        ...
                        if (sourceCode.getTokenAfter(comma).range[0] === comma.range[1]) {

                            /*
                             * `var x,y`
                             * comma ^^ token after comma
                             */
                            spaceAfter = " ";
                        }
                        yield fixer.replaceText(comma, `; ${declaration.kind}${spaceAfter}`);
                    }
                }
            };
        }
        ...
        /**
         * Fixer to merge a VariableDeclaration into individual declarations
        ...
        function mergeDeclarations() { // eslint-disable-line no-unused-vars
            const declarations = [].slice.call(arguments);

            return function *fix(fixer) {
                let textToInsert = "";

                // remove and store the non-first declarations
                for (const declaration of declarations.slice(1)) {
                    const firstVariable = sourceCode.getFirstToken(declaration, 1);
                    let source = sourceCode.getText(declaration);
                     ...
                     *     select this part
                     */
                    source = source.slice(firstVariable.range[0] - declaration.range[0]);
                    if (hasSemi(declaration)) {
                ...
                if (hasSemi(first)) {
                    yield fixer.insertTextBefore(sourceCode.getLastToken(first), textToInsert);
                } else {
                    yield fixer.insertTextAfter(first, textToInsert);
                }
            };
        }
