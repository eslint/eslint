...
module.exports = {
    ...
    areBracesNecessary(node, sourceCode) {
         ...
         *        else
         *            bar();
         * @param {ASTNode} candidateNode Node representing the code to check.
         * @returns {boolean} `true` if an `if` statement within the code would become associated with an `else` appended to that code.
         */
        function hasUnsafeIf(candidateNode) {
            switch (candidateNode.type) {
                case "IfStatement":
                    if (!candidateNode.alternate) {
                        return true;
                    }
                    return hasUnsafeIf(candidateNode.alternate);
                case "ForStatement":
                case "ForInStatement":
                case "ForOfStatement":
                ...
                case "WithStatement":
                case "WhileStatement":
                    return hasUnsafeIf(candidateNode.body);
                default:
                    return false;
            }
        }

        if (!hasUnsafeIf(node.body[0])) {
            return false;
        }
        const nextToken = sourceCode.getTokenAfter(node);

        return nextToken && nextToken.value === "else" && nextToken.type === "Keyword";
    },
