...
module.exports = {
    ...
    /**
     * Returns text between two nodes or tokens. Order does not matter.
     * @param {SourceCode} sourceCode The source code object to get tokens.
     * @param {ASTNode|Token} first The first node or token to get text after.
     * @param {ASTNode|Token} second The second node or token to get text before.
     * @param {boolean} [removeComments=false] Whether comments should be removed from result.
     * @returns {string} Text between specified nodes or tokens.
     * @public
     */
    getTextBetween(sourceCode, first, second, removeComments) {
        if (first === second) {
            return "";
        }

        const text = sourceCode.getText();
        let [currentToken, endToken] = first.range[1] <= second.range[0]
            ? [first, second]
            : [second, first];

        currentToken = sourceCode.getLastToken(currentToken) || currentToken;
        endToken = sourceCode.getFirstToken(endToken) || endToken;
        if (removeComments) {
            let result = "";
            ...
            while (currentToken !== endToken) {
                ...
                result += text.slice(currentToken.range[1], nextToken.range[nextToken === endToken || isCommentToken(nextToken) ? 0 : 1]);
                currentToken = nextToken;
            }
            return result;
        }

        return text.slice(currentToken.range[1], endToken.range[0]);
    },
