    ...
    switch (node.type) {
        ...
        case "CallExpression": {
            ...
            if (
                ...
                const inBooleanPosition = functionName === "Boolean";

                return (
                    node.arguments.length === 0 || isConstant(scope, node.arguments[0], inBooleanPosition));
            }
            return false;
        }
        case "JSXElement": // ESLint has a policy of not assuming any specific JSX behavior.
...
module.exports = {
    meta: {
        ...
        messages: {
            ...
            bothAlwaysNew: "Unexpected comparison of two newly constructed object. These two values can never be equal."
        }
    },
