            ...
            ConditionalExpression(node) {
                if (isBooleanLiteral(node.alternate) && isBooleanLiteral(node.consequent)) {
                    ...
                    context.report({
                        ...
                        fix: fixer => {
                            let nodeTest = astUtils.getParenthesisedText(sourceCode, node.test);
                            let nodeAlternate = astUtils.getParenthesisedText(sourceCode, node.alternate);

                            if (node.test.type === "ConditionalExpression") {
                                nodeTest = `(${nodeTest})`;
                            }
                            ...
                            return fixer.replaceText(node, `${nodeTest} || ${nodeAlternate}`);
                        }
                    });
                }
            }
