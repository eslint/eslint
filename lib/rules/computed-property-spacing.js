    ...
    create(context) {
        ...
        function reportInconsistentSpaces(node, beforeToken, firstToken, lastToken, afterToken) {
            context.report({
                ...
                data: {
                    expression: astUtils.getTextBetween(sourceCode, beforeToken, afterToken, true).trim(),
                    maxSpaces
                },
                fix(fixer) {
                    return [
            ...
            return function(node) {
                ...
                if (consistentSpaces) {
                    if (beforeFirstOnSameLine || lastAfterOnSameLine) {
                        textBetweenBeforeFirst = astUtils.getTextBetween(sourceCode, before, first, true);
                        textBetweenLastAfter = astUtils.getTextBetween(sourceCode, last, after, true);
