...
module.exports = {
	...
	create(context) {
		...
		function isDirective(node) {
			 ...
			 *
			 * Only `FunctionBody`, `ScriptBody`, `ModuleBody`, and TypeScript module/namespace bodies
			 * can have directive prologue. Class static blocks do not have directive prologue.
			 */
			if (!looksLikeDirective(node)) {
				return false;
			}
			...
			// Handle TypeScript module and namespace declarations
			if (parent.type === "TSModuleBlock") {
				const grandParent = parent.parent;
				if (
					grandParent &&
					(grandParent.type === "TSModuleDeclaration" ||
						grandParent.type === "TSNamespaceDeclaration")
				) {
					const moduleDirectives = takeWhile(
						looksLikeDirective,
						parent.body,
					);
					return moduleDirectives.includes(node);
				}
			}
