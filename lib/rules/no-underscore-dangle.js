...
module.exports = {
    ...
    create(context) {
        ...
        function checkForDanglingUnderscoreInVariableExpression(node) {
            const shouldDeepCheck = node && node.type && node.type === "VariableDeclarator" && node.id.type;

            if (shouldDeepCheck && node.id.type === "ArrayPattern" && Array.isArray(node.id.elements)) {
                node.id.elements.forEach(element => {
                    deepCheckDestructured(element, node.id.type);
                });
            }

            if (shouldDeepCheck && node.id?.type === "ObjectPattern" && Array.isArray(node.id.properties)) {
