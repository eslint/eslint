        ...
        function verify(target) {
            ...
            function verifyAssignmentInfoIsUnused(targetAssignment, allAssignments) {
                ...
                const otherAssignmentAfterTargetAssignment = allAssignments
                    .find(assignment => {
                        if (assignment === targetAssignment || assignment.segment !== targetAssignment.segment) {
                            return false;
                        }
                        if (targetAssignment.node.range[1] <= assignment.node.range[0]) {

                            /*
                             * The assignment is located after the target assignment.
                             * e.g. x=1; x=2
                             *      ^^^  ^^^ assignment
                             *      └ targetAssignment
                             */
                        ...
                        if (assignment.node.range[0] <= targetAssignment.node.range[0] && targetAssignment.node.range[1] <= assignment.node.range[1]) {

                            /*
                             * The assignment wrap the target assignment.
                             * e.g. x=(x=1);
                             *         ^^^ targetAssignment
                             *      ^^^^^^^ assignment
                             */
                ...
                for (const reference of readReferences) {
                    ...
                    if (
                        targetAssignment.node.range[1] <= reference.identifier.range[0] &&
