...
module.exports = {
	...
	create(context) {
		...
		/**
		 * Checks whether the given AST node contains a YieldExpression,
		 * not descending into nested function/class bodies.
		 * @param {ASTNode} node The AST node to check.
		 * @returns {boolean} `true` if the node contains a yield expression.
		 */
		function containsYield(node) {
			let found = false;

			/**
			 * Visits a single AST node, returning true if traversal should stop.
			 * @param {ASTNode} child The child node.
			 * @returns {boolean} `true` to stop traversal.
			 */
			function visit(child) {
				...
				// Don't descend into nested functions or class bodies
				if (
			...
			/**
			 * Recursively walk an AST node, calling visitor for each child.
			 * @param {ASTNode} current The current node being walked.
			 * @returns {void}
			 */
		 ...
		 * @property {boolean} isGenerator Whether the code path is for a generator function.
		 * @property {Array} generatorTryFinallyBlocks The try blocks of TryStatements with finalizers that contain yield expressions, within a generator function.
		 */
		/**
		...
		function verify(target) {
			...
			function verifyAssignmentIsUsed(targetAssignment, allAssignments) {
				...
				/*
				 * In a generator function, if there is a try-finally block that
				 * contains a yield expression, an assignment that appears before
				 * the try statement may be observable in the finally block via
				 * `iterator.return()`. Skip such assignments to avoid false positives.
				 */
				if (
					target.generatorTryFinallyBlocks.some(
						tryStatement =>
							targetAssignment.identifier.range[1] <=
							tryStatement.range[0],
					)
				) {
					return;
				}
		...
		return {
			onCodePathStart(codePath, node) {
				...
				const isGenerator = Boolean(
					node.generator || (node.type === "Program" && false), // Programs are not generators
				);
			...
			TryStatement(node) {
				...
				/*
				 * In generator functions, calling `iterator.return()` can interrupt
				 * a yield expression and transfer control directly to the finally block,
				 * bypassing the rest of the try body. This means an assignment that
				 * precedes the try-finally block may be observable in the finally block
				 * even if the code path analysis doesn't model this control flow.
				 * To avoid false positives, track these try blocks so we can skip
				 * assignments that appear before them.
				 */
				if (
					scopeStack.isGenerator &&
					node.finalizer &&
					containsYield(node.block)
				) {
					scopeStack.generatorTryFinallyBlocks.push(node);
				}
			},
