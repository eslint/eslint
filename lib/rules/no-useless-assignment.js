...
module.exports = {
    ...
    create(context) {
        ...
        function verify(target) {
             ...
             * @returns {void}
             */
            function verifyAssignment({ segment, variable, identifier }, otherAssignments) {
                ...
                let cacheOtherAssignment = null;

                /**
                 * Get other assignment on the current segment and after current assignment.
                 * @returns {AssignmentInfo|undefined} other assignment
                 */
                function getOtherAssignmentAfterAssignment() {
                    if (cacheOtherAssignment) {
                        return cacheOtherAssignment.result;
                    }
                    cacheOtherAssignment = {
                        result: otherAssignments
                            .find(otherAssignment =>
                                otherAssignment.segment === segment &&
                            identifier.range[0] < otherAssignment.identifier.range[0])
                    };
                ...
                for (const reference of readReferences) {
                    ...
                    if (
                        identifier.range[0] < reference.identifier.range[0] &&
                        isInSegment(segment, reference.identifier)
                    ) {

                        const otherAssignment = getOtherAssignmentAfterAssignment();

                        if (
                            otherAssignment &&
                            otherAssignment.identifier.range[0] < reference.identifier.range[0]
                        ) {
