 ...
 * @param {boolean} checkArrayFromAsync Whether to check for `Array.fromAsync` callbacks.
 * @returns {string} The method name if the node is a callback method,
 *      null otherwise.
 */
function getArrayMethodName(node, checkArrayFromAsync) {
	...
	while (currentNode) {
		...
		switch (parent.type) {
			...
			case "CallExpression":
				...
				if (
					checkArrayFromAsync &&
					astUtils.isArrayFromAsyncMethod(parent.callee)
				) {
					if (
						parent.arguments.length >= 2 &&
						parent.arguments[1] === currentNode
