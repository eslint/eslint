...
module.exports = rule({
    ...
    create(context) {
         ...
         * @param {AST<"Identifier">} node The node to report.
         * @returns {void}
         * @private
         */
        ...
        return {
            ...
            Identifier(node) {
                ...
                if (node.parent.type === "MemberExpression") {
                    ...
                    } else if (effectiveParent.type === "AssignmentExpression" && nameIsUnderscored && (effectiveParent.right.type !== "MemberExpression" || effectiveParent.left.type === "MemberExpression" && getStaticPropertyName(effectiveParent.left) === node.name)) {
                        report(node);
                    }
                 ...
                 */
                    ...
                    if (node.parent.type === "Property" && node.parent.parent.type === "ObjectPattern") {
                    ...
                    if (nameIsUnderscored && !ALLOWED_PARENT_TYPES.has(effectiveParent.type) && !(node.parent.type === "AssignmentPattern" && node.parent.right === node)) {
                        report(node);
                    }

                // Check if it's an import specifier
                } else if (node.parent.type === "ImportSpecifier" || node.parent.type === "ImportNamespaceSpecifier" || node.parent.type === "ImportDefaultSpecifier") {

                    // Report only if the local imported identifier is underscored
                    if (node.parent.local.name === node.name && nameIsUnderscored) {
                        report(node);
                    }
