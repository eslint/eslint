    ...
    create(context) {
        ...
        function checkForLoops(node) {
            ...
            if (!(node.async || node.generator)) {
                ...
                const ArrowFunctionExpression = node.type === "ArrowFunctionExpression";

                if (parentNode.type === "CallExpression" && (isFunctionExpression || ArrowFunctionExpression)) {
                    ...
                    const hasUnsafeRefsInNonIIFE = unsafeRefs.some(r => {
                        const parentType = r.identifier.parent.type;
                        const grandParentType = r.identifier.parent.parent.type;

                        return (parentType === "FunctionExpression" || parentType === "ArrowFunctionExpression") && grandParentType !== "CallExpression";
                    });

                    if (!isFunctionReferenced && !hasUnsafeRefsInNonIIFE && parentNode.arguments.length === 0) {
                        return;
                    }
                }
            }
