...
module.exports = {
    ...
    create(context) {
        ...
        function isNotBaseTen(node) {
            ...
            (node.raw.startsWith("0") && !node.raw.startsWith("0."));
        }
         ...
         */
        function checkNotBaseTenNumber(node) {
        ...
        function normalizeInteger(stringInteger) {
            const significantDigits = removeTrailingZeros(stringInteger);

            return {
                magnitude: stringInteger.length - 1,
        ...
        function normalizeFloat(stringFloat) {
            if (stringFloat.startsWith("0") || stringFloat.startsWith(".")) {
                const decimalDigits = stringFloat.split(".").pop();
            ...
            return {
                magnitude: stringFloat.indexOf(".") - 1,
                coefficient: addDecimalPointToNumber(stringFloat.replace(".", ""))
        ...
        function convertNumberToScientificNotation(stringNumber) {
            const absoluteValue = stringNumber.startsWith("-") ? stringNumber.replace("-", "") : stringNumber;
            const splitNumber = absoluteValue.replace("E", "e").split("e");
         ...
         * @param {Node} node the node being evaluated
         * @returns {boolean} true if they don not match
         */
        function checkBaseTenNumber(node) {
        ...
        function checkNumber(node) {
            return isNotBaseTen(node) ? checkNotBaseTenNumber(node) : checkBaseTenNumber(node);
        }


        return {
            Literal(node) {
                if (node.value && isNumber(node) && checkNumber(node)) {
                    context.report({
