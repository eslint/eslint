...
const numericSeparatorPattern = /_/gu;
const nonBaseTenPrefixPattern = /^0[xbo]/iu;
const legacyOctalPattern = /^0[0-7]+$/u;
...
function getRaw(node) {
	return node.raw.replace(numericSeparatorPattern, "");
}
...
function isBaseTen(node) {
	return (
		!nonBaseTenPrefixPattern.test(node.raw) &&
		!legacyOctalPattern.test(node.raw)
	);
}
...
function losesPrecision(node) {
	/*
	 * Safe integers (abs value <= 2^53 - 1) are exactly representable
	 * in IEEE 754 double precision, so they never lose precision
	 * regardless of the base or notation used in source.
	 */
