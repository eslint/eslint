...
module.exports = {
	...
	create(context) {
		...
		/**
		 * Checks if there are comments between two tokens.
		 * @param {Token} startToken The starting token.
		 * @param {Token} endToken The ending token.
		 * @returns {boolean} True if there are comments between the tokens, false otherwise.
		 */
		function hasCommentsBetweenTokens(startToken, endToken) {
			const tokens = sourceCode.getTokensBetween(startToken, endToken, {
				includeComments: true,
			});

			return tokens.some(isCommentToken);
		}

		/**
		 * Checks if there are comments in Array constructor expressions.
		 * @param {ASTNode} node A CallExpression or NewExpression node.
		 * @returns {boolean} True if there are comments between new/Array or Array/(, false otherwise.
		 */
		function hasCommentsInArrayConstructor(node) {
			if (node.type === "NewExpression") {
				const newToken = sourceCode.getFirstToken(node);
				if (hasCommentsBetweenTokens(newToken, node.callee)) {
					return true;
				}
			}

			const parenToken = sourceCode.getTokenAfter(node.callee);

			if (!parenToken || !isOpeningParenToken(parenToken)) {
				return false;
			}

			return hasCommentsBetweenTokens(node.callee, parenToken);
		}
		...
		function check(node) {
			...
			if (variable && variable.identifiers.length === 0) {
				...
				const shouldSuggest =
					isSpreadCase || hasCommentsInArrayConstructor(node);
				...
				context.report({
					node,
					messageId: shouldSuggest ? "preferLiteral" : messageId,
					fix(fixer) {
						if (shouldSuggest) {
							return null;
						}
