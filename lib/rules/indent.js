...
module.exports = {
    ...
    create(context) {
        ...
        function addElementListIndent(elements, startToken, endToken, offset) {
            ...
            // Whatever the offset type, the banner style aligns the closing bracket with previous elements
            if (typeof offset === "number") {
                offsets.setDesiredOffset(endToken, startToken, options.banner ? offset : 0);
            } else if (options.banner) {
                offsets.matchOffsetOf(getFirstToken(elements[0]), endToken);
            } else {
        ...
        function addParensIndent(tokens) {
            ...
            parenPairs.forEach(pair => {
                ...
                // When the banner option is set, we handle parameter closing parens as part of addElementListIndent
                if (!parameterParens.has(rightParen)) {
        ...
        const baseOffsetListeners = {
            ...
            SwitchStatement(node) {
                ...
                // In the banner style, the closing curly is shifted right
                if (options.banner) {
                    offsets.setDesiredOffset(closingCurly, openingCurly, 1);
                }
            ...
            TemplateLiteral(node) {
                node.expressions.forEach((expression, index) => {
                    ...
                    offsets.setDesiredOffset(sourceCode.getFirstToken(nextQuasi), tokenToAlignFrom, options.banner ? 1 : 0);
                });
            },
