    ...
    create: function(context) {
        ...
        return {
            Property: function(node) {
                ...
                } else if (node.value.type === "Identifier" && node.key.name !== "get" && node.key.name === node.value.name && APPLY_TO_PROPS) {

                    // {x: x} should be written as {x}
                    context.report(node, "Expected property shorthand.");
                } else if (node.value.type === "Identifier" && node.key.type === "Literal" && node.key.value !== "get" && node.key.value === node.value.name && APPLY_TO_PROPS) {
