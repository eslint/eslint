...
module.exports = {
	...
	create(context) {
		...
		/**
		 * Checks whether a function/class expression reaches the given
		 * initializer node through a chain of wrapper calls and/or
		 * logical/conditional operators, e.g. the function expression in
		 * `wrap(function a() {})` or `foo || wrap(function a() {})`.
		 * @param {ASTNode} node The function/class expression node.
		 * @param {ASTNode} initializerNode The initializer node to reach.
		 * @returns {boolean} True if `node` reaches `initializerNode` through wrappers.
		 */
		function isWrappedInitializer(node, initializerNode) {
			let current = node;

			while (current !== initializerNode) {
