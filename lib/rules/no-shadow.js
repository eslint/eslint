...
module.exports = {
	...
	create(context) {
		 ...
		 *    If given `foo?.(a)` in `foo?.(a)`, it returns the surrounding `ChainExpression` node.
		...
		/**
		 * Checks whether a function/class expression is passed directly as an
		 * argument to a single wrapper call that is the initializer, e.g. the
		 * function expression in `wrap(function a() {})`,
		 * `foo || wrap(function a() {})` or `wrap?.(function a() {})`. The call
		 * itself may be wrapped in logical/conditional operators or be part of an
		 * optional chain, but nested calls are not allowed.
		 * @param {ASTNode} node The function/class expression node.
		 * @param {ASTNode} initializerNode The initializer node to reach.
		 * @returns {boolean} True if `node` is a direct argument of the wrapper call.
		 */
		function isWrappedInitializer(node, initializerNode) {
			return (
				node.parent.type === "CallExpression" && // directly passed as a callback
				node.parent.callee !== node &&
				unwrapExpression(node.parent) === initializerNode
			);
		}
		 ...
		 *
		 * It is also not reported when assigned through a wrapper call, as long as
		 * the inner name is never referenced (the reference would otherwise resolve
		 * to the unwrapped function/class rather than the assigned value):
		 *    var a = wrap(function a() {});
		 *
		 * But it should report at declarations such as:
		 *    var a = function(a) {};
		 *    var a = function() { function a() {} };
		 *    var a = wrap(function a() { a(); });
		...
		function isFunctionNameInitializerException(
			...
			/*
			 * Direct assignment, possibly through logical/conditional operators.
			 * The inner name binding refers to the function/class itself, so it
			 * is always safe regardless of whether it is referenced.
			 */
			if (initializerNode === unwrapExpression(nodeToCheck)) {
				return true;
			}

			/*
			 * Assignment through one or more wrapper calls, e.g.
			 * `const TabList = React.forwardRef(function TabList() {})`.
			 * Here the inner name binding refers to the unwrapped function/class,
			 * not the value assigned to the outer variable, so it is only safe
			 * when the inner name is never referenced.
			 */
			return (
				innerVariable.references.length === 0 &&
				isWrappedInitializer(nodeToCheck, initializerNode)
			);
		}
