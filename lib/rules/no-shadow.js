...
const FUNCTION_EXPRESSION_NODES_TYPES = [
    "ArrowFunctionExpression",
    "FunctionExpression"
];

const INSTANTIATION_NODES_TYPES = [
    "ClassDeclaration",
    "FunctionDeclaration"
];

const CALL_EXPRESSION_NODE_TYPE = ["CallExpression"];
...
module.exports = {
    ...
    create(context) {
         ...
         * @returns {[ASTNode, number]} an array that contains the node and its height.
         */
        ...
        function isInitPatternNode(variable, shadowedVariable) {
            ...
            const [functionExpressionNode] = findSelfOrAncestor(
                innerDef.node,
                node => isNodeMatchesTypes(node, FUNCTION_EXPRESSION_NODES_TYPES)
            );

            if (functionExpressionNode) {
                const { parent } = functionExpressionNode;

                const [, instantiationNodeHeight] = findSelfOrAncestor(parent, node => isNodeMatchesTypes(node, INSTANTIATION_NODES_TYPES));

                const [callExpressionNode, callExpressionNodeHeight] = findSelfOrAncestor(parent, node => isNodeMatchesTypes(node, CALL_EXPRESSION_NODE_TYPE));

                if ((callExpressionNode && (callExpressionNodeHeight < instantiationNodeHeight))) {
                    const [shadowingVariableAncestor] = findSelfOrAncestor(
                        callExpressionNode.parent, node => isNodeMatchesTypes(node, INIT_NODES_TYPES)
                    );

                    const shadowingVariableNode = outerDef && outerDef.name;

                    const [shadowedVariableAncestor] = findSelfOrAncestor(shadowingVariableNode.parent, node => (node === shadowingVariableAncestor));

                    return !!shadowedVariableAncestor;
                }
            }
            return false;
        }
