...
const FUNCTIONS_HOISTED_NODES = new Set(["FunctionDeclaration"]);
...
module.exports = {
		...
		schema: [
			{
				...
				properties: {
					...
					hoist: { enum: ["all", "functions", "never", "types", "functions-and-types"] },
					allow: {
						type: "array",
						items: {
							type: "string",
	...
	create(context) {
		const [{ builtinGlobals, hoist, allow, ignoreOnInitialization, ignoreTypeValueShadow = true, ignoreFunctionTypeParameterNameValueShadow = true }] =
			context.options;
		...
		function isGlobalAugmentation(scope) {
			return (
				(scope.type === "tsModule" && scope.block.kind === "global") ||
		...
		function isTypeValueShadow(variable, shadowedVariable) {
			...
			const isShadowedValue = 
				!("isValueVariable" in shadowedVariable) ||
				!firstDefinition ||
		 ...
		 */
		function isFunctionTypeParameterNameValueShadow(variable, shadowedVariable) {
			...
			const isShadowedValue =
				"isValueVariable" in shadowedVariable ? shadowedVariable.isValueVariable : true;
			if (!isShadowedValue) {
				return false;
			}
		...
		function isGenericOfStaticMethod(variable) {
			...
			if (typeParameter.type !== "TSTypeParameter") {
				return false;
			}
			...
			if (
				...
				functionExpr.type !== "TSEmptyBodyFunctionExpression"
			) {
				return false;
			}
		...
		/**
		 * Checks if the variable is a generic of a class
		 * @param {Object} variable The variable to check
		 * @returns {boolean} Whether the variable is a generic of a class
		 */
		 ...
		 */
		function isGenericOfAStaticMethodShadow(variable, shadowedVariable) {
			return isGenericOfStaticMethod(variable) && isGenericOfClass(shadowedVariable);
		}
		...
		function isInTdz(variable, scopeVar) {
			...
			if (hoist === "functions") {
				return !FUNCTIONS_HOISTED_NODES.has(outerDef.node.type);
			}
			...
			if (hoist === "functions-and-types") {
				return (
					!FUNCTIONS_HOISTED_NODES.has(outerDef.node.type) &&
					!TYPES_HOISTED_NODES.has(outerDef.node.type)
				);
			}

			// For "all" and invalid options
			return (
				inner &&
				outer &&
				inner[1] < outer[0] &&
