...
const FUN_DEC_NODES_TYPES = ["MethodDefinition", "FunctionDeclaration"];
...
module.exports = {
    ...
    create(context) {
        const options = {
            ...
            hoist:
                (context.options[0] && context.options[0].hoist) || "functions",
        ...
        function isInitPatternNode(variable, shadowedVariable) {
            ...
            let fun = innerDef && innerDef.node;

            while (fun && !isNodeMatchesTypes(fun, FUN_NODES_TYPES)) {
                if (isNodeMatchesTypes(fun, FUN_DEC_NODES_TYPES)) {
                    fun = null;
                } else {
                    fun = fun.parent;
                }
            ...
            if (fun) {
                ...
                const nestedFun = findSelfOrAncestor(parent, node =>
                    isNodeMatchesTypes(node, FUN_NODES_TYPES));

                if (!nestedFun) {
                    const callExpression = findSelfOrAncestor(parent, node =>
                        isNodeMatchesTypes(node, CALL_EXP_NODE_TYPE));

                    if (callExpression) {
                        const shadowingVariableAncestor = findSelfOrAncestor(
                            callExpression.parent,
                            node => isNodeMatchesTypes(node, INIT_NODES_TYPES)
                        );

                        const shadowingVariable = outerDef && outerDef.name;

                        const shadowedVariableAncestor = findSelfOrAncestor(
                            shadowingVariable,
                            node => node === shadowingVariableAncestor
                        );
