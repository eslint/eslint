...
module.exports = {
    ...
    create(context) {
        ...
        function findSelfOrAncestor(node, match) {
            ...
            while (currentNode) {
                const result = match(currentNode);
        ...
        function isInitPatternNode(variable, shadowedVariable) {
            ...
            const callExpressionNode = findSelfOrAncestor(
                innerDef.node,
                node => {
                    const { type } = node;
            ...
            let callExpressionRoot = callExpressionNode;
            let currentNode = callExpressionRoot && callExpressionRoot.parent;

            while (currentNode && currentNode.type === "MemberExpression") {
                callExpressionRoot = currentNode.parent;
                currentNode = currentNode.parent.parent;
            }
            if (callExpressionRoot) {
                return !!findSelfOrAncestor(
                    ...
                    node => {
                        ...
                        if (rightNode === callExpressionRoot) {
                            return 1;
                        }
                        return 0;
