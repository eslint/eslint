...
const BUILT_IN_ERROR_TYPES_ARRAY = Array.from(BUILT_IN_ERROR_TYPES);
...
module.exports = {
					...
					errorClassNames: {
						...
						items: {
							oneOf: [
								{
									type: "string",
									not: { enum: BUILT_IN_ERROR_TYPES_ARRAY },
								},
								{
									type: "object",
									...
									properties: {
										name: {
											type: "string",
											not: {
												enum: BUILT_IN_ERROR_TYPES_ARRAY,
											},
										},
										argumentPosition: {
											type: "integer",
	...
	create(context) {
		...
		for (const item of errorClassNames) {
			...
			if (errorClassNamesMap.has(name)) {
				throw new Error(
					`Configuration error in preserve-caught-error: Duplicate configuration found for error class "${name}".`,
				);
			}
		...
		function isThrowingNewError(throwStatement) {
			...
			if (
				callee.type === "Identifier" &&
				errorClassNamesMap.has(callee.name)
			) {
				return true;
			}
			...
			if (
				...
				errorClassNamesMap.has(callee.property.name)
			) {
				return true;
			}
