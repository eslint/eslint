...
module.exports = {
		...
		schema: [
			{
				...
				properties: {
					...
					allowedErrorClassNames: {
						...
						uniqueItems: true,
						description:
							"List of custom error class names to apply the rule to, in addition to built-in error types.",
						default: [],
					},
				},
	...
	create(context) {
		...
		const [{ requireCatchParameter, allowedErrorClassNames }] =
			context.options;
		 ...
		 * Also covers custom error class names specified via the `allowedErrorClassNames` option.
		...
		function isThrowingNewError(throwStatement) {
			...
			const { callee } = throwStatement.argument;

			/*
			 * Check built-in error types — must be an actual global reference to avoid false positives.
			...
			/*
			 * Check custom error class names provided via the `allowedErrorClassNames` option.
			 * Custom classes are user-defined and are not subject to the global reference check.
			 */
			if (allowedErrorClassNames.includes(callee.name)) {
				return true;
			}
