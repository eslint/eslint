				...
				if (parentCatch && isThrowingNewError(throwStatement)) {
					const caughtError =
						...
						if (thrownErrorCause === null) {
							// If there is no `cause` attached to error
							context.report({
										...
										fix(fixer) {
											...
											if (
												...
												return fixer.replaceText(
													throwStatement,
													`throw ${throwExpression.type === "NewExpression" ? "new " : ""}${errorType}(${aggregateErrors}, ${errorMessage}, { cause: ${caughtError.name} });`,
												);
											}
