...
module.exports = {
	meta: {
		...
		messages: {
			...
			requireCatchParameter:
				"Start referencing the caught error using the catch parameter.",
			partiallyLostError:
				"Re-throws cannot preserve the caught error as a part of it is being lost due to destructuring.",
			referenceCompleteCaughtError:
				"Make sure to reference the complete caught error when there are re-throws.",
			caughtErrorShadowed:
							...
							suggest: [
								{
									...
									fix(fixer) {
										const replaceRange = [
											parentCatch.range[0], // The start of the catch clause
											parentCatch.param.range[1] + 1, // End of the catch param
										];
								...
								suggest: [
									{
										...
										fix(fixer) {
											const insertAfterRange = [
												parentCatch.range[0], // The beginning of catch keyword
												parentCatch.range[0] + 5, // The end of catch keyword
											];
											return fixer.insertTextAfterRange(
												insertAfterRange,
												"(error)",
							...
							suggest: [
								{
									...
									fix(fixer) {
										/*
										 * In case `cause` is attached using object property shorthand
										 * e.g. throw Error("fail", { cause });
										 */
										if (thrownErrorCause.name === "cause") {
											return fixer.replaceText(
												thrownErrorCause,
												`cause: ${caughtError.name}`,
											);
