...
module.exports = {
	meta: {
		...
		schema: [
			{
				type: "object",
				properties: {
					disallowUncaughtErrors: {
						type: "boolean",
						default: false,
						description:
							"Disallows catch blocks that discard the caught error without referencing it.",
					},
				},
		...
		messages: {
			...
			readCaughtError:
				"Start referencing the caught error that is being discarded.",
		},
		hasSuggestions: true,
	},
		...
		return {
			ThrowStatement(node) {
				...
				if (parentCatch && isThrowingNewError(throwStatement)) {
					...
					if (!caughtError) {
						if (options.disallowUncaughtErrors) {
							context.report({
								node: parentCatch,
