...
module.exports = function(context) {
    ...
    /**
     * Check if a semicolon is oneline exception, only true if:
     *   - next token is brace
     *   - next token is in same line of last token
     * @param {Token} lastToken last token of current node.
     * @returns {boolean} whether the semicolon is unnecessary.
     */
    function isOnelineException(lastToken) {
        var nextToken = context.getTokenAfter(lastToken);
        if (!nextToken) {
            return false;
        }
        var isInSameLine = lastToken.loc.end.line === nextToken.loc.start.line;
        var isBrace = nextToken.value === "}";
        return isInSameLine && isBrace;
    }
        ...
        if (always) {
            if (!isSemicolon(lastToken)) {
                if (!exceptOneline) {
                    ...
                    if (!isOnelineException(lastToken)) {
                        report(node);
                    }
                }
            }
