    ...
    create(context) {
        ...
        function checkRestrictedPathAndReport(importSource, importNames, node) {
            ...
            groupedRestrictedPaths[importSource].forEach(restrictedPathEntry => {
                ...
                const restrictedImportNamePattern = restrictedPathEntry.importNamePattern ? new RegExp(restrictedPathEntry.importNamePattern, "u") : null;
                const allowedImportNames = restrictedPathEntry.allowImportNames;
                const allowedImportNamePattern = restrictedPathEntry.allowImportNamePattern ? new RegExp(restrictedPathEntry.allowImportNamePattern, "u") : null;

                if (restrictedImportNames || restrictedImportNamePattern || allowedImportNames || allowedImportNamePattern) {
                    if (importNames.has("*")) {
                    ...
                    /**
                     * If allowImportNames is set, restrict all names except allowed ones
                     *      (unless name is specifically mentioned in importNames)
                     */
                    importNames.forEach((specifiers, importName) => {
                        let restricted = false;

                        if (allowedImportNames) {
                            if (!allowedImportNames.includes(importName)) {
                                restricted = true;
                            }
                        }

                        if (allowedImportNamePattern && !restricted) {
                            if (!allowedImportNamePattern.test(importName)) {
                                restricted = true;
                            }
                        }

                        if (restrictedImportNames && !restricted) {
                            if (restrictedImportNames.includes(importName)) {
                                restricted = true;
                            }
                        }

                        if (restrictedImportNamePattern && !restricted) {
