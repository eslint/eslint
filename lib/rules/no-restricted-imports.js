...
module.exports = {
	...
	create(context) {
		...
		/**
		 * Check if a type import is allowed for a given path
		 * @param {string} importSource - The path of the import
		 * @returns {boolean} Whether the type import is allowed
		 */
		...
		return {
			...
			TSImportEqualsDeclaration(node) {
				if (node.moduleReference.type === "TSExternalModuleReference") {
					...
					// Create a proper synthesized import declaration
					...
					// Use the existing logic with our synthesized import
					checkRestrictedPathAndReport(
						importSource,
						new Map(), // No specific import names for default import
						synthesizedImport,
					);
				}
			},
