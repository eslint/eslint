...
module.exports = {
	...
	create(context) {
		...
		function reportPathForPatterns(node, group, importNames, importSource) {
			// Use provided importSource or fallback to node.source.value for backward compatibility
			const actualImportSource = importSource || node.source.value.trim();
			...
			if (
				...
				context.report({
					...
					messageId: customMessage
						...
						: "patterns",
					data: {
						importSource: actualImportSource,
						customMessage,
					},
				});
				return;
