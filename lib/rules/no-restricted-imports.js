    ...
    anyOf: [
        ...
        },
        {
            ...
            items: {
                ...
                additionalProperties: false,
                required: ["group"],
                not: { required: ["importNames", "allowImportNames", "importNamePattern"] }
            },
            uniqueItems: true
        }
    ]
...
module.exports = {
    meta: {
        ...
        messages: {
            ...
            allowedImportName: "only '{{allowedImportNames}}' import from '{{importSource}}' is allowed other than that are restricted.",
            // eslint-disable-next-line eslint-plugin/report-message-format -- Custom message might not end in a period
            allowedImportNameWithCustomMessage: "only '{{allowedImportName}}' import from '{{importSource}}' is allowed other than that are restricted. {{customMessage}}",
    ...
    create(context) {
        ...
        const restrictedPatternGroups = restrictedPatterns.map(({ group, message, caseSensitive, importNames, importNamePattern, allowImportNames }) => ({
            matcher: ignore({ allowRelativePaths: true, ignorecase: !caseSensitive }).add(group),
            customMessage: message,
            importNames,
            importNamePattern,
            allowedImportNames: allowImportNames
        }));
        ...
        function reportPathForPatterns(node, group, importNames) {
            ...
            const allowedImportNames = group.allowedImportNames;

            /*
             * If we are not restricting to any specific import names and just the pattern itself,
             * report the error and move on
             */
            if (!restrictedImportNames && !restrictedImportNamePattern && !allowedImportNames) {
                context.report({
                    node,
