...
module.exports = {
	...
	create(context) {
		...
		// Build sets of allowed type imports
		...
		// Function to check if a type import is allowed for a given path
		function isAllowedTypeImportPath(importSource) {
			return allowedTypeImportPathNameSet.has(importSource);
		}

		// Check if a type import is allowed by pattern
		function isAllowedTypeImportPattern(importSource) {
			return restrictedPatternGroups.some(group => {
				if (!group.allowTypeImports) {
					return false;
				}
		...
		function checkRestrictedPathAndReport(importSource, importNames, node) {
			...
			if (node.type === "ExportNamedDeclaration" && isTypeOnlyExport(node) && 
				isAllowedTypeImportPath(importSource)) {
				return;
			}
		...
		return {
			...
			TSImportEqualsDeclaration(node) {
				if (node.moduleReference.type === "TSExternalModuleReference" && 
					...
					// Create a synthetic import declaration to reuse existing logic
					const importSource = node.moduleReference.expression.value;
					const importNames = new Map();
					
					// Only check if it's not a type-only import that's allowed
					if (!(node.isTypeOnly && 
						(isAllowedTypeImportPath(importSource) || 
						isAllowedTypeImportPattern(importSource)))) {
						
						checkRestrictedPathAndReport(importSource, importNames, {
