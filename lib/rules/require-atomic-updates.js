 ...
 * @param {Map<Identifier, escope.Reference>} referenceMap The map from identifier nodes to each reference object.
 * @returns {Map<Identifier, escope.Reference>} `referenceMap`.
 */
function getReferenceMap(scope, referenceMap) {
    for (const reference of scope.references) {
        referenceMap.set(reference.identifier, reference);
    }
    for (const childScope of scope.childScopes) {
        if (childScope.type !== "function") {
            getReferenceMap(childScope, referenceMap);
        }
    }
        ...
        return {
            onCodePathStart(codePath) {
                ...
                stack = {
                    upper: stack,
                    codePath,
                    referenceMap: shouldVerify ? getReferenceMap(scope, new Map()) : null
                };
            },
            onCodePathEnd() {
                stack = stack.upper;
