...
module.exports = {
    ...
    create(context) {
         ...
         */
        function generateReportLocation(char, kind, node) {
            if (node.type === "TemplateLiteral" && node.expressions.length || sourceCode.getText(node).includes("\\")) {
                return node.loc;
            }

            const kindsForEarlierReport = new Set([
                "emojiModifier",
                "zwj",
                node.type === "Literal" && "regionalIndicatorSymbol"
            ]);
            const reportStartOffset = kindsForEarlierReport.has(kind) ? -1 : 0;

            return {
                end: {
                    line: node.loc.end.line,
                    column: node.loc.start.column + char.end + 1
                },
                start: {
                    line: node.loc.start.line,
                    column: node.loc.start.column + char.start + reportStartOffset
                }
            };
        }
        ...
        function verify(node, pattern, flags, unicodeFixer) {
            ...
            for (const { chars, kind } of foundKinds) {
                ...
                for (const char of chars) {
                    context.report({
                        loc: generateReportLocation(char, kind, node),
                        messageId: kind,
                        suggest
                    });
                }
