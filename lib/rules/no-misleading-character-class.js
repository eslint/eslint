    ...
    create(context) {
        ...
        function generateReportLocation(chars, node) {
            ...
            switch (node.type) {
                ...
                case "Literal":
                    if (node.raw.includes("\\u") || (typeof node.value === "string" && node.value !== node.raw.slice(1, -1))) {
                        return null;
                    }
                    break;
            ...
            return {
                    ...
                    column: node.loc.start.column + chars[chars.length - 1].end + 1
                },
                start: {
                    line: node.loc.start.line,
                    column: node.loc.start.column + chars[0].start + 1
                }
            };
        }
        ...
        function verify(node, pattern, flags, unicodeFixer) {
            ...
            for (const { kind, matches } of foundKindMatches) {
                ...
                for (const chars of matches) {
                    context.report({
                        loc: generateReportLocation(chars, node) || node.loc,
                        messageId: kind,
                        suggest
                    });
                }
