...
module.exports = {
    ...
    create(context) {
        ...
        function generateReportLocation(chars, node) {
            ...
            // We only attempt to use a granular location for literals without dynamic expressions
            ...
            // Factoring in backslash-escaped characters, bail if the first character doesn't match real text
            const first = chars[0];
            const reportStartOffset = countDoubleEscapedBackslashes(rawText.slice(0, first.start));

            if (!rawText.slice(first.start, first.end + 1 + reportStartOffset).includes(first.raw)) {
                return node.loc;
            }


            // Factoring in backslash-escaped characters, also bail if the last character doesn't match real text
            const last = chars[chars.length - 1];
            const reportEndOffset = countDoubleEscapedBackslashes(rawText.slice(first.start, last.end));

            if (!rawText.slice(last.start, last.end + last.parent.raw.length + reportEndOffset).includes(last.raw)) {
                return node.loc;
            }
