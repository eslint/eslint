...
const kindsForReportingOnPreviousElement = new Set(["surrogatePair", "surrogatePairWithoutUFlag"]);

const kindsForSingleCharacterAdjustment = new Set(["combiningClass", "surrogatePairWithoutUFlag"]);
...
module.exports = {
    ...
    create(context) {
         ...
         * @param {Object} previousReport Kind and start offset for the previous report, if there was one.
         ...
         */
        function getReportStartOffset(char, kind, node, previousReport = {}) {
            if (node.type !== "Literal" && (node.type !== "TemplateLiteral" || node.expressions.length)) {
                return null;
            }

            // If a problematic character exists twice in the string, start each Nth search after N - 1
            const searchStart = previousReport && previousReport.kind === kind
                ? (previousReport.startOffset + 1)
                : char.start - char.raw.length + 1;

            const searchableLength = searchStart + char.start + char.raw.length * 2;
            const offsetStart = node.raw.slice(0, searchableLength).indexOf(char.raw, searchStart);

            return offsetStart === -1 ? null : offsetStart;
        }

        /**
         * Determines where a report on a character should truly start.
         * @param {number} reportStartOffset Original estimate for where the character is.
         * @param {Character} char Individual character being reported on.
         * @param {string} kind What kind of character report is being made.
         * @param {Node} node Parent string node to report within.
         * @returns {number} Adjusted start location for a character's report.
         */
        function getReportStartOffsetAdjusted(reportStartOffset, char, kind, node) {

            // Characters that aren't started by manual \u encodings are only adjusted by 1 or 2
            if (!char.raw.startsWith("\\u")) {
                return reportStartOffset - (kindsForSingleCharacterAdjustment.has(kind) ? 1 : 2);
            }

            // If the character is the second of a joined pair, report on the previous character's start
            if (kindsForReportingOnPreviousElement.has(kind) && char.raw.startsWith("\\u")) {
                const previousElement = char.parent.elements[char.parent.elements.indexOf(char) - 1];
                const doubleBackslashMatches = node.raw.slice(0, char.start).match(/\\\\/gu);

                return previousElement.start + (doubleBackslashMatches ? doubleBackslashMatches.length : 1);
            }

            // Otherwise, regex literal nodes require no offsets
            if (node.type === "Literal" && node.regex) {
                return reportStartOffset;
            }

            // Other nodes are quoted strings: offset by 1 for their starting quote or backtick
            return reportStartOffset - 1;
        }
        ...
        function generateReportLocation(char, kind, node, reportStartOffset) {
            ...
            const reportStartOffsetAdjusted = getReportStartOffsetAdjusted(reportStartOffset, char, kind, node);
        ...
        function verify(node, pattern, flags, unicodeFixer) {
            ...
            for (const { chars, kind } of foundKinds) {
                ...
                for (const char of chars) {
                    const reportStartOffset = getReportStartOffset(char, kind, node, previousReport);
                    ...
                    previousReport = { kind, startOffset: reportStartOffset };
                }
            }
        }
