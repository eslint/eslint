    ...
    create(context) {
        ...
        return {
            ...
            ":matches(FunctionDeclaration, FunctionExpression)"(node) {
                if (node.params.some(param => param.type === "Identifier" && param.name === "this")) {
                    stack.push({
                        init: true,
                        node,
                        valid: true
            ...
            ":matches(FunctionDeclaration, FunctionExpression):exit"(){
                stack.pop();
            },
            ...
            ThisExpression(node) {
                ...
                if (current && !current.valid && node.parent.parent.type !== "AccessorProperty") {
                    context.report({
                        node,
