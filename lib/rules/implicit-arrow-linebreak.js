            ...
            if (tokenBefore.loc.end.line === fixerTarget.loc.start.line && option === "below") {
                ...
                context.report({
                    ...
                    fix: fixer => {
                        ...
                        const comments = sourceCode.getCommentsAfter(tokenBefore);
                        const hasComments = comments.length > 0;

                        if (hasComments) {
                            const { value, type } = comments[0];

                            const text = type === "Line" ? `//${value}` : `/*${value}*/`;

                            let firstToken = sourceCode.getFirstToken(node);

                            if (node.parent.type === 'VariableDeclarator') {
                                firstToken = sourceCode.getFirstToken(node.parent.parent);
                            }
