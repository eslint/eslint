...
>>>>>>> 759d487d... Add test cases for async arrow functions, method chains, object literals, add parameter and documentation for column for formatComments, add findFirstToken helper
...
=======
         * @returns {Token} The node to prepend comments to
>>>>>>> 759d487d... Add test cases for async arrow functions, method chains, object literals, add parameter and documentation for column for formatComments, add findFirstToken helper
         */
         ...
         * @param {ASTNode} fixerTarget - The arrow function body
         * @returns {Function[]} autofixer -- wraps function bodies with parentheses
         */
        function addParentheses(fixer, arrow, fixerTarget) {
            const parenthesesFixes = [];
            let closingParentheses = "";

            let followingBody = fixerTarget;
            ...
            return [...parenthesesFixes,
                fixer.insertTextAfter(fixerTarget, closingParentheses)
            ];
        }
         ...
         * @param {Token} arrow The arrow token.
         * @param {ASTNode} fixerTarget the function body
         * @param {ASTNode} node The node
         * @returns {Function} autofixer -- validates the node to adhere to besides
         */
        function autoFixBesides(arrow, fixerTarget, node) {
            return fixer => {
                const placeBesides = fixer.replaceTextRange([arrow.range[1], fixerTarget.range[0]], " ");

                const comments = sourceCode.getCommentsInside(node).filter(comment =>
                    comment.loc.start.line < fixerTarget.loc.start.line);
                ...
                if (comments.length) {
                    ...
                    if (sourceCode.getTokenAfter(arrow, isArrowToken)) {
                        return addParentheses(fixer, arrow, fixerTarget);
                    }
