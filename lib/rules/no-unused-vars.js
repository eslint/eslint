...
const { isDefinitionFile } = require("./utils/file-utils");
...
/**
 * Accumulates identifiers within the declaration node.
 * @param {ASTNode} declaration Declaration of a variable.
 * @param {ASTNode[]} identifiers Storage for identifiers within the declaration.
 * @returns {void}
 */
function collectPatternIdentifiers(declaration, identifiers) {
	switch (declaration.type) {
		...
		case "AssignmentPattern":
			collectPatternIdentifiers(declaration.left, identifiers);
			break;
		...
		case "RestElement":
			collectPatternIdentifiers(declaration.argument, identifiers);
			break;

		case "TSAsExpression":
		case "TSNonNullExpression":
		case "TSSatisfiesExpression":
			collectPatternIdentifiers(declaration.expression, identifiers);
			break;
...
module.exports = {
	...
	create(context) {
		 ...
		 * @param {ASTNode[] | Set<ASTNode>} nodes The candidate function nodes.
		...
		/**
		 * Determine whether a variable definition is a type import. e.g. "import type { Foo } from 'foo';".
		...
		/**
		 * Gives a set of AST nodes representing enum declarations for a given variable.
		...
		function handleFixes(fixer, unusedVar) {
			...
			if (parentType === "RestElement") {
				...
				if (
					...
					parent.parent.decorators &&
					parent.parent.decorators.length > 0
				) {
					if (parent.parent.params.length === 1) {
						return fixer.removeRange(parent.range);
					}
		...
		function markDeclarationChildAsUsed(node) {
			...
			switch (node.type) {
				...
				default:
					// no-op or handle unexpected node types if needed
					break;
			}
		...
		return {
			...
			[ambientDeclarationSelector("Program")](node) {
				if (
					...
					!checkForOverridingExportStatements(node.parent)
				) {
			...
			[ambientDeclarationSelector(
				...
				if (!checkForOverridingExportStatements(node.parent.parent)) {
					markDeclarationChildAsUsed(node);
				}
			},
			...
			[ambientDeclarationSelector("TSModuleDeclaration > TSModuleBlock")](
				...
				if (
					isDefinitionFile(context.filename) &&
					!checkForOverridingExportStatements(node.parent.parent)
				) {
					markDeclarationChildAsUsed(node);
				}
			},
