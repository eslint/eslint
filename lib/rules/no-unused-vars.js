...
const { PatternVisitor } = require("@typescript-eslint/scope-manager");
	...
	create(context) {
		...
		function collectUnusedVariables(scope, unusedVars) {
			...
			if (scope.type !== "global" || config.vars === "all") {
				for (let i = 0, l = variables.length; i < l; ++i) {
					...
					// 		if (
					// 			defNode.type === "TSInterfaceDeclaration" ||
					// 			defNode.type === "TSTypeAliasDeclaration" ||
					// 			defNode.type === "ClassDeclaration" ||
					// 			defNode.type === "TSDeclareFunction" ||
					// 			defNode.type === "TSEnumDeclaration" ||
					// 			defNode.type === "VariableDeclarator" ||
					// 			defNode.type === "TSModuleDeclaration"
					// 		) {
					// 			continue;
					// 		}
		...
		function handleFixes(fixer, unusedVar) {
			...
			if (
				...
				if (parent.params.length === 1) {
					return fixer.removeRange([id.decorators[0].range[0], id.range[1]]);
				}
		...
		return {
			...
			Identifier(node) {
                ...
                if (node.typeAnnotation) {
					if (
						...
						node.parent.type === "TSConstructSignatureDeclaration" ||
			...
			TSTypeAnnotation(node) {
				const scope = sourceCode.getScope(node);
				const variable = scope.set.get(node.typeAnnotation);
