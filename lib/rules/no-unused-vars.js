...
module.exports = {
    ...
    create(context) {
        ...
        function handleFixes(fixer, unusedVar) {
            ...
            if (allWriteReferences.length > 1 && parentType !== "AssignmentPattern") {
                return null;
            }

            // skip fix when variable in assignment pattern is reassigned
            if (allWriteReferences.length > 2 && parentType === "AssignmentPattern") {
                return null;
            }

            // skip fix when initialized variable is reassigned
            if (allWriteReferences.length > 0 && parent.init === null && parentType !== "AssignmentPattern") {
                return null;
            }

            // skip fix when function parameters are ressaigned
            if (isFunction(parent) && allWriteReferences.length > 0 && parent.params.includes(id)) {
                return null;
            }

            // skip fix when function parameters with default value are reassigned
            if (isFunction(parent.parent) && parentType === "AssignmentPattern" && allWriteReferences.length > 1) {
                return null;
            }
