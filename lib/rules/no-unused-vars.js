		...
		if (firstOption) {
			if (typeof firstOption === "string") {
				...
				config.ignoreClassWithStaticInitBlock =
					...
					config.ignoreClassWithStaticInitBlock;
				config.ignoreExplicitResourceManagement =
					firstOption.ignoreExplicitResourceManagement ||
					config.ignoreExplicitResourceManagement;
			...
			if (scope.type !== "global" || config.vars === "all") {
				for (i = 0, l = variables.length; i < l; ++i) {
					...
					if (
						!isUsedVariable(variable) &&
						!isExported(variable) &&
						!(config.ignoreExplicitResourceManagement && usesExplicitResourceManagement(variable)) &&
