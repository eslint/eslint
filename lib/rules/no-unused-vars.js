...
module.exports = {
    ...
    create(context) {
         ...
         * @param {Array} writeReferences collection of write references of unused variables
         * @returns {Object} fixer object
         */
        function handleFixes(fixer, unusedVar, writeReferences) {
             ...
             * @returns {Object} fixer object
             */
            function fixObjectAndArrayVariableVariable(node) {
                const parentNode = node.parent;
            ...
            function fixNestedArrayVariable(node) {
                ...
                if (hasSingleElement(parentNode)) {
                    ...
                    // fixes { a: [{ b }] } or { a: [[ b ]] }
                    if (getTokenBeforeValue(parentNode) === ":") {
                        return fixObjectAndArrayVariableVariable(parentNode);
                    }
            ...
            if (writeReferences.length > 1 && parentType !== "AssignmentPattern") {
                return null;
            }

            // skip fix when initialized variable in reassigned
            if (writeReferences.length > 0 && parent.init === null && parentType !== "AssignmentPattern") {
                return null;
            }

            // skip fix when function parameters are ressaigned
            if (isFunction(parent) && writeReferences.length > 0 && parent.params.includes(id)) {
                return null;
            }
            ...
            if (parentType === "VariableDeclarator") {
                if (parent.parent.declarations.length === 1) {
                    ...
                    // removes only variable in 'if (foo()) var bar;' not semicolon.
                    if (parent.parent.parent.type === "IfStatement") {
                        return fixer.removeRange(
                            [sourceCode.getTokenBefore(parent.parent).range[1], parent.range[1]]
                        );
                    }
            ...
            if (parent.parent.type === "ObjectPattern") {
                if (parent.parent.properties.length === 1) {
                     ...
                     * fix const { a: { b } } = foo;
                     */
                    return fixObjectAndArrayVariableVariable(parent.parent);
                }
            ...
            // remove unused varibales inside an array
            if (parentType === "ArrayPattern") {
                if (hasSingleElement(parent)) {
                     ...
                     * fix const { a: [b] } = foo;
                     */
                    return fixObjectAndArrayVariableVariable(parent);
                }
                            ...
                            suggest: [
                                {
                                    ...
                                    fix(fixer) {
                                        return handleFixes(fixer, unusedVar, writeReferences);
                                    }
                                }
                            ]
                        });
