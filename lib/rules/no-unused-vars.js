...
module.exports = function(context) {
    ...
    if (firstOption) {
        if (typeof firstOption === "string") {
            ...
            config.exceptions = firstOption.exceptions || config.exceptions;
            ...
            if (firstOption.exceptionsIgnorePattern) {
                config.exceptionsIgnorePattern = new RegExp(firstOption.exceptionsIgnorePattern);
            }
        }
    }
        ...
        if (scope.type !== "TDZ" && (scope.type !== "global" || config.vars === "all")) {
            for (i = 0, l = variables.length; i < l; ++i) {
                ...
                if (def) {
                    ...
                    if (type === "CatchClause") {
                        if (config.exceptions === "none") {
                            continue;
                        }
                        // skip ignored parameters
                        if (config.exceptionsIgnorePattern && config.exceptionsIgnorePattern.test(def.name.name)) {
                            continue;
                        }
                    }
