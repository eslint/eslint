...
module.exports = {
    ...
    create(context) {
        ...
        function report(node, startOffset, character) {
            const escapeStart = sourceCode.getIndexFromLoc(node.loc.start) + startOffset;

            context.report({
                node,
                loc: sourceCode.getLocFromIndex(escapeStart),
                message: "Unnecessary escape character: \\{{character}}.",
                data: { character },
                fix(fixer) {
                    const escapeEnd = escapeStart + 1;

                    return fixer.removeRange([escapeStart, escapeEnd]);
                }
            });
        }
