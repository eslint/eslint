    ...
    create(context) {
        ...
        function validate(escapes, node, match) {
            ...
            if (isUnnecessaryEscape && !isQuoteEscape) {
                const positionOfEscape = node.loc.start.column + match.index;

                context.report({
                    node,
                    loc: {
                        line: node.loc.start.line,
                        column: positionOfEscape
                    },
                    ...
                    fix(fixer) {
                        let position = positionOfEscape;

                        if (isTemplateElement) {
                            position++;
                        }

                        return fixer.removeRange([position, position + 1]);
                    }
                });
            }
        }
