...
function isUndefined(expression) {
    return (expression.type === "Identifier" && expression.name === "undefined") ||
           (expression.type === "UnaryExpression" && expression.operator === "void" && expression.argument.type === "Literal" && expression.argument.value === 0);
}
 ...
 */
function isImplicitNullishComparison(expression) {
    return expression.type === "BinaryExpression" &&
           expression.operator === "==" &&
           isReference(expression.left) &&
           astUtils.isNullOrUndefined(expression.right);
}
 ...
 */
function isExplicitNullishComparison(expression) {
    return expression.type === "LogicalExpression" &&
           ...
           astUtils.isSameReference(expression.left.left, expression.right.left, true) &&
           ((astUtils.isNullLiteral(expression.left.right) && isUndefined(expression.right.right)) ||
           (isUndefined(expression.left.right) && astUtils.isNullLiteral(expression.right.right)));
}
 ...
 */
function getExistence(expression) {
    ...
    switch (true) {
        ...
        case isImplicitNullishComparison(expression):
            return { reference: expression.left, operator: "??" };
        case isExplicitNullishComparison(expression):
            return { reference: expression.left.left, operator: "??" };
        default: return null;
    }
}
...
module.exports = {
    meta: {
        ...
        messages: {
            ...
            convertLogical: "Convert this logical expression to an assignment.",
    ...
    create(context) {
         ...
         */
        function shouldBeFixed(assignment) {
            return assignment.left.type === "Identifier" &&
                   (context.getScope().isStrict || !isInsideWithBlock(assignment));
        }
         ...
         * @param {ASTNode} assignment Used to check if the fix is safe
         * @returns {Object} Descriptor with either an added fix or suggestion
         */
        function createConditionalFixer(descriptor, suggestion, assignment) {
            if (shouldBeFixed(assignment)) {
                return {
                    ...descriptor,
                    fix: suggestion.fix
                };
        ...
        if (mode === "never") {
            return {
                ...
                "AssignmentExpression"(assignment) {
                    ...
                    context.report(createConditionalFixer(descriptor, suggestion, assignment));
                }
            };
        }
        ...
        return {
            ...
            'LogicalExpression[right.type="AssignmentExpression"][right.operator="="]'(logical) {
                ...
                const suggestion = {
                    ...
                    data: { operator: logical.right.operator },
                    fix(ruleFixer) {
                        ...
                        const removeStart = ruleFixer.removeRange(leftToStartRight); // Also removes start parenthesis

                        const endParenthesis = sourceCode.getTokenAfter(logical.right);
                        const removeEndParenthesis = ruleFixer.remove(endParenthesis);

                        const operatorToken = getOperatorToken(logical.right);
                        const changeOperator = ruleFixer.insertTextBefore(operatorToken, logical.operator);

                        return [
                            removeStart, // -> foo = bar)
                            removeEndParenthesis, // -> foo = bar
                            changeOperator // -> foo ||= bar
                        ];
                    }
                };

                context.report(createConditionalFixer(descriptor, suggestion, logical.right));
            ...
            "IfStatement[alternate=null]"(ifNode) {
                ...
                const existence = getExistence(ifNode.test);
                ...
                if (
                    ...
                    const suggestion = {
                        ...
                        fix(ruleFixer) {
                            ...
                            const logicalOperator = ruleFixer.insertTextBefore(operatorToken, existence.operator);

                            const removeIf = ruleFixer.removeRange([ifNode.range[0], body.range[0]]);
                            const removeAfter = ruleFixer.removeRange([body.range[1], ifNode.range[1]]); // Only present if "if" had a body

                            return [
                                removeIf, // -> foo = bar
                                logicalOperator, // -> foo ||= bar
                                removeAfter // -> foo ||= bar
                            ];
                        }
                    };

                    context.report(createConditionalFixer(descriptor, suggestion, body.expression));
                }
            }
        };
    }
