                ...
                if (
                    ...
                    const suggestion = {
                        ...
                        *fix(ruleFixer) {
                            ...
                            if (astUtils.isParenthesised(sourceCode, body.expression.left) && (prevToken !== null && !astUtils.isSemicolonToken(prevToken))) {

                                // Remove all parenthesis as otherwise the parser may combine the expression with the previous
                                const parens = sourceCode.getTokensBetween({ range: [0, body.range[0]] }, operatorToken).filter(token =>
                                    astUtils.isOpeningParenToken(token) || astUtils.isClosingParenToken(token));

                                yield* parens.map(paren => ruleFixer.remove(paren)); // if (foo) (foo) = bar --> if (foo) foo = bar
                            }
