            ...
            Property(node) {
                ...
                if (sorter(prevName, thisName) > 0) {
                    context.report({
                        ...
                        fix(fixer) {
                            const propertyListIncludesComments = node.parent.properties.some((property) => {
                            ...
                            const propertyListIncludesSpreadProperty = node.parent.properties.some((property) => {
                            ...
                            const buildPropertyRangeIndex = (properties) => {
                                return properties
                                    .map((property) => {
                                        let name;
                            ...
                            const propertyRangeIndex = buildPropertyRangeIndex(node.parent.properties);

                            const newPropertiesText = propertyRangeIndex
                                .sort((property0, property1) => {
                                    return sorter(property0.name, property1.name);
                                })
                                .map((property0) => {
                                    return property0.nodeText;
                                })
                                .join(', ');
