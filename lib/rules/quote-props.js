...
module.exports = function(context) {
    ...
    var MODE = context.options[0],
        ...
        CHECK_COMPUTED = context.options[1] && context.options[1].computed,
        NUMBERS = context.options[1] && context.options[1].numbers,

        MESSAGE_UNNECESSARY_QUOTED = "Unnecessarily quoted property '{{property}}' found.",
    ...
    function checkUnnecessaryQuotes(node) {
        ...
        if (key.type === "Literal" && typeof key.value === "string") {
            ...
            if (CHECK_UNNECESSARY && areQuotesRedundant(tokens, NUMBERS)) {
                context.report(node, MESSAGE_UNNECESSARY_QUOTED, {property: key.value});
            }
    ...
    /**
     * Ensures that a property's key is computed only when necessary
     * @param   {ASTNode} node Property AST node
     * @returns {void}
     */
    function checkUnnecessaryComputed(node) {
        var key = node.key;

        if (key.type === "Literal" && typeof key.value === "string" && CHECK_UNNECESSARY) {
    ...
    return {
        "Property": function(node) {
            if (node.method || node.shorthand) {
                return;
            }

            if (!node.computed && (MODE === "always" || !MODE)) {
            ...
            if (MODE === "as-needed") {
                ...
                } else if (CHECK_COMPUTED) {
                    checkUnnecessaryComputed(node);
                }
            }
        },
