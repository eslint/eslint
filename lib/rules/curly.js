...
/**
 * Returns location of given position with `end` position next to `start` position.
 * @param {Position} position position whose very next needs to be created.
 * @returns {Location} the new location.
 */
function createNextLocation(position) {
    ...
    create(context) {
        ...
        function getReportLoc(node, name, isMissing) {
            ...
            if (name === "else") {
                ...
                if (!isMissing) {
                    const token = sourceCode.getTokenAfter(getElseKeyword(node));

                    return token.loc;
                }

                return getElseKeyword(node).loc;
            }

            if (isMissing) {

                if (name === "for" || name === "do") {
                    const token = sourceCode.getTokenBefore(node.body);

                    return token.loc;
                }

                if (name === "for-of" || name === "for-in") {
                    const token = sourceCode.getTokenAfter(node.right);
            ...
            if (blockStartLoc.has(name)) {
                return createNextLocation(node.body.loc.start);
            }

            if (name === "if") {
                return isMissing ? createNextLocation(node.test.loc.end) : createNextLocation(node.consequent.loc.start);
            }

            if (name === "while") {
                return isMissing ? createNextLocation(astUtils.getNextLocation(sourceCode, node.test.loc.end)) : createNextLocation(node.body.loc.start);
            }
