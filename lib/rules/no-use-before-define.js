...
function isEvaluatedDuringInitialization(reference, options) {
	if (isFromSeparateExecutionContext(reference)) {
		...
		let hasReferenceInFunctionAsArgument = false;
		...
		if (
			...
			if (funcNodes.length > 0) {
				hasReferenceInFunctionAsArgument = funcNodes.some(fn => {
					const idLoc = reference.identifier.range;
					const funcLoc = fn.body.range;

					return idLoc[0] >= funcLoc[0] && idLoc[1] <= funcLoc[1];
				});
			}
		}

		if (
			hasReferenceInFunctionAsArgument &&
