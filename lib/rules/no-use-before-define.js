...
/**
 * Checks whether a function containing a reference is directly assigned
 * as the variable's initializer (provably deferred), as opposed to being
 * passed as an argument to a call or placed in a data structure (potentially
 * invoked immediately).
 *
 * For example:
 *   const x = () => x;          // direct assignment -> true (safe)
 *   const x = fn(() => x);      // argument to call -> false (not safe)
 *   const x = [() => x];        // array element -> false (not safe)
 * @param {eslint-scope.Reference} reference A reference to check.
 * @returns {boolean} `true` if the outermost separating function is directly assigned as the variable's init.
 */
function isDirectFunctionAssignment(reference) {
	const variable = reference.resolved;
	let scope = reference.from;

	/*
	 * Walk up the scope chain to find the outermost function scope
	 * that separates the reference from the variable's scope.
	 */
	let outermostSeparatingFunctionNode = null;

	while (variable.scope.variableScope !== scope.variableScope) {
		if (isClassStaticInitializerScope(scope.variableScope)) {
			scope = scope.variableScope.upper;
		} else {
			// This scope's variableScope is a function scope that separates us
			outermostSeparatingFunctionNode = scope.variableScope.block;
			scope = scope.variableScope.upper;
		}
	}

	if (!outermostSeparatingFunctionNode) {
		return false;
	}

	/*
	 * Walk up the AST from the outermost separating function node
	 * to determine if it's directly assigned as the variable's initializer.
	 */
	let node = outermostSeparatingFunctionNode;
	...
	while (node) {
		...
		if (parent.type === "VariableDeclarator" && parent.init === node) {
			// Check if this declarator defines the same variable
			const defNode = variable.defs[0];

			return defNode && defNode.node === parent;
		}

		if (parent.type === "AssignmentPattern" && parent.right === node) {
			// Default parameter value - considered safe (deferred)
			return true;
		}
		...
		if (
			...
			// Class member - considered safe (deferred)
			return true;
		}

		/*
		 * If we encounter a call expression, array, object, etc.,
		 * the function is NOT directly assigned - it may be invoked immediately.
		 */
		if (
 ...
 * @param {boolean} [options.allowDeferredReferences] Whether to allow all deferred references (default: true).
...
function isEvaluatedDuringInitialization(reference, options = {}) {
	if (isFromSeparateExecutionContext(reference)) {
		if (options.allowDeferredReferences !== false) {
			/*
		...
		/*
		 * When allowDeferredReferences is false, only skip if the function
		 * is directly assigned as the variable's initializer (provably deferred).
		 */
		if (isDirectFunctionAssignment(reference)) {
			return false;
		}

		// Fall through to the initializer range check below
	}
