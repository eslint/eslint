...
module.exports = {
	meta: {
		...
		messages: {
			...
			removeUnusedPrivateClassMember: "Remove '{{classMemberName}}'.",
		},
	},
	...
	create(context) {
		...
		return {
			...
			PrivateIdentifier(privateIdentifierNode) {
				...
				// Track that this member has references beyond its definition
				memberDefinition.hasReferences = true;
			...
			"ClassBody:exit"() {
				...
				for (const [
					classMemberName,
					{ declaredNode, isUsed, hasReferences },
						...
						suggest: [
							{
								...
								*fix(fixer) {
									...
									const members =
										declaredNode.parent.body.filter(
											(m) =>
												m.key.type ===
													"PrivateIdentifier" &&
												m.key.name ===
									...
									// A semicolon may be needed when the first token after all removed members is `[`,
									// to prevent it being parsed as a property access on whatever precedes the block.
									const lastMember = members.at(-1);
									const nextToken =
										sourceCode.getTokenAfter(lastMember);
									const addSemiColon =
									...
									for (const member of members) {
										const tokenAfter =
											sourceCode.getTokenAfter(member, {
												includeComments: true,
											});

										// Walk back through comments immediately preceding the member to include them in the removal
										let startLine = member.loc.start.line;
										let precedingToken = sourceCode.getTokenBefore(
											member,
											{ includeComments: true },
										);
										...
										while (
											precedingToken &&
											(precedingToken.type === "Block" ||
												precedingToken.type === "Line") &&
											precedingToken.loc.end.line + 1 >= startLine
										) {
											startLine = precedingToken.loc.start.line;
											precedingToken = sourceCode.getTokenBefore(
												precedingToken,
												{ includeComments: true },
											);
										}
										...
										const start =
											!precedingToken ||
											precedingToken.loc.end.line < startLine
												? sourceCode.lineStartIndices[
													startLine - 1
												]
									...
									if (addSemiColon) {
										const firstMemberIndex =
											declaredNode.parent.body.indexOf(
												members[0],
											);
										const priorMember =
											declaredNode.parent.body[
												firstMemberIndex - 1
											];
