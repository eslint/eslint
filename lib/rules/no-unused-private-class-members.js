...
module.exports = {
    ...
    create(context) {
         ...
         * @private
         */
        function isWriteOnlyAssignment(privateIdentifierNode, isSetter) {
        ...
        return {
            ...
            PrivateIdentifier(privateIdentifierNode) {
                const propertyDefinition = trackedClasses[0].get(privateIdentifierNode.name);

                // In case any other usage was already detected, the property was already removed from the map
                ...
                if (isWriteOnlyAssignment(privateIdentifierNode, propertyDefinition.isSetter)) {
                    return;
                }
                ...
                if (wrappingExpressionType === "ArrayPattern" &&
                    parentOfWrappingExpressionType === "AssignmentExpression") {
                    return;
                }

                trackedClasses[0].delete(privateIdentifierNode.name);
            },
            ...
            "ClassBody:exit"() {
                ...
                for (const [classMemberName, { declaredNode }] of unusedPrivateMembers.entries()) {
                    context.report({
                        node: declaredNode,
