...
module.exports = {
    ...
    create(context) {
        ...
        function isWriteOnlyAssignmentStatementWithoutPerformingAnyReads(privateIdentifierNode, isSetter) {
            if (privateIdentifierNode.parent.parent.type !== "AssignmentExpression") {
                return false;
            }
            ...
            const assignmentExpression = privateIdentifierNode.parent.parent;

            // It is a write-only usage, since we still allow usages on the right for reads
            if (assignmentExpression.left !== privateIdentifierNode.parent) {
                return false;
            }
            ...
            if (assignmentExpression.operator !== "=") {
                 ...
                 */
                return assignmentExpression.parent.type === "ExpressionStatement";
            }
        ...
        /**
         * Check whether the current node is in a write only for-loop
         * @param {ASTNode} privateIdentifierNode Node referring to a private identifier
         * @returns {boolean} Whether the node is a write only for-loop
         */
        function isWriteOnlyInAssignmentExpression(privateIdentifierNode) {
            if (privateIdentifierNode.parent.parent.type !== "ForInStatement" &&
                privateIdentifierNode.parent.parent.type !== "ForOfStatement" &&
                privateIdentifierNode.parent.parent.type !== "AssignmentPattern") {
                return false;
            }

            const forLoopStatement = privateIdentifierNode.parent.parent;

            return forLoopStatement.left === privateIdentifierNode.parent;
        }
        ...
        return {
            ...
            PrivateIdentifier(privateIdentifierNode) {
                ...
                if (isWriteOnlyInAssignmentExpression(privateIdentifierNode)) {
                    return;
                }
