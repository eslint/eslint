...
module.exports = function(context) {
    ...
    function isMultiline(node) {
        var items = node.properties || node.elements || node.specifiers;
        var firstItem = lodash.first(items);
        var lastItem = lodash.last(items);
        if (!firstItem || !lastItem) {
            return false;
        }

        if (firstItem !== lastItem) {
            return firstItem.loc.start.line !== lastItem.loc.end.line;
        }
