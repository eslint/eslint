...
/**
 * Gets the index of the `startLocation` in `tokens`.
 * `startLocation` can be the value of `node.range[1]`, so this checks about `startLocation - 1` as well.
 *
 * @param {(Token|Comment)[]} tokens - The tokens to find an index.
 * @param {Object} imap - The map from locations to indices.
 * @param {number} startLocation - The location to get an index.
 * @returns {number} The index.
 * @private
 */
function getFirstIndex(tokens, imap, startLocation) {
    if (typeof imap[startLocation] !== "undefined") {
        return imap[startLocation];
    }
    if (typeof imap[startLocation - 1] !== "undefined") {
        return imap[startLocation - 1] + 1;
    }
    return 0;
}

/**
 * Gets the index of the `endLocation` in `tokens`.
 * The information of end locations are recorded at `endLocation - 1` in `imap`, so this checks about `endLocation - 1` as well.
 *
 * @param {(Token|Comment)[]} tokens - The tokens to find an index.
 * @param {Object} imap - The map from locations to indices.
 * @param {number} endLocation - The location to get an index.
 * @returns {number} The index.
 * @private
 */
function getLastIndex(tokens, imap, endLocation) {
    if (typeof imap[endLocation] !== "undefined") {
        return imap[endLocation] - 1;
    }
    if (typeof imap[endLocation - 1] !== "undefined") {
        return imap[endLocation - 1];
    }
...
class ForwardTokenCursor extends Cursor {
     ...
     * @param {Object} imap - The map from locations to indices in `tokens`.
    ...
    constructor(tokens, comments, imap, start, end) {
        super();
        this.tokens = tokens;
        this.index = getFirstIndex(tokens, imap, start);
        this.indexEnd = getLastIndex(tokens, imap, end);
    }
...
class BackwardTokenCursor extends Cursor {
    ...
    constructor(tokens, comments, imap, start, end) {
        super();
        this.tokens = tokens;
        this.index = getLastIndex(tokens, imap, end);
        this.indexEnd = getFirstIndex(tokens, imap, start);
    }
...
class ForwardTokenCommentCursor extends Cursor {
    ...
    constructor(tokens, comments, imap, start, end) {
        ...
        this.tokenIndex = getFirstIndex(tokens, imap, start);
        this.commentIndex = search(comments, start);
        this.border = end;
    }
...
class BackwardTokenCommentCursor extends Cursor {
    ...
    constructor(tokens, comments, imap, start, end) {
        ...
        this.tokens = tokens;
        this.comments = comments;
        this.tokenIndex = getLastIndex(tokens, imap, end);
        this.commentIndex = search(comments, end) - 1;
        this.border = start;
    }
...
class CursorPair {
     ...
     * @private
     */
    createCursor(tokens, comments, imap, start, end, includeComments) {
        return new (includeComments ? this.TokenCommentCursor : this.TokenCursor)(tokens, comments, imap, start, end);
    }
}
 ...
 * @private
 */
function createCursor(cursors, tokens, comments, imap, start, end, includeComments, filter, skip, count) {
    let cursor = cursors.createCursor(tokens, comments, imap, start, end, includeComments);
 ...
 * @private
 */
function createCursorWithSkip(cursors, tokens, comments, imap, start, end, opts) {
    ...
    return createCursor(cursors, tokens, comments, imap, start, end, includeComments, filter, skip, -1);
}
 ...
 * @private
 */
function createCursorWithCount(cursors, tokens, comments, imap, start, end, opts) {
    let includeComments = false;
    let count = 0;
    let filter = null;
    ...
    return createCursor(cursors, tokens, comments, imap, start, end, includeComments, filter, 0, count);
}
...
function createCursorWithPadding(tokens, comments, imap, start, end, beforeCount, afterCount) {
    if (typeof beforeCount === "undefined" && typeof afterCount === "undefined") {
        return new ForwardTokenCursor(tokens, comments, imap, start, end);
    }
    if (typeof beforeCount === "number" || typeof beforeCount === "undefined") {
        return new PaddedTokenCursor(tokens, comments, imap, start, end, beforeCount | 0, afterCount | 0);
    }
    return createCursorWithCount(forwardCursors, tokens, comments, imap, start, end, beforeCount);
}
