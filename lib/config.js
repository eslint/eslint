...
class Config {
    ...
    constructor(options, linterContext) {
        ...
        let commandLineConfig = options.configFile;

        if (commandLineConfig) {
            debug(`Using command line config ${commandLineConfig}`);
            const isNamedConfig =
                isResolvable(commandLineConfig) ||
                isResolvable(`eslint-config-${commandLineConfig}`) ||
                commandLineConfig.charAt(0) === "@";

            if (!isNamedConfig) {
                commandLineConfig = path.resolve(this.options.cwd, commandLineConfig);
            }

            this.useSpecificConfig = ConfigFile.load(commandLineConfig, null, this);
        }
        ...
        if (typeof this.personalConfig === "undefined") {
            ...
            if (PERSONAL_CONFIG_DIR) {
                ...
                if (filename) {
                    ...
                    config = ConfigFile.load(filename, null, this.configCache);
                }
            }
    ...
    getLocalConfigHierarchy(directory) {
        ...
        for (const localConfigFile of localConfigFiles) {
            ...
            const localConfig = ConfigFile.load(localConfigFile, null, this.configCache);
        ...
        if (!configs.length && !this.useSpecificConfig) {
