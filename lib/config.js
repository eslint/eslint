...
function Config(options) {
    ...
    useBaseConfig = options.baseConfig;

    if (options.reset || useBaseConfig === false) {
        this.baseConfig = { rules: {} };
    } else if (typeof useBaseConfig === "object" && useBaseConfig !== null) {
        this.baseConfig = useBaseConfig;
    } else {
        if (typeof useBaseConfig === "string") {
            useBaseConfig = path.resolve(process.cwd(), useBaseConfig);
        } else {
            useBaseConfig = path.resolve(__dirname, "..", "conf", "eslint.json");
        }
        this.baseConfig = require(useBaseConfig);
    }
