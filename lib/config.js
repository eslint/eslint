...
var LOCAL_CONFIG_FILENAMES = [".eslintrc", ".eslintrc.yaml", ".eslintrc.yml", ".eslintrc.json"],
    PACKAGE_CONFIG_FILENAME = "package.json",
    PACKAGE_CONFIG_FIELD_NAME = "eslintConfig",
    PERSONAL_CONFIG_PATHS = userHome ? LOCAL_CONFIG_FILENAMES.map(function(fileName) {
...
function getPersonalConfig() {
    var config = {};
    var personalConfigPath = PERSONAL_CONFIG_PATHS && find(PERSONAL_CONFIG_PATHS, fs.existsSync, fs);
...
function getLocalConfig(thisConfig, directory) {
    var found,
        ...
        rootPath,
        projectConfigPaths = LOCAL_CONFIG_FILENAMES.map(function(fileName) {
    ...
    for (i = 0; i < numFiles; i++) {
        ...
        if (PERSONAL_CONFIG_PATHS.indexOf(localConfigFile) > -1 && projectConfigPaths.indexOf(localConfigFile) === -1) {
            continue;
        }
