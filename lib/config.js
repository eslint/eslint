...
class Config {
    ...
    constructor(providedOptions, linterContext) {
        ...
        this._rootConfigReferencePath = path.join(this.options.cwd, "placeholder.js");

        this.baseConfig = options.baseConfig
            ? ConfigFile.loadObject(options.baseConfig, this, this._rootConfigReferencePath, "(Provided base config)")
            : { rules: {} };
    ...
    loadSpecificConfig(config) {
        if (config) {
            ...
            const isLocalPath = fs.existsSync(path.resolve(this.options.cwd, config)) && !config.startsWith("@");

            this.specificConfig = ConfigFile.load(
                isLocalPath ? path.resolve(this.options.cwd, config) : config,
                this,
