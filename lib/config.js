...
/**
 * Adds the given baseDirectory to each glob configuration item of the given configuration array.
 * @param {Object[]} configurations List of glob configurations
 * @param {string} baseDirectory Directory path where the configuration file was found
 * @returns {void}
 */
function addBaseDirectoryToGlobConfigurations(configurations, baseDirectory) {
...
function getLocalConfig(thisConfig, directory) {
    ...
    for (i = 0; i < numFiles; i++) {
        ...
        if (localConfig.files) {
            addBaseDirectoryToGlobConfigurations(localConfig.files, path.dirname(localConfigFile));
        }
 ...
 */
Config.prototype.getConfigForDirectory = function(filePath) {
    var config,
        userConfig,
...
/**
 * Applies all glob pattern based configuration for a given file path and directory config.
 * @param {string} filePath a file to build the config for
 * @param {Object} directoryConfig config object of a directory
 * @returns {Object} config object
 */
Config.prototype.getConfigForFile = function(filePath, directoryConfig) {
    var config = ConfigOps.merge({}, directoryConfig),
        globConfigurations = directoryConfig.files;

    if (globConfigurations) {
        delete config.files;

        globConfigurations.forEach(function(globConfiguration) {
            var relativePath = filePath.substr(globConfiguration.baseDirectory.length + 1);
            var matches = globConfiguration.patterns.some(function(pattern) {
                return minimatch(relativePath, pattern);
            });

            if (matches) {
                config = ConfigOps.merge(config, globConfiguration.config);
            }
        });
    }
...
/**
 * Convienent method for calling getConfigForFile with getConfigForDirectory as directory config on the same file path
 * @param {string} filePath a file in whose directory we start looking for a local config
 * @returns {Object} config object
 */
Config.prototype.getConfig = function(filePath) {
    var directoryConfig = this.getConfigForDirectory(filePath);

    return this.getConfigForFile(filePath, directoryConfig);
};
