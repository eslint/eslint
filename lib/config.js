...
/**
 * Look file in directory and its parents
 * @param  {string} directory The path of directory to start
 * @param  {string} lookFor   name of file
 * @returns {srting}           Path to found file
 */
function findFileRecursive(directory, lookFor) {
    ...
    return findFileRecursive(parentDirectory, lookFor);
}
...
Config.prototype.findLocalConfigFile = function (directory) {
    return findFileRecursive(directory, LOCAL_CONFIG_FILENAME);
};
...
Config.prototype.findIgnoreFile = function (directory) {
    return findFileRecursive(directory, ESLINT_IGNORE_FILENAME);
};
