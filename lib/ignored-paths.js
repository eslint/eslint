 ...
 */
function findIgnoreFile(eslintrcDirectory) {
    if (!ignoreFileFinder) {
        ignoreFileFinder = new FileFinder(ESLINT_IGNORE_FILENAME);
    }

    return ignoreFileFinder.findInDirectoryOrParents(eslintrcDirectory);
}
...
IgnoredPaths.load = function (options) {
    ...
    if (options.configFile) {
        eslintrcDirectory = path.dirname(options.configFile);
    }

    if (options.ignore) {
        patterns = loadIgnoreFile(options.ignorePath || findIgnoreFile(eslintrcDirectory));
    } else {
        patterns = [];
    }
