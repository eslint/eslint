...
CLIEngine.prototype = {
    ...
    executeOnFiles: function(files) {
         ...
         * @param {string} pattern Pattern as passed
         * @param {string} filename The file to be linted
         * @returns {void}
         */
        function executeOnFile(pattern, filename) {
            // file has already been processed
            if (processed[filename]) {
                return;
            }

            // only check for extensions if the pattern has not extension in it
            if (path.extname(pattern) === "" && extensions.indexOf(path.extname(filename)) === -1) {
                return;
            }
        ...
        files.forEach(function(pattern) {
            glob.sync(pattern, globOptions).forEach(executeOnFile.bind(null, pattern));
        });
