...
class CLIEngine {
    ...
    // FIXME: avoid boolean trap
    // FIXME: it's messy for executeOnFiles to sometimes return a Promise, might be better to create a new method.
    /**
     * Executes the current configuration on an array of file and directory names.
     * @param {string[]} patterns An array of file and directory names.
     * @returns {Object} The results for all files that were linted.
     */
    executeOnFiles(patterns, useWorkers) {
        const promises = [],
        ...
        const cacheFile = getCacheFile(this.options.cacheLocation || this.options.cacheFile, this.options.cwd);
        const workers = workerFarm(require.resolve("./process-file"));
        ...
        function executeOnFile(filename, warnIgnored) {
            ...
            if (warnIgnored) {
                promises.push(Promise.resolve(createIgnoreResult(filename, options.cwd)));
                return;
            }
            ...
            if (options.cache) {
                ...
                if (!changed) {
                     ...
                     */
                    promises.push(Promise.resolve(descriptor.meta.results));
            ...
            if (useWorkers) {
                promises.push(new Promise(resolve => {
                    workers(filename, configHelper.getConfig(filename), options, this.options.rulePaths || [], this.options.cwd, res => {
                ...
                // FIXME: this is copy-pasted
                require("./process-file")(filename, configHelper.getConfig(filename), options, this.options.rulePaths || [], this.options.cwd, res => {
                    if (options.cache) {
        ...
        fileList.forEach(fileInfo => {
            ...
            // FIXME: this is ugly, shouldn't need to use .call
            executeOnFile.call(this, fileInfo.filename, fileInfo.ignored, this.linter);
        });
