...
function processText(text, configHelper, filename, fix, allowInlineConfig) {
    ...
    if (processor) {
        ...
        parsedBlocks.forEach(block => {
            if (fix && processor.autoFix) {
                const output = multipassFix(block, config, {
                    filename,
                    allowInlineConfig
                });

                unprocessedMessages.push(output.messages);
                fixedResults.push(output.output);
                fixed = !fixed && output.fixed;
            } else {
        ...
        if (processor.autoFix) {
            fixedResult = {
                messages,
                fixed,
                output: processor.autoFix(fixedResults, filename)
            };
        }
    } else {
