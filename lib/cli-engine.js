...
var fs = require("fs"),
    ...
    ProcessPool = require("./process-pool"),
...
CLIEngine.prototype = {
    ...
    executeOnFiles: function(patterns) {
        ...
        var pool = new ProcessPool(path.join(__dirname, "/execute.js"), function() { process.exit(0);});

        patterns.forEach(function(pattern) {
            if (shell.test("-f", pattern) && !ignoredPaths.contains(pattern)) {
                executeOnFile(pattern);
            } else {
                glob.sync(pattern, globOptions).forEach(function(item) {
                    var filepath = path.resolve(item);
                    var config = configHelper.getConfig(filepath);
                    pool.addTask({filename: item, config: config}, function(result) {
                        results.push(result);
                    });
                });
            }
