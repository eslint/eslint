 ...
 * @param {string} config.filePath The path to the file of `text`.
...
function verifyText({
    ...
    /*
     * `filePath` is `/path/of/cwd/<text>` if a user didn't specify filename on
     * `CLIEngine#executeOnText()`, so it extracts the `<text>` part for the
     * lint result. (In another case, it may be `/path/of/cwd/<input>`.)
     * On the other hand, `ConfigArray#extractConfig(filePath)` requires an
     * absolute path, so it passed `filePath` as is to the `Linter` object.
     */
