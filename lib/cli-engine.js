...
CLIEngine.prototype = {
    ...
    executeOnText: function(text, filename) {
        ...
        globOptions = {
            ...
            realpath: false,
            stat: false,
            ignore: ignoredPaths.map(processPath)
        };

        if (filename && options.ignore && glob.sync(filename, globOptions).length === 0) {
    ...
    isPathIgnored: function(filePath) {
        ...
        if (this.options.ignore) {
            ignoredPaths = IgnoredPaths.load(this.options).patterns;
            globOptions = {
                nodir: true,
                realpath: true,
                ignore: ignoredPaths.map(processPath)
            };
            return glob.sync(filePath, globOptions).length === 0;
        }
