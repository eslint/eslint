...
function parse(text, providedParserOptions, parserName, parserMap, filePath) {
    ...
    try {
        parser = (typeof parserName.parse === "function") ? parserName : (parserMap.get(parserName) || require(parserName));
    } catch (ex) {
        return {
            success: false,
            error: {
    ...
    const sharedTraversalContext = Object.freeze(
        Object.assign(
            Object.create(BASE_TRAVERSAL_CONTEXT),
            {
                ...
                parserOptions,
                parserPath: typeof parserName === "string" ? parserName: undefined,
                parser: typeof parserName.parse === "function" ? parserName : undefined,
...
module.exports = class Linter {
    ...
    verify(textOrSourceCode, config, filenameOrOptions) {
        ...
        // load rule defs from simple config
        if (config.defs && config.defs.ruleNamespaces) {
            for (const namespace of Object.keys(config.defs.ruleNamespaces)) {
                for (const ruleId of Object.keys(config.defs.ruleNamespaces[namespace])) {
                    this.defineRule(`${namespace}/${ruleId}`, config.defs.ruleNamespaces[namespace][ruleId]);
                }
            }
        }
