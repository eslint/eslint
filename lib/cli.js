 ...
 * @param {string} source If receiving input via stdin, the source code to lint, otherwise null/undefined.
 * @returns {int} The total number of errors.
 */
function processFile(filename, configHelper, source) {

    // clear all existing settings for a new file
    eslint.reset();

    var filePath = filename !== "<stdin>" ? path.resolve(filename) : filename,
        config,
 ...
 * @returns {int} The total number of errors.
 */
function processFiles(files, configHelper, source) {
    ...
    files.forEach(function(file) {
        ...
        if (file !== "<stdin>" && isDirectory(file)) {
    ...
    errors = fullFileList.reduce(function(previous, file) {
        return previous + processFile(file, configHelper, source);
    }, 0);
...
var cli = {
    ...
    execute: function(argv, source) {
        ...
        // Normalize source to null if undefined
        if (source === undefined) {
            source = null;
        }
        ...
        if (currentOptions.v) { // version from package.json
        ...
        } else if (currentOptions.h || (!files.length && source === null)) {
            ...
            result = processFiles(files, configHelper, source);
