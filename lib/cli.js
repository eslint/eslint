...
const cli = {
	...
	async execute(args, text) {
		...
		let suppressionsFileLocation;

		/** @type {SuppressionsService|undefined} */
		let suppressionsService;

		/** @type {SuppressedViolations|undefined} */
		let suppressions;

		if (!useStdin) {
			suppressionsFileLocation = getCacheFile(
			...
			if (options.fix || options.fixDryRun) {
				suppressions = await suppressionsService.load();
			}
		}
		...
		if (options.concurrency !== "off") {
			...
			engine = await ESLint.fromOptionsModule(optionsURL, {
				[suppressionsSnapshot]: suppressions,
			});
		} else {
			const eslintOptions = await translateOptions(options);

			if (suppressions) {
				eslintOptions[suppressionsSnapshot] = suppressions;
			}
