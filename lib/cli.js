...
/**
 * Creates an option module from the provided CLI options and encodes it as a data URL.
 * @param {ParsedCLIOptions} options The CLI options.
 * @returns {string} The URL of the option module.
 */
function createOptionModule(options) {
	...
	return `data:text/javascript;base64,${Buffer.from(optionsSrc).toString("base64")}`;
}
	...
	async execute(args, text, allowFlatConfig = true) {
		...
		if (options.printConfig) {
			...
			const engine = usingFlatConfig
				...
				: new LegacyESLint(await translateOptions(options, false));
		...
		if (options.concurrency && options.concurrency !== "off") {
			const optionsURL = createOptionModule(options);
			engine = await ESLint.fromOptionModule(optionsURL);
		} else {
