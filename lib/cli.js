...
const cli = {
    ...
    async execute(args, text) {
        ...
        if (currentOptions.version) {
            ...
            const engine = new CLIEngine(translateOptions(currentOptions));
            const parallel = currentOptions.parallel;
            let report;

            if (useStdin) {
                report = engine.executeOnText(text, currentOptions.stdinFilename, true);
            } else if (parallel) {
                report = await engine.executeOnFilesInParallel(files);
            } else {
