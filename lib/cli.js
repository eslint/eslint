...
function printResults(config) {
    ...
    try {
        ...
        printError("Could not find formatter '%s'.", config.format);
        return false;
    }
...
function processFile(filename, configHelper) {
    ...
    if (existsSync(filePath)) {
        ...
        printError("Could not find file at '" + filePath + "'.");
        return 1;
    }
...
function processFiles(files, configHelper) {
    ...
    return (config && printResults(configHelper.getConfig())) ? errors : 1;
...
var cli = {
    ...
    execute: function(argv) {
        ...
        if (currentOptions.v) { // version from package.json
            ...
            if (configHelper.useSpecificConfig === false) {
                return 0;
            }
