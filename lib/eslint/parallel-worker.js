...
/**
 * Get the parent port.
 * @returns {{workerId:number, master:MessagePort}} The parent port.
 */
function getThreadParentPort() {
    ...
    return { workerId: threadId, master: parentPort };
}

/**
 * Get the parent port.
 * @returns {{workerId:number, master:NodeJS.Process}} The parent port.
 */
function getProcessParentPort() {
    process.postMessage = process.send;
    return { workerId: process.pid, master: process };
}

// Select the implementation.
const { master, workerId } =
...
function sendResult(senderId, result) {
    return new Promise(resolve => {
        master.on("message", function listener(msg) {
            if (msg.senderId === senderId) {
                master.removeListener("message", listener);
                resolve(msg.next);
            }
        });
        master.postMessage({ senderId, result });
    });
}
...
(async function main() {
    ...
    master.setMaxListeners(initialFiles.length);
