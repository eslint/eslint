...
async function createConfigArray({
    ...
    if (shouldUseIgnoreFile) {
        ignoreFilePath = path.resolve(cwd, ignorePath || ".eslintignore");
    ...
    if (allIgnorePatterns.length) {
        ...
        const ignoreFileDirectory = path.dirname(ignoreFilePath || cwd);

        // add the ignores to the front of the config array
        configs.unshift({
            ignores: [filePath => {
                const relativePath = path.relative(ignoreFileDirectory, filePath);
...
class FlatESLint {
    ...
    async calculateConfigForFile(filePath) {
        ...
        const options = this[ESLintSymbol.options];
        const absolutePath = path.resolve(options.cwd, filePath);
        const configs = await createConfigArray(options);
    ...
    async isPathIgnored(filePath) {
        ...
        return configs.isIgnored(absolutePath);
    }
}
