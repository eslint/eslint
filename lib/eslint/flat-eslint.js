...
async function calculateConfigArray(eslint, {
    ...
    if (overrideConfigFile) {
        configFilePath = overrideConfigFile;
    } else if (useESLintRC) {
...
function verifyText({
     ...
     */
    const filePathToVerify = filePath === "<text>" ? path.join(cwd, "code.js") : filePath;
 ...
 * @returns {Rule|null} The rule or undefined.
 * @throws {TypeError} If the plugin cannot be found.
 */
function getRule(ruleId, config) {
    ...
    if (!plugin) {
        throw new TypeError(`Could not find plugin "${pluginName}".`);
    }

    return plugin.rules && plugin.rules[ruleName];
}
...
function *iterateRuleDeprecationWarnings(config) {
    ...
    for (const [ruleId, ruleConfig] of Object.entries(config.rules)) {
        ...
        const rule = getRule(ruleId, config);
...
class FlatESLint {
    ...
    constructor(options = {}) {
        ...
        const linter = new Linter({ cwd: processedOptions.cwd });
    ...
    async lintText(code, options = {}) {
        ...
        const resolvedFilename = filePath && path.resolve(cwd, filePath);
        let config;
        ...
        if (resolvedFilename && this.isPathIgnored(resolvedFilename)) {
            ...
            config = configs.getConfig(filePath);
