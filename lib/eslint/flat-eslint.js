...
async function loadFlatConfigFile(filePath) {
    ...
    return new FlatConfigArray(module.default, {
        basePath: path.resolve(path.dirname(filePath))
    });
}
...
async function calculateConfigArray(eslint, {
    ...
    if (allIgnorePatterns.length) {
        ...
        configs.unshift({
            ...
            /*
             * TODO: We need to get ignore patterns as strings in the config
             * array for proper optimization. This currently doesn't work because
             * config array doesn't handle negation patterns in the ignores field.
             * Once it does, we can update this logic to the following.
             */
    ...
    const { fixed, messages, output } = linter.verifyAndFix(
        ...
        configs,
        {
            ...
            filterCodeBlock(blockFilename) {
                return !configs.isIgnored(blockFilename);
            }
        }
    );
...
class FlatESLint {
    ...
    async lintFiles(patterns) {
        ...
        if (filePaths.length === 0) {
            throw new Error(`No files matching '${patterns}' were found${globInputPaths ? "" : " (glob was disabled)"}.`);
        }

        debug(`Files found in: ${Date.now() - startTime}ms`);
