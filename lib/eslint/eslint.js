...
/**
 * Aggregates timing data from multiple sources into a unified report.
 * @param {Array<Object<string, number>>} timingDataArray Array of timing data objects from workers.
 * @returns {void}
 */
function aggregateAndDisplayTimingData(timingDataArray) {
	timing.reset();

	for (const workerTimingData of timingDataArray) {
		timing.mergeData(workerTimingData);
	}
...
async function runWorkers(
	...
	function workerExecutor(resolve_, reject) {
		...
		worker.once(
			"message",
			(
				/** @type {{ results: WorkerLintResults, timingData: Object | null }} */ message,
			) => {
				const workerDuration = hrtimeBigint() - workerStartTime;
				const { results: indexedResults, timingData } = message;
				...
				if (timingData) {
					collectedTimingData.push(timingData);
				}
				resolve_();
			},
		);
	...
	if (timing.enabled && collectedTimingData.length > 0) {
		aggregateAndDisplayTimingData(collectedTimingData);
	}
