 ...
 * @property {number} [concurrency] The number of threads. If `1` is present then ESLint doesn't make worker threads. Defaults to a dependent variable on the number of target files.
 */

/**
 * @typedef {Object} ESLintInternalSlots
 * @property {CLIEngine} engine The CLI engine.
 * @property {number} concurrency The number of threads. If `1` is present then ESLint doesn't make worker threads.
 */
...
class ESLint {
    ...
    constructor({
        ...
        if (concurrency !== void 0 && !(Number.isSafeInteger(concurrency) && concurrency >= 1)) {
            throw new TypeError("'concurrency' should be a positive integer.");
        }
    ...
    async executeOnFiles(patterns) {
        ...
        return executeOnFiles(
            ...
            async(targets, postprocess) => {
                ...
                const realConcurrency =
                    concurrency || estimateBestConcurrency(targets.length);
