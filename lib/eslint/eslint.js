...
class ESLint {
     ...
     * @returns {AsyncIterableIterator<LintResult>} The results for all files that were linted.
     */
    async *executeOnFiles(patterns) {
        ...
        try {
            ...
            const results = context.concurrency === 1
     ...
     * @returns {Promise<LintResult>} The results for the linting.
     */
    executeOnText(text, filename, warnIgnored) {
        ...
        try {
            const { engine } = internalSlotsMap.get(this);
            const { results } = engine.executeOnText(text, filename, warnIgnored);

            promise = Promise.resolve(results[0]);
        } catch (error) {
