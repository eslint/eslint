 ...
 */
function calculateWorkerCount(concurrency, fileCount) {
	switch (concurrency) {
		case "auto": {
...
/**
 * Ensures that config arrays are loaded for all of the specified files.
 * @param {string[]} filePaths The file paths to load the config arrays for.
 * @param {ConfigLoader} configLoader The config loader to use.
 * @param {number} concurrency The maximum number of concurrent operations.
 * @param {AbortController} abortController Used to stop linting when an error occurs.
 * @returns {Promise<void>} A promise that resolves when all config arrays are loaded.
 */
async function loadConfigArraysForFiles(
	filePaths,
	configLoader,
	concurrency,
	abortController,
	...
	// Using retrier to load config arrays concurrently.
	const retrier = new Retrier(() => false, { concurrency });

	/**
	 * Loads the config array for a specified file.
	 * @param {string} filePath The file path to load the config array for.
	 * @returns {Promise<void>} A promise that resolves when the config array is loaded.
	 */
	...
	try {
		await Promise.all(
			filePaths.map(filePath =>
				retrier.retry(() => loadConfigArrayForFile(filePath), {
					signal: abortSignal,
				}),
			),
		);
...
async function runWorkers(
	 ...
	 * @param {() => void} resolve Called when the worker thread terminates successfully.
	 * @param {(error: Error) => void} reject Called when the worker thread terminates with an error.
	 * @returns {void}
	 */
	function workerExecutor(resolve, reject) {
...
async function lintFilesWithConcurrency(
	...
	const [results] = await Promise.all([
		runWorkers(filePaths, workerCount, eslintOptionsOrURL, abortController),
...
async function lintFilesWithoutConcurrency(eslint, filePaths) {
	...
	const retrier = createLintFileRetrier({ concurrency: 100 });
