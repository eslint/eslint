...
class ESLint {
    ...
    async getFormatter(format) {
        ...
        if (Number.isSafeInteger(formatter.version)) {
            if (formatter.version >= 2) {
                return async function *adapter2(results, { isTTY = false } = {}) {
                    yield* formatter.format(
                        results,
                        createMetadata(engine, { isTTY })
                    );
                };
            }
            throw new Error("Unknown formatter version");
        }
        ...
        return async function *adapter(resultIterator) {
            ...
            yield formatter(results, createMetadata(engine, {}));
        };
    }
}
