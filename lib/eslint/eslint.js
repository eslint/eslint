...
async function loadFlatConfigFile(filePath, hasUnstableTSConfigFlag) {
    ...
    if (isTS && !isDeno && !isBun) {
        ...
        /**
         * @type {typeof import("jiti").createJiti}
         */
        const createJiti = await import("jiti").then(jitiModule => {

            if (typeof jitiModule?.createJiti === "function") {
                return jitiModule.createJiti;
            }

            process.emitWarning("You are using an outdated version of the 'jiti' library. The top-level await syntax is not supported in this version. Please update to the latest version of 'jiti' to ensure compatibility and access to the latest features.", "ESLintOutdatedJitiWarning");
        ...
        const config = typeof jiti?.import === "function" ? await jiti.import(fileURL.href) : await jiti(fileURL.href);
