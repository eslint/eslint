...
function processOptions({
    allowInlineConfig = true, // ← we cannot use `overrideConfig.noInlineConfig` instaed because `allowInlineConfig` has side-effect that suppress warnings that show inline configs are ignored.
    baseConfig = null,
    cache = false,
    ...
    plugins = {},
    reportUnusedDisableDirectives = null, // ← should be null by default because if it's a string then it overrides the 'reportUnusedDisableDirectives' setting in config files. And we cannot use `overrideConfig.reportUnusedDisableDirectives` instaed because we cannot configure the `error` severity with that.
    ...
    if (typeof plugins !== "object") {
        ...
        errors.push("'plugins' must not include the empty string in keys.");
    }
...
/**
 * Check if a value has one or more properties that that value is not undefined.
 ...
 *
 * - Remove the deprecated `source` property from each result.
...
function processCLIEngineLintReport(cliEngine, { results }) {
    return results.map(({
        ...
        get usedDeprecatedRules() {
            return getOrFindUsedDeprecatedRules(cliEngine, filePath);
        }
    }));
}
