 ...
 * @param {Map<string, any>} packageCache The cache to use.
 * @param {string} basedir The directory to start searching from.
 * @param {string} originalBasedir The original directory to start searching from.
 ...
 * @param {string} longerDir Longer dir path, e.g. `/User/foo/bar/baz`
 * @param {string} shorterDir Shorter dir path, e.g. `/User/foo`
 * @param {Function} cb Callback function to call for each dir.
 * @returns {void}
 */
...
/**
 * Set cached `findNearestPackageData` value based on `basedir`. When one is found,
 * and we've already traversed some directories between `basedir` and `originalBasedir`,
 * we cache the value for those in-between directories as well.
 * @param {Map<string, any>} cache The cache to use.
 * @param {any} basedir The package data to cache.
 * @param {string} originalBasedir The original directory to start searching from.
 ...
 * Set cached `findNearestPackageData` value based on `basedir`. When one is found,
 * @param {Map<string, any>} cache The cache to use.
 * @param {any} data The package data to cache.
 * @param {string} basedir The directory to cache for.
 * @param {string} originalBasedir The original directory to start searching from.
 * @returns {void} The cached value or `null`.
 */
...
/**
 * Find the nearest package data for a given directory.
 * @param {string} basedir The directory to start searching from.
 * @returns {Promise<{ type?: 'module' | 'commonjs' }>} The package data.
 */
async function findNearestPackageData(basedir) {

    /**
     * @type {Map<string, { type?: 'module' | 'commonjs' }>}
     */
    ...
    while (basedir) {
        ...
        if ((await fs.stat(pkgPath).catch(() => {}))?.isFile()) {
 ...
 *  Set cached `findNearestPackageData` value based on `basedir`. When one is found,
 * @param {string} filePath The file path to check.
 * @returns {Promise<boolean>} `true` if the file is an ESM file, `false` if it's a CJS file.
 */
async function isFilePathESM(filePath) {
    if (!isFileTS(filePath)) {
        return false;
    }
    const pathExtension = path.extname(filePath);

    if (pathExtension === ".mts") {
        return true;
    }

    if (pathExtension === ".cts") {
        return false;

    }

    /*
     * if (/\.m[jt]s$/u.test(filePath)) {
     *     return true;
     * }
     * if (/\.c[jt]s$/u.test(filePath)) {
     *     return false;
     * }
     */
     ...
     *     try {
     *         const pkg = await findNearestPackageData(path.dirname(filePath));
     */
