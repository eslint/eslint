...
/**
 * Counts the number of files that aren't ignored. In a cached run with "metadata" strategy, unchanged files run are not counted.
 * @param {ESLint} eslint ESLint instance.
 * @param {string[]} filePaths File paths to lint.
 * @param {number} maxFileCount The maximum number of files to count. 1 or higher.
 * @returns {number} The number of files that have changed or are new since the last cached lint run.
 */
function countFilesToProcess(eslint, filePaths, maxFileCount) {
	...
	for (const filePath of filePaths) {
		...
		if (
			countAllMatched ||
			!lintResultCache.getValidCachedLintResults(filePath, config)
		) {
			processableFileCount++;
			if (processableFileCount >= maxFileCount) {
				break;
			}
		}
	}
...
function calculateWorkerCount(
	...
	switch (concurrency) {
		...
		case "auto": {
			/*
			 * When concurrency is "auto", the number of worker threads is calculated as the number of files
			 * that need to be processed (`processableFileCount`) divided by the number of files assumed to be
			 * best handled by one worker thread (`AUTO_FILES_PER_WORKER`), rounded up to the next integer.
			 * Two adustments are made to this calculation: first, the number of workers is capped at half the
			 ...
			 */
			{
				...
				// No multithreading if there are less then 4 available cores.
				if (maxWorkers < 2) {
					return 0;
				}

				/*
				 * The maximum number of processable files to be counted. For this number of files or more,
				 * the maximum number of workers (`maxWorkers`) will be used.
				 */
				const maxFileCount =
					AUTO_FILES_PER_WORKER * (maxWorkers - 1) + 1;

				/*
				 * The number of files that need to be processed, typically all non-ignored files.
				 * In a cached run with "metadata" strategy, unchanged files are not counted.
				 */
				const processableFileCount = countFilesToProcess(
					eslint,
					filePaths,
					maxFileCount,
				);
				workerCount = Math.ceil(
					processableFileCount / AUTO_FILES_PER_WORKER,
				);
			}
			break;
		}
		default:
			workerCount = Math.min(concurrency, filePaths.length);
			break;
	}
