...
async function loadFlatConfigFile(filePath) {
    ...
    if (isTS && !isDeno && !isBun) {
        ...
        const createJiti = await import("jiti").then(jitiModule => jitiModule?.createJiti ?? jitiModule.default, () => {
        ...
        const config = /** @type {{ default?: FlatConfigArray }} */ (await jiti.import(fileURL.href));
 ...
 * @param {boolean} [hasUnstableTSConfigFlag=false] `true` if the `unstable_ts_config` flag is enabled, `false` if it's not.
 * @returns {Promise<{configFilePath:string|undefined;basePath:string;error:Error|null}>} Location information for
 *      the config file.
 */
async function locateConfigFileToUse({ configFile, cwd }, hasUnstableTSConfigFlag = false) {
...
async function calculateConfigArray(eslint, {
    ...
    const { configFilePath, basePath, error } = await locateConfigFileToUse({ configFile, cwd }, eslint.hasFlag("unstable_ts_config"));
