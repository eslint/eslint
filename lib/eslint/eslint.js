...
/** @typedef {import("../services/suppressions-service").SuppressedViolations} SuppressedViolations */
...
// Used internally. Do not expose.
const suppressionsSnapshot = Symbol(
	"Suppressions loaded from the suppressions file before linting.",
);
 ...
 * @param {SuppressedViolations | null} suppressions The suppressions used to skip fixes for suppressed rules, or `null` if suppressions are not used.
 ...
 * @returns {Promise<LintResult[]>} Lint results.
 */
async function lintFilesWithoutMultithreading(eslint, filePaths, suppressions) {
	const {
		configLoader,
		linter,
		lintResultCache,
...
/**
 * Returns the suppressions to use for a lint run.
 * If a suppressions snapshot was passed to the constructor, that snapshot is used.
 * Otherwise, if the `applySuppressions` option is enabled, the suppressions file is loaded.
 * @param {ESLint} eslint The ESLint instance.
 * @returns {Promise<SuppressedViolations | null>} The suppressions, or `null` if suppressions are not used.
 */
async function loadSuppressions(eslint) {
	const {
		options,
		suppressionsSnapshot: snapshot,
		suppressionsService,
	} = privateMembers.get(eslint);
	...
	if (options.applySuppressions) {
		return suppressionsService.load();
	}
...
class ESLint {
	...
	constructor(options = {}) {
		...
		privateMembers.set(this, {
			...
			warningService,
			suppressionsService,
			suppressionsSnapshot: options[suppressionsSnapshot],
		});
	 ...
	 * @param {Object} [internalOptions] Options that are not part of the public API.
	 * @returns {ESLint} The new ESLint instance.
	 */
	static async fromOptionsModule(optionsURL, internalOptions) {
		...
		const options = {
			...loadedOptions,
			[suppressionsSnapshot]: internalOptions?.[suppressionsSnapshot],
			[disableCloneabilityCheck]: true,
		};
	...
	async lintFiles(patterns) {
		...
		const suppressions = await loadSuppressions(this);
		...
		if (workerCount) {
			...
			results = await lintFilesWithoutMultithreading(
				this,
				filePaths,
				suppressions,
			);
	...
	async lintText(code, options = {}) {
		...
		const suppressions = filePath ? await loadSuppressions(this) : null;
		...
		if (resolvedFilename && configStatus !== "matched") {
			...
			const fixer = getFixerForSuppressions(
				getFixerForFixTypes(fix, fixTypesSet, config),
				suppressions,
				resolvedFilename,
				cwd,
			);
