...
class ESLint {
	...
	/**
	 * Suppress lint violations for the given results and rules.
	 * @param {LintResult[]} results The lint results to apply suppressions to.
	 * @param {Object} options The options for suppression.
	 * @param {string[]} [options.rules] An array of rule IDs to suppress. If not provided, all violations are suppressed.
	 * @param {string} [options.suppressionsLocation] The location of the suppressions file. Defaults to "eslint-suppressions.json".
	 * @returns {Promise<void>} A promise that resolves when suppression is complete.
	 */
	async suppressViolations(results, options = {}) {
		const { rules, suppressionsLocation = "eslint-suppressions.json" } = options;
		const { options: eslintOptions } = privateMembers.get(this);
		const suppressionsFileLocation = path.resolve(eslintOptions.cwd, suppressionsLocation);
		const suppressions = new SuppressionsService({
			filePath: suppressionsFileLocation,
			cwd: eslintOptions.cwd,
		});

		await suppressions.suppress(results, rules);
	}

	/**
	 * Prune unused suppressions from the suppressions file.
	 * @param {LintResult[]} results The lint results to use for pruning.
	 * @param {Object} options The options for pruning.
	 * @param {string} [options.suppressionsLocation] The location of the suppressions file. Defaults to "eslint-suppressions.json".
	 * @returns {Promise<void>} A promise that resolves when pruning is complete.
	 */
	async pruneUnusedSuppressions(results, options = {}) {
		const { suppressionsLocation = "eslint-suppressions.json" } = options;
		const { options: eslintOptions } = privateMembers.get(this);
		const suppressionsFileLocation = path.resolve(eslintOptions.cwd, suppressionsLocation);
		const suppressions = new SuppressionsService({
			filePath: suppressionsFileLocation,
	...
	/**
	 * Apply suppressions to the given results based on the suppressions file.
	 * @param {LintResult[]} results The lint results to apply suppressions to.
	 * @param {Object} options The options for applying suppressions.
	 * @param {string} [options.suppressionsLocation] The location of the suppressions file. Defaults to "eslint-suppressions.json".
	 * @returns {Promise<{results: LintResult[], unused: Object}>} A promise that resolves to an object containing the updated results and unused suppressions.
	 */
	async applySuppressions(results, options = {}) {
		const { suppressionsLocation = "eslint-suppressions.json" } = options;
		const { options: eslintOptions } = privateMembers.get(this);
		const suppressionsFileLocation = path.resolve(eslintOptions.cwd, suppressionsLocation);
		const suppressions = new SuppressionsService({
		...
		const loadedSuppressions = await suppressions.load();
		return suppressions.applySuppressions(results, loadedSuppressions);
	}
}
