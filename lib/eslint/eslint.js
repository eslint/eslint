 ...
 * @import { IndexedLintResult } from "./worker.js";
 */
...
function processLintReport(eslint, results) {
	...
	const descriptor = {
		...
		enumerable: true,
		get() {
			const filePath = path.isAbsolute(this.filePath)
				? this.filePath
				: getPlaceholderPath(cwd);
			const configs = configLoader.getCachedConfigArrayForFile(filePath);

			const usedDeprecatedRules = getOrFindUsedDeprecatedRules(
				filePath,
				configs,
			);
			Object.defineProperty(this, "usedDeprecatedRules", {
				writable: true,
				value: usedDeprecatedRules,
			});
	...
	for (const result of results) {
		if (!Object.hasOwn(result, "usedDeprecatedRules")) {
...
const AUTO_FILES_PER_WORKER = 35; // Exact value to be determined.
 ...
 * @param {ESLintOptions | string} eslintOptionsOrURL The unprocessed ESLint options or the URL of the option module.
 * @param {Map<string, Map<string, RulesMeta>>} rulesMetaCache A cache for rules metadata.
...
async function runWorkers(
	...
	const warningChannelName = warningService.channelName;
	const workerOptions = {
		workerData: {
			eslintOptionsOrURL,
			filePathIndexArray,
	...
	function workerExecutor(resolve_, reject) {
		...
		worker.once(
			"message",
			(/** @type {IndexedLintResult[]} */ indexedResults) => {
				for (const { index, rulesMeta, ...result } of indexedResults) {
					Object.freeze(result.usedDeprecatedRules);
					results[index] = result;
					rulesMetaCache.set(result.filePath, rulesMeta);
				}
				resolve_();
			},
		);
	...
	try {
		warningChannel.onmessage = event => {
			if (event.data.type === "ESLintEmptyConfigWarning") {
				warningService.emitEmptyConfigWarning(
					event.data.configFilePath,
				);
			}
		};
 ...
 * @returns {Promise<LintResult[]>} Lint results.
 */
async function lintFilesWithConcurrency(
	eslint,
	filePaths,
	workerCount,
	eslintOptionsOrURL,
) {
	const { configLoader, lintResultCache, rulesMetaCache, warningService } =
		privateMembers.get(eslint);
...
const lintFileRetryCodes = new Set(["ENFILE", "EMFILE"]);

/**
 * Lint files without concurrency.
 * @param {ESLint} eslint ESLint instance.
 * @param {string[]} filePaths File paths to lint.
 * @returns {Promise<LintResult[]>} Lint results.
 */
async function lintFilesWithoutConcurrency(eslint, filePaths) {
	const {
		configLoader,
		linter,
		lintResultCache,
	...
	const retrier = new Retrier(error => lintFileRetryCodes.has(error.code), {
		concurrency: 100,
	});
...
function validateOptionCloneability(options) {
	...
	const error = new TypeError(
		`The ${uncloneableOptionKeys.length === 1 ? "option" : "options"} ${new Intl.ListFormat("en-US").format(uncloneableOptionKeys.map(key => `"${key}"`))} cannot be cloned. When concurrency is enabled, all options must be cloneable. Remove uncloneable options or use an option module.`,
	);
...
/**
 * Ensures that warnings about are emitted only once per file.
 */
class ConcurrencySafeWarningService extends WarningService {
	/** Empty config files that already have a warning. */
	#emptyConfigFilePaths = new Set();

	/** A unique name for the BroadcastChannel associated with this service. */
	channelName = `ESLint Warning Channel ${randomUUID()}`;

	emitEmptyConfigWarning(configFilePath) {
		if (this.#emptyConfigFilePaths.has(configFilePath)) {
			return;
		}

		this.#emptyConfigFilePaths.add(configFilePath);
		super.emitEmptyConfigWarning(configFilePath);
	}
}
...
class ESLint {
	...
	/**
	 * The unprocessed options or the URL of the option module. Only set when concurrency is enabled.
	...
	constructor(options = {}) {
		...
		const warningService = new ConcurrencySafeWarningService();
	 ...
	 * @param {string | URL} optionsURL The URL of the option module.
	 * @returns {ESLint} The new ESLint instance.
	 */
	static async fromOptionModule(optionsURL) {
		const loadedOptions = await loadOptionsFromModule(optionsURL);
		const options = { ...loadedOptions, [disableCloneabilityCheck]: true };
		const eslint = new ESLint(options);

		if (options.concurrency !== "off") {
			// Save the option module URL in an instance field to pass to worker threads in `lintFiles()`.
			eslint.#optionsOrURL = String(optionsURL);
		}
		return eslint;
	}
	...
	getRulesMetaForResults(results) {
		...
		for (const result of results) {
			...
			const cachedRulesMetaForResult = rulesMetaCache.get(
				result.filePath,
			);
			...
			for (const { ruleId } of allMessages) {
				...
				if (cachedRulesMetaForResult) {
					// Use cached metadata if available.
					if (cachedRulesMetaForResult.has(ruleId)) {
						const ruleMeta = cachedRulesMetaForResult.get(ruleId);

						resultRulesMeta.set(ruleId, ruleMeta);
					}

					// If the rule is not in the cache, ignore it.
					continue;
				}
		...
		return Object.fromEntries(resultRulesMeta.entries());
	}
	...
	async lintFiles(patterns) {
		...
		const { cache, cwd, globInputPaths, errorOnUnmatchedPattern } =
			eslintOptions;
		...
		if (workerCount) {
			...
			results = await lintFilesWithConcurrency(
				this,
				filePaths,
				workerCount,
				this.#optionsOrURL,
			...
			debug(`Linting without concurrency.`);
			results = await lintFilesWithoutConcurrency(this, filePaths);
		}
