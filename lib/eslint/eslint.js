...
class ESLint {
	...
	async loadFormatter(name = "stylish") {
		...
		/*
		 * A formatter name is a file path when it is relative (starts with `.`)
		 * or absolute. A bare slash elsewhere — e.g. `foo/subpath` — indicates
		 * an npm package with a subpath export, not a file. Scoped packages like
		 * `@scope/name` always contain `/` but are correctly routed through the
		 * npm path below because they start with `@`, not `.`.
		 */
		const isFilePath =
			normalizedFormatName.startsWith(".") ||
			path.isAbsolute(normalizedFormatName);
		if (isFilePath) {
