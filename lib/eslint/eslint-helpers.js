...
const debug = require("debug")("eslint:eslint-helpers");
...
function processOptions({
	...
	if (
		...
		errors.push("'concurrency' must be positive integer, 'auto' or 'off'.");
	}
...
/**
 * Loads ESLint constructor options from an option module.
 * @param {string|URL} optionsURL The URL of the option module to load.
 * @returns {ESLintOptions} ESLint constructor options.
 */
...
function createConfigLoader(
	...
	return linter.hasFlag("unstable_config_lookup_from_file")
...
/**
 * Creates a retrier to use with `lintFile`.
 * @param {Object} [options] The options for the retrier.
 * @param {number} [options.concurrency] The maximum number of concurrent tasks.
 * @returns {Retrier} A new retrier instance configured to retry on ENFILE and EMFILE errors.
 */
function createLintFileRetrier(options) {
	return new Retrier(error => lintFileRetryCodes.has(error.code), options);
}
