 ...
 * @param {Array<string>} args.extensions Array of file extensions to use
 *      when a pattern doesn't have any extension. 
...
async function findFiles({
    ...
    const globbyPatterns = patterns

        // filter out glob patterns if not enabled
        .filter(pattern => globInputPaths || !isGlobPattern(pattern))

        // add file extensions to glob patterns ending with *
        .map(pattern => {
            
            // normalize separators to make globby happy
            const normalized = pattern.replace(/\\/gu, "/");
            
            if (!normalized.endsWith("*") || !extensions) {
                return normalized;
            }
    ...
    // TODO: How to handle default ignores?
    globbyPatterns.unshift("!node_modules");

    return globby(globbyPatterns, { cwd, absolute: true });
}
