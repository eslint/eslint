    ...
    stats.forEach((stat, index) => {
        ...
        if (stat) {
            ...
            if (stat.isDirectory()) {
                ...
                /*
                 * If `extensions` is provided, then append them to the end of
                 * the pattern. Otherwise, use the default files from the config
                 * array as the suffix.
                 */
                if (extensions) {
                    globbyPatterns.push(
                        `${posixPattern}/**/*{${extensions.join(",")}}`
                    );
                } else {
                    const filePatterns = configs.files
                        .filter(filePattern => typeof filePattern === "string" && filePattern.startsWith("**"))
                        .map(filePattern => path.posix.join(posixPattern,filePattern));
