...
async function globSearch({
    ...
    // Matches patterns such as "subd*/*.js" but not "**/*.js" or "subdir/*.js"
    const globAtStartPattern = /^[^*]*\*[^*]/u;
    ...
    const walk = hfs.walk(
        basePath,
        {
            async directoryFilter(entry) {
                ...
                /*
                 * In order to account for the special case of a "." pattern, we
                 * need to know if a pattern matches, and if so, which one.
                 */
                let matchesPattern;
                let matchIndex = 0;

                for (const matcher of matchers) {
                    if (matcher.match(entry.path, true)) {
                        matchesPattern = true;
                        break;
                    }
                ...
                // no match, so we can skip this directory
                if (!matchesPattern) {
                    return false;
                }

                const absolutePath = normalizeToPosix(path.resolve(basePath, entry.path));
                 ...
                 * for a config file or just use the default config.
                 *
                 * In general, we assume that the user wants to lint anything
                 * that is passed directly. This is complicated because we start
                 * the search from the base directory and work our way down. So,
                 * we need to know if we are in a directory that was passed directly
                 * or if we are in a directory that was matched by a pattern in
                 ...
                 * explicitly asking to lint the current directory.
                 *
                 ...
                 * this case to act like case 1, where we don't look for a config
                 * file in the parent directory.
                 *
                 * 5. A recursive glob pattern is passed (e.g., "**"). We want this
                 * case to act like case 2.
                 */
                const matchedPattern = matchers[matchIndex].pattern;

                // something like "subd*/*"
                const isLeadingGlob = globAtStartPattern.test(matchedPattern);
                const parent = globParent(patterns[matchIndex]);

                /*
                 * If there's a leading glob and we are at the first level of
                 * traversal, we need to create a path that includes the parent
                 * directory and the current directory. Otherwise, we just use
                 * the parent directory.
                 */
                const pathToMatch = isLeadingGlob && entry.depth === 1
                    ? path.join(parent, entry.path)
                    : parent;

                /*
                 * If the path to match is inside the entry's full path,
                 * then we should load the config for the directory.
                 * Otherwise, we should use the default config.
                 */
                const configs = isPathInside(absolutePath, pathToMatch)
                    ? await configLoader.loadConfigArrayForDirectory(absolutePath)
                    : await configLoader.loadDefaultConfigArray();
