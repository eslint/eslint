 ...
 */
class UnmatchedSearchPatternsErrror extends Error {
...
async function globSearch({
        ...
        entryFilter(entry) {
            ...
            const matchesPattern = unmatchedPatterns.size > 0
                ? matchers.reduce((previousValue, matcher) => {
                    ...
                    // update unmatched patterns for potential errors
                    if (pathMatches) {
    ...
    if (errorOnUnmatchedPattern && unmatchedPatterns.size > 0) {
        throw new UnmatchedSearchPatternsErrror({
            basePath,
...
async function globMultiSearch({ searches, configs, errorOnUnmatchedPattern }) {
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        if (result.status === "fulfilled") {
            ...
            // if the are no results we need to check for ignored files
