    ...
    const walk = hfs.walk(
        basePath,
        {
            async directoryFilter(entry) {
                ...
                /*
                 * If we are in a directory that was passed directly on the command line,
                 * don't look for a config file. If "subdir" is passed, then we don't want
                 * to look for a config file in its parent directory. The same applies if "subdir/*"
                 * "sub*r/*" etc. are passed.
                 *
                 * This special case only applies to immediate children of `basePath`, so we
                 * don't need to worry about any paths that don't include a slash.
                 *
                 * Basically, if the raw pattern isn't "." and the entry path doesn't include
                 * a slash, then we don't need to look for a config file.
                 */
