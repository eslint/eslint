            ...
            async directoryFilter(entry) {
                ...
                for (const matcher of matchers) {
                    if (matcher.match(entry.path, true)) {
                        matchedPatterns.push(matcher.pattern);
                    }
                }
                 ...
                 */
                const shouldLoadConfigFile = matchedPatterns.some(pattern => {

                    /*
                     * If the pattern starts with "**", then we always look for
                     * a config file.
                     */
                     ...
                     * traversal, we don't need to look for a config file.
                     */
                    const hasLeadingGlob = globAtStartPattern.test(pattern);

                    if (hasLeadingGlob && entry.depth === 1) {
                        return true;
                    }

                    /*
                     * Otherwise, we need to check if the path is inside the
                     * parent directory.
                     */
                    const parent = globParent(pattern);

                    return isPathInside(absolutePath, parent);
                });
