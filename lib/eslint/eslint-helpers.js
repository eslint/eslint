...
/** @typedef {import("../services/suppressions-service.js").SuppressedViolations} SuppressedViolations */
...
/**
 * Creates a fixer function that skips fixes for rules that are suppressed for a file.
 * @param {Function|boolean} fix The original fix option or a fixer function.
 * @param {SuppressedViolations|null} suppressions The suppressions, or `null` if suppressions are not used.
 * @param {string} filePath The path of the file to lint.
 * @param {string} cwd The current working directory.
 * @returns {Function|boolean} The fixer function or the original fix value.
 */
function getFixerForSuppressions(fix, suppressions, filePath, cwd) {
	if (!fix || !suppressions) {
		return fix;
	}

	const suppressedRuleIds = SuppressionsService.getSuppressedRuleIds(
		suppressions,
		filePath,
		cwd,
	);
	...
	return message =>
		!suppressedRuleIds.has(message.ruleId) && originalFix(message);
}
...
async function lintFile(
	...
	// set up fixer for fixTypes and suppressions if necessary
