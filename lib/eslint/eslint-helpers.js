...
async function globSearch({
    ...
    const walker = hfs.walk(
        basePath,
        {
            async directoryFilter(entry) {
                const absolutePath = normalizeToPosix(path.resolve(basePath, entry.path));
                const matchesPattern = matchers.some(matcher => matcher.match(entry.path, true));

                return matchesPattern && !await configs.isDirectoryIgnored(absolutePath);
            },
            async entryFilter(entry) {
                ...
                const fileIsIgnored = await configs.isFileIgnored(absolutePath);
                ...
                return matchesPattern && !fileIsIgnored;
            }
        }
    );
...
async function findFiles({
    ...
    await Promise.all(stats.map(async (stat, index) => {
        ...
        if (stat) {
            ...
            if (stat.isFile()) {
                results.push({
                    filePath,
                    ignored: await configs.isFileIgnored(filePath)
                });
            }
