...
function validateRuleOptions(rule, ruleId, options, source = null) {
    try {
        ...
        const ruleOptions = Array.isArray(options) ? options.slice(1) : [];
        const ruleOptionsWithDefaults = rule && rule.meta ? deepMergeArrays(rule.meta.defaultOptions, ruleOptions) : ruleOptions;

        if (severity !== 0) {
            validateRuleSchema(rule, ruleOptionsWithDefaults);
        }

        return [severity, ...ruleOptionsWithDefaults];
    } catch (err) {
...
function validateRules(
    ...
    Object.keys(rulesConfig).forEach(id => {
        ...
        rulesConfig[id] = validateRuleOptions(rule, id, rulesConfig[id], source);
    });
}
