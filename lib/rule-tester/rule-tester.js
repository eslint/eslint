...
/**
 * A test case that is expected to throw (schema validation or rule execution).
 * @typedef {Object} FatalTestCase
 * @property {string} [name] Name for the test case.
 * @property {string} [code] Code for the test case. If omitted, empty string is used.
 * @property {any[]} [options] Options for the test case (invalid options to trigger the fatal error).
 * @property {{ message?: string | RegExp, name?: string }} error Expected error. At least one of message or name is required.
 * @property {Function} [before] Function to execute before testing the case.
 * @property {Function} [after] Function to execute after testing the case regardless of its result.
 * @property {boolean} [only] Run only this test case or the subset of test cases with this property.
 */
...
/*
 * All allowed property names in fatal test case error objects.
 */
const fatalErrorObjectParameters = new Set(["message", "name"]);
...
/**
 * Asserts that the common properties of a valid/invalid/fatal test case have the correct types.
 * @param {Object} item The test case object to check.
 * @param {Object} [options] Options for the assertion.
 * @param {boolean} [options.isCodeOptional=false] If true, 'code' is optional but must be a string when present.
 * @returns {void}
 */
function assertTestCommonProperties(item, options = {}) {
	const { isCodeOptional = false } = options;

	if (isCodeOptional) {
		if (hasOwnProperty(item, "code")) {
			assert.ok(
				typeof item.code === "string",
				"Fatal test case property 'code', if specified, must be a string",
			);
		}
	} else {
		assert.ok(
...
function assertValidTestCase(item, seenTestCases) {
	// must not have properties of invalid or fatal test cases
	assert.ok(
		item.errors === void 0,
	...
	assert.ok(
		item.error === void 0,
		"Valid test case must not have 'error' property",
	);
...
function assertInvalidTestCase(
	...
	// must not have property of fatal test cases
	assert.ok(
		item.error === void 0,
		"Invalid test case must not have 'error' property",
	);
...
/**
 * Asserts that the fatal test case object is valid.
 * @param {Object} item The fatal test case object to check.
 * @param {string} ruleName The name of the rule being tested.
 * @returns {void}
 * @throws {AssertionError} If the test case is not valid.
 */
function assertFatalTestCase(item, ruleName) {
	assert.ok(
		item && typeof item === "object",
		`Fatal test case for rule ${ruleName}: Could not find test case object`,
	);

	assert.ok(
		hasOwnProperty(item, "error") && typeof item.error === "object",
		"Fatal test case must have an 'error' object",
	);
	...
	assert.ok(
		error !== null &&
			(hasOwnProperty(error, "message") || hasOwnProperty(error, "name")),
		"Fatal test case 'error' must specify at least one of 'message' or 'name'",
	);

	for (const propertyName of Object.keys(error)) {
		assert.ok(
			fatalErrorObjectParameters.has(propertyName),
			`Invalid fatal error property name '${propertyName}'. Expected one of ${friendlyFatalErrorObjectParameterList}.`,
		);
	}

	assert.ok(
		item.errors === void 0,
		"Fatal test case must not have 'errors' property",
	);
	assert.ok(
		item.output === void 0,
		"Fatal test case must not have 'output' property",
	);

	assertTestCommonProperties(item, { isCodeOptional: true });
}
	...
	return key => {
		if (testLocations === null) {
			...
			if (existsSync(sourceFile)) {
				...
				const fatalStartIndex = content.findIndex(line =>
					/\bfatal\s*:/u.test(line),
				);
				...
				if (fatalStartIndex >= 0) {
					testLocations.fatal = `${sourceFile}:${
						sourceLine + fatalStartIndex
					}`;
				}

				// Scenario basics — find end of each section (start of next section, or end of content)
				const sectionStarts = [
					validStartIndex,
					invalidStartIndex,
					fatalStartIndex,
				...
				/**
				 * Returns the end index for a section that starts at `startIndex`.
				 * @param {number} startIndex The start index of the section.
				 * @returns {number} The end index.
				 */
				function findSectionEnd(startIndex) {
					const next = sectionStarts.find(i => i > startIndex);

					return next !== void 0 ? next : content.length;
				}

				const validLines = content.slice(
					validStartIndex,
					findSectionEnd(validStartIndex),
				);
				const invalidLines = content.slice(
					invalidStartIndex,
					findSectionEnd(invalidStartIndex),
				);
				const fatalLines =
					fatalStartIndex >= 0
						? content.slice(
								fatalStartIndex,
								findSectionEnd(fatalStartIndex),
							)
						: [];
				...
				const invalidLineIndexes = invalidLines
					...
					.filter(Boolean);
				const fatalLineIndexes = fatalLines
					.map((l, i) =>
						l.trimStart().startsWith("error:") ? i : null,
					)
					.filter(Boolean);
				...
				Object.assign(
					...
					Object.fromEntries(
						fatalLineIndexes.map((location, fatalIndex) => [
							`fatal[${fatalIndex}]`,
							`${sourceFile}:${
								sourceLine + fatalStartIndex + location
							}`,
						]),
					),
				);
...
class RuleTester {
	...
	run(ruleName, rule, test) {
		 ...
		 * @param {Object} [runOptions] Options for this run.
		 * @param {boolean} [runOptions.expectFatal] When true, catch schema/rule exceptions and return them as a fatal message.
		 * @throws {Error} If an invalid schema.
		 * @returns {Object} Eslint run result
		 * @private
		 */
		function runRuleForItem(item, runOptions = {}) {
			const expectFatal = runOptions.expectFatal === true;
			const code = expectFatal ? (item.code ?? "") : item.code;
			...
			try {
				...
				if (
					expectFatal &&
					typeof error.messageForTest !== "undefined"
				) {
					const name =
						error.name === "Error"
			...
			try {
				...
				if (expectFatal && typeof err.messageForTest !== "undefined") {
					const name =
						err.name === "Error"
		...
		/**
		 * Check if the fatal template is invalid or not.
		 * All fatal cases go through this.
		 * @param {Object} item Item to run the rule against (fatal test case).
		 * @returns {void}
		 * @private
		 */
		function testFatalTemplate(item) {
			const result = runRuleForItem(item, { expectFatal: true });
			...
			assert.strictEqual(
				...
				1,
				util.format(
					"Should have exactly one fatal error but had %d: %s",
					fatalMessages.length,
					util.inspect(result.messages),
				),
			);

			const message = fatalMessages[0];

			if (hasOwnProperty(item.error, "message")) {
				assertMessageMatches(message.message, item.error.message);
			}
			if (hasOwnProperty(item.error, "name")) {
				assert.strictEqual(
					message.name,
					item.error.name,
					`Expected error name to be '${item.error.name}' but got '${message.name}'`,
				);
			}
		}
		...
		this.constructor.describe(ruleName, () => {
			...
			if (test.fatal && test.fatal.length > 0) {
				this.constructor.describe("fatal", () => {
					test.fatal.forEach((fatal, index) => {
						const item =
							typeof fatal === "object" && fatal !== null
								? fatal
								: { code: fatal, error: {} };
						this.constructor[item.only ? "itOnly" : "it"](
							sanitize(
							...
							() => {
								try {
									runHook(item, "before");
									assertFatalTestCase(item, ruleName);
									testFatalTemplate(item);
								} catch (error) {
									if (error instanceof Error) {
										error.scenarioType = "fatal";
										error.scenarioIndex = index;
										error.stack = error.stack.replace(
											/^ +at /mu,
											[
												`    roughly at RuleTester.run.fatal[${index}] (${estimateTestLocation(`fatal[${index}]`)})`,
												`    roughly at RuleTester.run.fatal (${estimateTestLocation("fatal")})`,
												`    at RuleTester.run (${estimateTestLocation("root")})`,
												"    at ",
											].join("\n"),
										);
