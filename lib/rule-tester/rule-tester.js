...
/*
 * Default assertion options shared by all test cases. Can be overwritten via
 * setDefaultAssertionOptions().
 */
let sharedDefaultAssertionOptions = {};
...
class RuleTester {
	...
	/**
	 * Set the default assertion options to use for all future tests.
	 * Individual test cases can override these by specifying their own assertionOptions.
	 * @param {Object} options The assertion options to use as default.
	 * @throws {TypeError} If non-object options.
	 * @returns {void}
	 */
	static setDefaultAssertionOptions(options) {
		if (typeof options !== "object" || options === null) {
			throw new TypeError(
				"RuleTester.setDefaultAssertionOptions: options must be an object",
			);
		}
		sharedDefaultAssertionOptions = options;
	}

	/**
	 * Get the current default assertion options used for all tests.
	 * @returns {Object} the current default assertion options
	 */
	static getDefaultAssertionOptions() {
		return sharedDefaultAssertionOptions;
	}

	/**
	 * Reset the default assertion options to the initial empty state.
	 * @returns {void}
	 */
	static resetDefaultAssertionOptions() {
		sharedDefaultAssertionOptions = {};
	}
	...
	run(ruleName, rule, test) {
		...
		function testInvalidTemplate(item) {
			const {
				...
				requireData = false,
			} = test.assertionOptions ?? sharedDefaultAssertionOptions;
