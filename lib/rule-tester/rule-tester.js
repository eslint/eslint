...
function assertTestCommonProperties(item) {
	...
	// optional properties
	assert.ok(
		item.name === void 0 || typeof item.name === "string",
		`Optional test case property 'name' must be a string`,
	);
	assert.ok(
		item.only === void 0 || typeof item.only === "boolean",
		`Optional test case property 'only' must be a boolean`,
	);
	assert.ok(
		item.before === void 0 || typeof item.before === "function",
		`Optional test case property 'before' must be a function`,
	);
	assert.ok(
		item.after === void 0 || typeof item.after === "function",
		`Optional test case property 'after' must be a function`,
	);
	assert.ok(
		item.filename === void 0 || typeof item.filename === "string",
 ...
 * A valid test case must specify a string value for 'code' and must not have 'errors' or 'output' properties.
 ...
 * A valid test case must specify a string value for 'code' and must have 'errors' property.
 * Optional properties are checked for correct types.
...
class RuleTester {
	...
	run(ruleName, rule, test) {
		...
		function runRuleForItem(item) {
			...
			const { code, filename, options = [] } = item;
