                ...
                for (let i = 0, l = item.errors.length; i < l; i++) {
                    ...
                    if (typeof error === "string" || error instanceof RegExp) {
                        ...
                        if (
                            ...
                            assert.ok(false, "Test error object must contain exactly one of 'messageId' and 'message'.");
                        }
                        ...
                        if (hasOwnProperty(error, "suggestions")) {
                            ...
                            if (!error.suggestions || (Array.isArray(error.suggestions) && error.suggestions.length === 0)) {
                                ...
                                error.suggestions.forEach((expectedSuggestion, index) => {
                                    ...
                                    if (
                                        (!hasOwnProperty(expectedSuggestion, "messageId") && !hasOwnProperty(expectedSuggestion, "desc")) ||
                                        (hasOwnProperty(expectedSuggestion, "messageId") && hasOwnProperty(expectedSuggestion, "desc"))
                                    ) {
                                        assert.ok(false, "Test suggestion object must contain exactly one of 'messageId' and 'desc'.");
                                    }
                                    ...
                                    if (hasOwnProperty(expectedSuggestion, "output")) {
                                        ...
                                        assert.ok(false, "Test suggestion object is missing 'output`.");
                                    }
                                });
                            }
                        } else {
                            if (Array.isArray(message.suggestions) && message.suggestions.length > 0) {
                                assert.ok(false, "The rule produced suggestion(s). Please add a 'suggestions' property.");
                            }
                        }
                    } else {
