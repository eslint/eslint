...
function assertFatalTestCase(item, ruleName) {
	...
	if (hasOwnProperty(item, "options")) {
		assert.ok(
			Array.isArray(item.options),
			"Fatal test case property 'options' must be an array",
		);
	}
	...
	if (item.name) {
		assert.ok(
			typeof item.name === "string",
			"Optional fatal test case property 'name' must be a string",
		);
	}
	if (hasOwnProperty(item, "only")) {
		assert.ok(
			typeof item.only === "boolean",
			"Optional fatal test case property 'only' must be a boolean",
		);
	}
}
	...
	run(ruleName, rule, test) {
		...
		function runRuleForItem(item, runOptions = {}) {
			...
			try {
				...
				if (expectFatal && error.messageForTest !== undefined) {
					const name =
						error.name === "Error"
			...
			try {
				...
				if (expectFatal && err.messageForTest !== undefined) {
					const name =
						err.name === "Error"
