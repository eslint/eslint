...
class RuleTester {
	...
	run(ruleName, rule, test) {
		...
		function testInvalidTemplate(item) {
			...
			assert.ok(
				ruleHasMetaMessages ||
					(requireMessage !== "messageId" && !requireData),
				`Assertion options can not use '${requireData ? "requireData" : 'requireMessage: "messageId"'}' if rule under test doesn't define 'meta.messages'.`,
			);
