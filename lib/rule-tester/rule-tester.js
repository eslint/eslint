...
function assertErrorsProperty(errors, ruleName, assertionOptions = {}) {
	...
	if (isArray) {
		...
		const anyStringOrRegExp = errors.some(
			error => typeof error === "string" || error instanceof RegExp,
		);
		assert.ok(
			!anyStringOrRegExp ||
		...
		if (requireMessage) {
			const messageCount = errors.filter(
				error =>
					typeof error === "string" ||
			...
			const messageIdCount = errors.filter(
				error =>
					typeof error === "object" &&
			...
			if (requireMessage === "messageId") {
				...
				assert.strictEqual(
					messageCount,
					0,
					"Errors must not have a 'message' property",
				);
			} else if (requireMessage === "message") {
				assert.strictEqual(
					messageCount,
					totalErrors,
				...
				assert.strictEqual(
					messageIdCount,
					0,
					"Errors must not have a 'messageId' property",
				);
			} else {
				assert.strictEqual(
					messageCount + messageIdCount,
					totalErrors,
		...
		assert.ok(
			...
			`Invalid cases must have 'errors' value as an array${JSON.stringify(errors)}`,
		);
		assert.ok(
			errors > 0,
