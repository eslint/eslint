...
/**
 * Safely require a module, returning a fallback value if the module cannot be found.
 * @param {string} module The module to require.
 * @param {any} fallback The fallback value to return if the module cannot be found.
 * @returns {any} The required module or the fallback value.
 */
function tryRequire(module, fallback = {}) {
	try {
...
const { existsSync, readFileSync } = tryRequire("node:fs", {
	existsSync: () => false,
	readFileSync: () => "",
});
 ...
 * @returns {[string, number, number]} The invocation location.
 */
function getInvocationLocation(relative = getInvocationLocation) {
	// eslint-disable-next-line no-undefined -- Check whether this is a NodeJS env
	if (Error.prepareStackTrace !== undefined) {
		...
		return [
			sourceLocation.getFileName() || "unknown",
			sourceLocation.getLineNumber(),
			sourceLocation.getColumnNumber(),
		];
	}

	// Fallback for environments without `Error.prepareStackTrace`
	const dummyObject = {};
	Error.captureStackTrace(dummyObject, relative);
	const { stack } = dummyObject;

	const sourceLocation = stack
		.split("\n")[1]
		.replace(/.*\(/u, "")
		.replace(/\)$/u, "");

	const [sourceFile, sourceLine = "1", sourceColumn = "1"] = sourceLocation
		.replace(/:\\/u, "\\\\") // Windows workaround for C:\\
		.split(":");

	return [
		sourceFile.replace(/\\\\/u, ":\\") || "unknown",
		Number(sourceLine),
		Number(sourceColumn),
	];
}
...
class RuleTester {
	...
	run(ruleName, rule, test) {
		...
		const [sourceFile, sourceLine, sourceColumn] = getInvocationLocation(
			this.run,
		);
		 ...
		 * @param {string} key The key of the test case type (valid/invalid).
		 * @returns {string} The estimated location of the test case.
		 */
		function estimateTestLocation(key) {
			if (testLocations === null) {
				testLocations = { root: `${sourceFile}:${sourceLine}` };
				...
				if (existsSync(sourceFile)) {
					...
					const validLineIndexes = validLines
						.map((l, i) => (l.trimEnd().endsWith(",") ? i : null))
						.filter(Boolean);
					...
					for (let i = 0; i < invalidLineIndexes.length - 1; i++) {
						...
						const errorLines = invalidLines
							.slice(start, end)
							.map(s => s.trim());
						const errorLineIndexes = [0];
						for (let j = 1; j < errorLines.length; j++) {
							/*
							 * Search for the end of the previous error and the start of the next error
							 * This only detects error blocks and not simple error messages as those aren't unique enough to be detectable.
							 */
							if (
								errorLines[j - 1].endsWith("},") &&
								errorLines[j].startsWith("{")
							) {
								errorLineIndexes.push(j);
							}
						}
			...
			return testLocations[key] || "unknown";
		}
								...
								try {
									...
									if (error instanceof Error) {
										...
										error.stack = error.stack.replace(
											/ +at /u,
											[
												`    roughly at RuleTester.run.valid[${index}] (${estimateTestLocation(
													`valid[${index}]`,
												)})`,
												`    roughly at RuleTester.run.valid (${estimateTestLocation(
													"valid",
												)})`,
												`    roughly at RuleTester.run (${estimateTestLocation(
													"root",
												)})`,
												"    at ",
											].join("\n"),
									...
									if (error instanceof Error) {
										...
										error.stack = error.stack.replace(
											...
											[
												...
												"number"
													? [
															`    roughly at RuleTester.run.invalid[${index}].error[${errorIndex}] (${estimateTestLocation(
																`invalid[${index}].errors[${errorIndex}]`,
															)})`,
														]
													: []),
												`    roughly at RuleTester.run.invalid[${index}] (${estimateTestLocation(
													`invalid[${index}]`,
												)})`,
												`    roughly at RuleTester.run.invalid (${estimateTestLocation(
													"invalid",
												)})`,
												`    roughly at RuleTester.run (${estimateTestLocation(
													"root",
