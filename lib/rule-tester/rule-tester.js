 ...
 * @property {string} [physicalFilename] The physical filename for the test case. If specified, `context.physicalFilename` returns this value; otherwise it falls back to `filename`.
...
function assertTestCommonProperties(item) {
	...
	if (hasOwnProperty(item, "physicalFilename")) {
		assert.ok(
			typeof item.physicalFilename === "string",
			"Optional test case property 'physicalFilename' must be a string",
		);
	}
	...
	run(ruleName, rule, test) {
		...
		function runRuleForItem(item) {
			...
			const filename = hasOwnProperty(item, "filename")
				? item.filename
				: void 0;
			const physicalFilename = hasOwnProperty(item, "physicalFilename")
			...
			try {
				...
				const verifyOptions = filename
					? { filename, physicalFilename }
					: void 0;

				messages = linter.verify(code, configs, verifyOptions);
			} finally {
			...
			if (messages.some(m => m.fix)) {
				...
				const errorMessageInFix = linter
					.verify(output, configs, verifyOptions)
					.find(m => m.fatal);
