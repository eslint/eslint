...
class RuleTester {
	...
	run(ruleName, rule, test) {
		 ...
		 * @param {boolean} [runOptions.expectFatal] When true, catch schema/rule exceptions and return `{ error }` with the caught error.
		...
		function testFatalTemplate(item) {
			...
			assert(
				result.error,
				"Expected an error to be thrown but the rule executed successfully",
			);
			...
			if (hasOwnProperty(item.error, "message")) {
				assertMessageMatches(error.messageForTest, item.error.message);
			}
			if (hasOwnProperty(item.error, "name")) {
				assert.strictEqual(
					name,
					item.error.name,
					`Expected error name to be '${item.error.name}' but got '${name}'`,
				);
			}
		}
