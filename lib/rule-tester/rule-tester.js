...
const
    ...
    SnapshotBuilder = require("./snapshot-builder"),
...
class RuleTester {
    ...
    /**
     * Generate a snapshot test for the given code and rule
     * @param {Function} rule The rule to test.
     * @param {Object} testCase The collection of tests to run.
     * @returns {void}
     */
    generateSnapshot(rule, testCase) {
        const testerConfig = this.testerConfig;
        const linter = this.linter;
        const ruleName = "snapshot";
        ...
        function runRuleForItem(item) {
            let config = lodash.cloneDeep(testerConfig),
                filename, output, beforeAST, afterAST;
        ...
        const result = runRuleForItem(testCase);
        const messages = result.messages;
        const snapshotBuilder = new SnapshotBuilder(testCase.code);
        ...
        messages.forEach(m => {
            ...
            snapshotBuilder.annotateError(position, message);
        });
