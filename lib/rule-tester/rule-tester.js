...
/**
 * A test case that is expected to trigger a fatal error.
 ...
 * @property {boolean} [only] Run only this test case or the subset of test cases with this property.
 * @property {{ message?: string | RegExp, name?: string }} error The expected error.
 */
...
/**
 * Extracts the sanitized error details for fatal rule tester cases.
 * @param {Error} error The caught error.
 * @returns {{message: string, name: string}|null} The normalized error data.
 */
function getFatalErrorDetails(error) {
	if (!error || typeof error !== "object") {
		return null;
	}

	if (!hasOwnProperty(error, "messageForTest")) {
		return null;
	}

	const messageValue = error.messageForTest;
...
function assertTest(test, ruleName) {
	...
	const hasFatal =
		!hasOwnProperty(test, "fatal") || Array.isArray(test.fatal);
	...
	assert.ok(
		hasFatal,
		`Test Scenarios for rule ${ruleName} is invalid: 'fatal' must be an array`,
	);
}
 ...
 * @returns {void}
 */
function assertTestCommonProperties(item, { requireCode = true } = {}) {
	if (requireCode || hasOwnProperty(item, "code")) {
		assert.ok(
...
/**
 * Asserts that a fatal test case object is valid.
 * A fatal test case must specify an `error` object with `message` and/or `name`.
...
function assertFatalTestCase(item, seenTestCases) {
	assertTestCommonProperties(item, { requireCode: false });
	assert.ok(
		!hasOwnProperty(item, "errors"),
		"Fatal test case must not have 'errors' property",
	);
	assert.ok(
		!hasOwnProperty(item, "output"),
		"Fatal test case must not have 'output' property",
	);
	assert.ok(
		hasOwnProperty(item, "error"),
		"Fatal test case must specify an 'error' property",
	);
	assert.ok(
		item.error && typeof item.error === "object",
		"Fatal test case property 'error' must be an object",
	);

	for (const propertyName of Object.keys(item.error)) {
		assert.ok(
	...
	const hasMessage = hasOwnProperty(item.error, "message");
	const hasName = hasOwnProperty(item.error, "name");

	assert.ok(
		hasMessage || hasName,
		"Fatal test case 'error' must specify 'message' and/or 'name'.",
	);

	if (hasMessage) {
		assert.ok(
			typeof item.error.message === "string" ||
			item.error.message instanceof RegExp,
			"Fatal test case error 'message' must be a string or RegExp.",
		);
	}

	if (hasName) {
		assert.strictEqual(
			typeof item.error.name,
			"string",
			"Fatal test case error 'name' must be a string.",
		);
	}
...
class RuleTester {
	...
	run(ruleName, rule, test) {
		...
		assertTest(test, ruleName);
			const fatalTests = Array.isArray(test.fatal) ? test.fatal : [];
		 ...
		 * @private
		 */
		function runRuleForItem(item, type) {
			const isFatalTest = type === "fatal";
			const code =
				typeof item.code === "string"
					? item.code
					: "";
			...
			const maybeCreateFatalResult = error => {
				if (!isFatalTest) {
					return null;
				}

				return getFatalErrorDetails(error);
			};
			...
			try {
				...
				const fatalDetails = maybeCreateFatalResult(error);

				if (fatalDetails) {
					return { fatalError: fatalDetails };
				}
		...
		function testFatalTemplate(item) {
			const result = runRuleForItem(item, "fatal");
			const fatalError = result.fatalError;

			assert.ok(
				fatalError,
				"Fatal test case did not produce an exception.",
			);
			...
			if (hasOwnProperty(item.error, "name")) {
				assert.strictEqual(
					fatalError.name,
					item.error.name,
					`Fatal error name '${fatalError.name}' does not match expected name '${item.error.name}'.`,
				);
			}
		}
		...
		this.constructor.describe(ruleName, () => {
			...
			if (fatalTests.length > 0) {
				this.constructor.describe("fatal", () => {
					const seenTestCases = new Set();
					fatalTests.forEach((fatal, index) => {
						const item = normalizeTestCase(fatal);
						const fallbackName = `(Test Case #${index + 1})`;
						const titleSource =
							item.name || item.code || fallbackName;
						const sanitizedTitle = sanitize(titleSource);
						const testTitle = sanitizedTitle || fallbackName;
