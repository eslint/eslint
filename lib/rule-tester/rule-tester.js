        ...
        function testInvalidTemplate(item) {
                    ...
                    if (typeof error === "string" || error instanceof RegExp) {
                        ...
                        if (hasOwnProperty(error, "suggestions")) {
                            ...
                            if (!message.suggestions) {
                                assert.ok(!error.suggestions || (Array.isArray(error.suggestions) && error.suggestions.length === 0), `Error should have no suggestions on error with message: "${message.message}"`);
                            } else if (!error.suggestions || Array.isArray(error.suggestions) && error.suggestions.length === 0) {
                                assert.ok(message.suggestions.length === 0, `Error should have suggestions on error with message: "${message.message}"`);
                            } else {
                                if (typeof error.suggestions === "number") {
                                    ...
                                    assert.fail(`Error should have an array of suggestions or a number for the amount of suggestions. Instead received "${util.inspect(message.suggestions)}" on error with message: "${message.message}"`);
                                }
                            }
                        }
                    } else {
            ...
            if (hasOwnProperty(item, "output")) {
                if (item.output === null) {
                    ...
                    assert.notStrictEqual(item.code, item.output, "Either omit the 'output' property or set it to null if there is no autofix.");
                }
            } else {
                assert.strictEqual(
                    result.output,
