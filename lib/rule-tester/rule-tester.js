	...
	if (isArray) {
		...
		for (const [number, error] of errors.entries()) {
			if (typeof error === "string" || error instanceof RegExp) {
				assert.ok(
					(!requireMessage || requireMessage !== "messageId") &&
						!requireLocation,
					`Error[${number}] should use an object when 'requireMessage' is 'messageId' or 'requireLocation' is true.`,
				);
				...
				if (requireMessage) {
					if (requireMessage === "message") {
						assert.ok(
							...
							`Error[${number}] should use 'message' (and not 'messageId') when 'requireMessage' is 'message'.`,
						);
					} else if (requireMessage === "messageId") {
						assert.ok(
							!hasOwnProperty(error, "message") &&
								hasOwnProperty(error, "messageId"),
							`Error[${number}] should use 'messageId' (and not 'message') when 'requireMessage' is 'messageId'.`,
						);
					} else {
						assert.ok(
							hasOwnProperty(error, "message") !==
								hasOwnProperty(error, "messageId"),
							`Error[${number}] should have either 'message' or 'messageId' property.`,
						);
					}
				}
				...
				if (hasOwnProperty(error, "message")) {
					assert.ok(
						!hasOwnProperty(error, "messageId"),
						`Error[${number}] should not specify both 'message' and a 'messageId'.`,
					);
					assert.ok(
						!hasOwnProperty(error, "data"),
						`Error[${number}] should not specify both 'data' and 'message'.`,
					);
				}
			} else {
				assert.fail(
					`Error[${number}] must be a string, RegExp, or an object.`,
				);
			}
		}
	} else {
