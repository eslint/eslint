...
exports.validateTests = function validateTests(ruleName, rule, test) {
	assert.ok(typeof ruleName === "string", "Rule name must be a string");
	...
	const hasValid = test.valid && Array.isArray(test.valid);
	const hasInvalid = test.invalid && Array.isArray(test.invalid);
