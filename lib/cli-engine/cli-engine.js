...
class CLIEngine {
    ...
    executeOnFiles(patterns) {
        ...
        const { results, lastConfigArraysFromResults } = this._verifyFiles(files);
    ...
    _verifyFiles(files) {
        ...
        const verifyFilesResults = files.map(({ config, filePath, ignored }) => {
    ...
    async verifyChunkOnWorker({ chunkFiles, chunkIndex }) {
        const {
            fileEnumerator,
            lintResultCache,
            linter,
        ...
        debugPerf(`Worker [${chunkIndex}] started: ${Date.now()}`);


        const startTime = Date.now();


        const verifyFilesResults = await bluebird.map(chunkFiles, async({ filePath, ignored, originalIndex }) => {
            ...
            if (elapsed >= 100) {
                debugPerf(`Config loaded in : ${elapsed}ms for file ${filePath}`);
            }
            const ignoredOrCachedResult = this._getIgnoredOrCachedResult({ config, filePath, ignored });

            if (ignoredOrCachedResult) {
                return { ...ignoredOrCachedResult, originalIndex };
            }
            ...
            return { result, config, originalIndex };
        }, {
            concurrency: parallelFiles
        });
