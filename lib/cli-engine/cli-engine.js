 ...
 * @returns {{cachedResults:LintResult[], targets:{config:ConfigArray,filePath:string}[]}} The results for all files that were linted.
 * @private
 */
function findTargetFiles(engine, patterns) {
    const {
        cacheFilePath,
        fileEnumerator,
        lastConfigArrays,
    ...
    for (const { config, filePath, ignored } of fileEnumerator.iterateFiles(patterns)) {
        if (ignored) {
            cachedResults.push(createIgnoreResult(filePath, cwd));
            continue;
        }
