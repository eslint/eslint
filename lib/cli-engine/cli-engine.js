...
class CLIEngine {
    ...
    async _verifyFilesAsync(files) {
         ...
         */
        const filesWithoutConfig = files.map(({ filePath, ignored }) => ({ filePath, ignored }));

        const chunkSize = Math.ceil(filesWithoutConfig.length / workersCount);
        const chunks = lodash.chunk(filesWithoutConfig, chunkSize);
