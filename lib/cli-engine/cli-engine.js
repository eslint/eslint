 ...
 * @param {CLIEngine} engine The CLIEngine instance that verifies the text.
 * @param {ConfigArray|undefined} config The config of verification.
 * @param {string} filePath The path to the file of `text`.
 * @param {string} text The source code.
 * @returns {LintResult} The result of linting.
 * @private
 */
function verifyText(engine, config, filePath, text) {
    const {
        configArrayFactory,
        fileEnumerator: { extensionRegExp },
        linter,
        options: {
            allowInlineConfig,
            cwd,
    ...
    const { fixed, messages, output } = linter.verifyAndFix(
        text,
        config || configArrayFactory.getConfigArrayForFile(filePathToVerify),
        {
            allowInlineConfig,
...
class CLIEngine {
     ...
     * @param {function({config:ConfigArray,filePath:string}[],function(LintResult[]):LintReport):T} verifyFiles The core logic to lint files.
     * @returns {T} The results for all files that were linted.
     * @private
     */
    _executeOnFiles(patterns, verifyFiles) {
        const {
            cacheFilePath,
            configArrayFactory,
            fileEnumerator,
        ...
        for (const { config, filePath, ignored } of fileEnumerator.iterateFiles(patterns)) {
            if (ignored) {
                resultMap.set(filePath, createIgnoreResult(filePath, cwd));
                continue;
            }
            ...
            if (lintResultCache) {
                ...
                if (cachedResult) {
                    ...
                    if (hadMessages && fix) {
                        ...
                        resultMap.set(filePath, cachedResult);
                        continue;
                    }
                }
            }
            ...
            // Enqueue.
            resultMap.set(filePath, null);
            lintTargets.push({ config, filePath });
        }

        // Delegate the core logic to `verifyFiles`.
        return verifyFiles(lintTargets, lintResults => {
            for (const result of lintResults) {
                const { filePath } = result;

                resultMap.set(filePath, result);
    ...
    executeOnFiles(patterns) {
        return this._executeOnFiles(patterns, (targets, postprocess) => {
            const results = [];
     ...
     */
    executeOnFilesInParallel(patterns) {
        return this._executeOnFiles(patterns, async(targets, postprocess) => {
            const { lintInParallel } = require("./parallel/master");
            const { options } = internalSlotsMap.get(this);
            const files = targets.map(t => t.filePath);
            const results = await lintInParallel(options, files);
    ...
    executeOnText(text, filename, warnIgnored) {
        ...
        if (resolvedFilename && ignoredPaths.contains(resolvedFilename)) {
            ...
            // Do lint.
            results.push(verifyText(this, config, resolvedFilename, text));
        }
