...
const debugPerf = require("debug")("eslint:cli-engine:perf");
...
class CLIEngine {
    ...
    executeOnFiles(patterns) {
        debugPerf("executeOnFiles started");
        const {
            cacheFilePath,
            fileEnumerator,
            lastConfigArrays,
        ...
        lastConfigArrays.length = 0; // TODO: is has all pushed
        ...
        const iterateFilesStartTime = Date.now();
        const files = [...fileEnumerator.iterateFiles(patterns)];

        debugPerf(`iterateFiles complete in: ${Date.now() - iterateFilesStartTime}ms`);

        if (parallel) {
            return this._verifyFilesAsync(files)
                .then(({ results, lastConfigArraysFromResults }) => this._afterVerifyFiles({ results, lastConfigArraysFromResults }, startTime));
        }

        // Iterate source code files.
        const { results, lastConfigArraysFromResults } = this._verifyFilesSync(files);
        ...
        return this._afterVerifyFiles({ results, lastConfigArraysFromResults }, startTime);

    }

    _afterVerifyFiles({ results, lastConfigArraysFromResults }, startTime) {
        const {
            lastConfigArrays,
            lintResultCache
        } = internalSlotsMap.get(this);

        // override lastConfigArrays but keep the same reference
        lastConfigArrays.splice(0, lastConfigArrays.length, ...lastConfigArraysFromResults);
    ...
    _getIgnoredOrCachedResult({ config, filePath, ignored }) {
        const {
            lintResultCache,
            lastConfigArrays,
            options: {
        ...
        if (ignored) {
            return { result: createIgnoreResult(filePath, cwd) };
        }
        ...
        if (lintResultCache) {
            ...
            if (cachedResult) {
                ...
                if (hadMessages && fix) {
                    ...
                    return { result: cachedResult, config };
                }
            }
        }
        return null;
    ...
    /**
     * @typedef {Object} VerifyFileResult
     * @property {LintResult} result the result of linting.
     * @property {ConfigArray} config The path to the current working directory.
     */

    /**
     * verifies a single file with config.
     * @param {FileAndConfig} file file with config array.
     * @returns {VerifyFileResult} result.
     */
    _verifyFile({ config, filePath, ignored }) {
        const {
            fileEnumerator,
            lintResultCache,
            linter,
        ...
        const ignoredOrCachedResult = this._getIgnoredOrCachedResult({ config, filePath, ignored });

        if (ignoredOrCachedResult) {
            return ignoredOrCachedResult;
        }
    ...
    /**
     * @typedef {Object} VerifyFilesResult
     * @property {ConfigArray[]} lastConfigArraysFromResults config arrays for files.
     * @property {LintResult[]} results results of linting files.
     */

    /**
     * verifies files synchronously in the main process.
     * @param {FileAndConfig[]} files files to verify.
     * @returns {VerifyFilesResult} result.
     */
    _verifyFilesSync(files) {
        const verifyFilesResults = files.map(file => this._verifyFile(file));

        const results = verifyFilesResults.map(({ result }) => result);
        const lastConfigArraysFromResults = verifyFilesResults.map(({ config }) => config).filter(item => item);

        return { results, lastConfigArraysFromResults };
    }

    /**
     * verifies files asynchronously on parallel child processes.
     * @param {FileAndConfig[]} files files to verify.
     * @returns {VerifyFilesResult} result.
     */
    async _verifyFilesAsync(files) {
        let pool;
        ...
        try {
            ...
            const workersCount = numParallel;
            ...
            /*
             * send files without config to workers to avoid serializing config
             * config will be loaded on the workers
             */
            const filesToSerialize = files
                .map(({ filePath, ignored }, originalIndex) => ({ filePath, ignored, originalIndex }))
                .sort(() => Math.random() - 0.5);

            const chunkSize = Math.ceil(filesToSerialize.length / workersCount);
            const chunks = lodash.chunk(filesToSerialize, chunkSize);
            const startTime = Date.now();

            debugPerf(`Number of files: ${filesToSerialize.length}`);
            debugPerf(`Workers starting: ${Date.now()}`);

            const chunkResults = await Promise.all(chunks.map(async(chunkFiles, chunkIndex) =>
                await pool.exec("verifyChunkOnWorker", [{
                    chunk: { chunkFiles, chunkIndex },
                    providedOptions: this.providedOptions
                }])));

            debugPerf(`Workers done in: ${Date.now() - startTime}ms`);

            const verifyFilesResults = [];

            chunkResults.forEach(chunkResult => {
                verifyFilesResults.push(...chunkResult);
            });

            verifyFilesResults.sort((r1, r2) => r1.originalIndex - r2.originalIndex);
    ...
    /**
     * @typedef {VerifyFileResult} VerifyChunkResult
     * @property {number} originalIndex the original index of the file before shuffling.
     */

    /**
     * @typedef {VerifyFileResult} VerifyFileChunk
     * @property {FileAndConfig[]} chunkFiles files in the chunk
     * @property {number} chunkIndex index of the chunk for logging & debugging
     */

    /**
     * verifies a chunk of the files, this is called on a separate worker process
     * @param {VerifyFileChunk} chunk chunk to verify
     * @returns {VerifyChunkResult[]} result
     */
    verifyChunkOnWorker({ chunkFiles, chunkIndex }) {
        const { configArrayFactory } = internalSlotsMap.get(this);

        debugPerf(`Worker [${chunkIndex}] started: ${Date.now()} with ${chunkFiles.length} files`);

        const startTime = Date.now();

        const verifyFilesResults = chunkFiles.map(({ filePath, ignored, originalIndex }) => {
            const config = configArrayFactory.getConfigArrayForFile(filePath);

            return {
                ...this._verifyFile({ config, filePath, ignored }),
                originalIndex
            };
        });

        debugPerf(`Worker [${chunkIndex}] complete in: ${Date.now() - startTime}ms`);

        return verifyFilesResults;
    }
