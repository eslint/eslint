...
class CLIEngine {
    ...
    async lintChunkInWorker({ chunkFiles }) {
        const {
            fileEnumerator,
            lintResultCache,
            linter,
        ...
        const verifyFilesResults = await bluebird.map(chunkFiles, async({ config: serializedConfig, filePath, ignored }) => {
            const config = new ConfigArray(...serializedConfig);
