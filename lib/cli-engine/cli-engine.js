...
class CLIEngine {
    ...
    executeOnFiles(patterns) {
        ...
        const verifyFilesResult = this._verifyFiles(files);
        ...
        return this._afterVerifyFiles(verifyFilesResult, startTime);
    }

    _afterVerifyFiles(verifyFilesResult, startTime) {
        const {
            lastConfigArrays,
            lintResultCache
        } = internalSlotsMap.get(this);

        const results = verifyFilesResult.map(({ result }) => result);
        const lastConfigArraysFromResults = verifyFilesResult.map(({ config }) => config).filter(item => item);
    ...
    _verifyFiles(files) {
        ...
        return files.map(({ config, filePath, ignored }) => {
