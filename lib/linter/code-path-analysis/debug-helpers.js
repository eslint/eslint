...
module.exports = {
    ...
    dumpDot: !debug.enabled ? debug : /* istanbul ignore next */ function(codePath) {
        ...
        for (const id in traceMap) { // eslint-disable-line guard-for-in -- Want ability to traverse prototype
            ...
            text += segment.reachable ? "label=\"" : "style=\"rounded,dashed,filled\",fillcolor=\"#FF9800\",label=\"<<unreachable>>\\n";

            text += segment.internal.nodes.length > 0 ? segment.internal.nodes.join("\\n") : "????";
    ...
    makeDotArrows(codePath, traceMap) {
        ...
        while (stack.length > 0) {
            ...
            text += lastId === segment.id ? `->${nextSegment.id}` : `;\n${segment.id}->${nextSegment.id}`;
        ...
        codePath.returnedSegments.forEach(finalSegment => {
            text += lastId === finalSegment.id ? "->final" : `;\n${finalSegment.id}->final`;
            lastId = null;
        });

        codePath.thrownSegments.forEach(finalSegment => {
            text += lastId === finalSegment.id ? "->thrown" : `;\n${finalSegment.id}->thrown`;
            lastId = null;
        });
