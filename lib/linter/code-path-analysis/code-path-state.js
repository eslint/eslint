...
class CodePathState {
	...
	makeCatchBlock() {
		...
		/*
		 * Merge the thrown paths from the `try` block into the `catch` block.
		 *
		 * We only add the end-of-try-block segments when the end of the try
		 * block is reachable. When the try block ends with `return`, `break`,
		 * `throw`, or `continue`, the end-of-block segments are unreachable,
		 * and connecting them to the catch clause would create an incorrect
		 * edge in the code path (the unreachable code after such statements
		 * can never actually throw, so it should not connect to catch).
		 *
		 * Throwable nodes within the try block already have their paths to
		 * catch set up via `makeFirstThrowablePathInTryBlock()` and
		 * `makeThrow()`, so only the implicit end-of-block path needs
		 * this reachability guard.
		 */
