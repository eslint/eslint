 ...
 *
 * Choices can be forking or non-forking. A forking choice is one where the code
 * path forks based on the choice. This is the case with logical expressions.
 * For example, a `&&` operator forks based on the evaluation of the left
 * expression -- it may continue to the right expression or skip it due to short-circuiting.
 * The same is true for `||` and `??` as they also short-circuit, albeit in
 * different ways than `&&`.
 *
 * A non-forking choice is one where the code path stays consistent. For example,
 * the test condition of an `if` statement is a non-forking choice. The test evaluates
 * simply to true or false so it either continues to the block or skips it. The same
 * is true for conditional statements and loops, both of which also use test conditions
...
class ChoiceContext {
    ...
    constructor(upperContext, kind, isForkingAsResult, forkContext) {
        ...
        /**
         * Indicates if the result of the choice forks the code path. This is
         * true for logical expressions and false otherwise.
         * @type {boolean}
         */
...
class ForLoopContext extends LoopContextBase {
     ...
     */
    constructor(upperContext, label, breakContext) {
        super(upperContext, "ForStatement", label);
...
class ForInLoopContext extends LoopContextBase {
     ...
     */
    constructor(upperContext, label, breakContext) {
        super(upperContext, "ForInStatement", label);
...
class ForOfLoopContext extends LoopContextBase {
     ...
     */
    constructor(upperContext, label, breakContext) {
        super(upperContext, "ForOfStatement", label);
...
class CodePathState {
    ...
    makeForUpdate() {
        ...
        /*
         * Possible bug? `continueDestSegments` is already assigned in `makeForTest`
         * but that value is thrown away here. Both the test and the update must be
         * executed each time through the loop so maybe `continueDestSegments` should
         * contain both?
         */
    ...
    makeForBody() {
         ...
         * `continueDestSegments` will be `undefined`. In that case, a
