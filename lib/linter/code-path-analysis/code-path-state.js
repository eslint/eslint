 ...
 *
 * A `break` statement is only valid in a few places in JavaScript: any type of
 * loop or a `switch` statement. Otherwise, `break` causes a syntax error.
 *
 * However, this class is also used to mark a `LabeledStatement`. Labeled statements
 * are valid anywhere statements are valid even if it's not possible to use
 * `break` to jump to them. When used to represent a labeled statement, the
 * `breakable` property is set to false; for all other statements, `breakable`
 * is true.
 */
class BreakContext {
     ...
     *      the `break` will have an effect.
     * @param {string|null} label The label following `break`.
    ...
    constructor(upperContext, breakable, label, forkContext) {
        ...
        /**
         * Indicates that `break` will have an effect.
         * @type {boolean}
         */
        this.breakable = breakable;

        /**
         * The label associated with the `break`.
         * @type {string|null}
         */
        this.label = label;
...
class ForLoopContext extends LoopContextBase {
    ...
    constructor(upperContext, label, breakContext) {
        ...
        /**
         * The end of the update expresion.
         * @type {Array<CodePathSegment>|null}
         */
 ...
 * @param {CodePathState} state A state The state to search within.
...
class CodePathState {
    ...
    makeSwitchCaseBody(isCaseBodyEmpty, isDefaultCase) {
        ...
        if (isDefaultCase) {
             ...
             * and that it will fall through to this case. So, we need can now
    ...
    /**
     * Creates new context for a `break` statement. This occurs inside of a loop,
