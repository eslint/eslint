...
class CodePathState {
    ...
    makeCatchBlock() {
        ...
        /*
         * Special case: If the try block ends with a return or throw statement,
         * code path analysis automatically creates an unreachable segment
         * after that. Because it's unreachable, we don't want to
         * continue the path into the catch block. Instead, we want to
         * skip over that unreachable segment so the end of the try block
         * goes right to the start of the catch block.
         *
         * To do that, we reset the head of the fork context back to the previous
         * (reachable) segments of the current head.
         */
        if (!forkContext.reachable && forkContext.head[0].prevSegments.length) {
            forkContext.replaceHead(forkContext.head[0].prevSegments);
        }
