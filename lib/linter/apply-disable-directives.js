 ...
 * @param {number} lineStart Character position the line starts at.
 * @returns {boolean} Whether the comment is alone on its line.
 */
function commentIsAloneOnLine(comment, line, lineStart) {
    if (comment.range[0] !== lineStart) {
        return false;
    }

    return comment.type === "Block"
        ? line.length === comment.value.length + "/**/".length
        : true;
}
...
function getDirectiveRemovalRange(directive, sourceCode) {
    ...
    const directiveContents = comment.value.split("--")[0].trimRight();
    const ruleIdsArea = directiveContents
        .trim()
        .slice(`eslint-${directive.unprocessedDirective.type}`.length)
        .replace(/(,\W)*$/gu, "");

    // If the directive's rule isn't the only one in the comment, only remove that rule
    if (ruleIdsArea.includes(",")) {
        const ruleStart = directiveContents.indexOf(directive.ruleId);
        ...
        if (comment.value.trimRight().endsWith(directive.ruleId)) {
            ...
            for (const suffix of [", ", ","]) {
                if (comment.value.slice(ruleEnd, ruleEnd + suffix.length) === suffix) {
                    return [commentStart + 2 + ruleStart, commentStart + ruleEnd + suffix.length];
                }
            }
        }
    ...
    if (commentIsAloneOnLine(comment, line, sourceCode.lineStartIndices[lineIndex])) {
        return [
            lineStart,
