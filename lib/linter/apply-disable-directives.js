 ...
 */
function groupByLine(directives) {
    const groups = [];
    ...
    for (const directive of directives) {
        ...
        } else if (directive.line === state.line) {
...
function processUnusedDisableDirectives(allDirectives, sourceCode) {
    const directiveGroups = groupByLine(allDirectives);

    return lodash.flatMap(directiveGroups).map(directives => {
...
function applyDirectives(options) {
    ...
    const unusedDisableDirectives = processUnusedDisableDirectives(unusedDisableDirectivesToReport, options.sourceCode)
