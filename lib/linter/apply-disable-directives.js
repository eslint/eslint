...
function groupByParentComment(directives) {
    ...
    for (const directive of directives) {
        if (state) {
            if (directive.unprocessedDirective.parentComment === state.unprocessedDirective.parentComment) {
                state.group.push(directive);
            } else {
 ...
 */
function createIndividualDirectivesRemoval(directives, commentToken, sourceCode) {
    const commentStart = sourceCode.lineStartIndices[commentToken.loc.start.line - 1] + commentToken.loc.start.column;
 ...
 */
function createCommentRemoval(directives, commentToken, sourceCode) {
    const range = [
        sourceCode.lineStartIndices[commentToken.loc.start.line - 1] + commentToken.loc.start.column,
        sourceCode.lineStartIndices[commentToken.loc.end.line - 1] + commentToken.loc.end.column
    ];
    ...
    return flatMap(
        ...
        directives => {
            ...
            return remainingRuleIds.size
                ? createIndividualDirectivesRemoval(directives, parentComment.commentToken, sourceCode)
                : [createCommentRemoval(directives, parentComment.commentToken, sourceCode)];
        }
    );
}
