...
function assertValidNodeInfo(descriptor) {
    if (descriptor.node) {
        assert.ok(typeof descriptor.node === "object", "Node must be an object");
    } else {
        assert.ok(descriptor.loc, "Node must be provided when reporting error if location is not provided");
    }
}
...
function assertValidFix(fix) {
    if (fix) {
        assert.ok(fix.range && typeof fix.range[0] === "number" && typeof fix.range[1] === "number", `Fix has invalid range: ${JSON.stringify(fix, null, 2)}`);
    }
}
...
function mergeFixes(fixes, sourceCode) {
    ...
    for (const fix of fixes) {
        assert.ok(fix.range[0] >= lastPos, "Fix objects must not be overlapped in a report.");
