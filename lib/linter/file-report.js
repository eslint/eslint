 ...
 * @property {Language} language The language to use to adjust line and column offsets.
 */
 ...
 * @private
 */
function createLintingProblem(options, severity = 2) {
	const {
		ruleId = null,
...
class FileReport {
	 ...
	 * @returns {LintMessage} The created problem object.
	 ...
	 */
	addError(descriptor) {
		this.messages.push(createLintingProblem(descriptor, 2));
		return this.messages.at(-1);
	}

	/**
	 * Adds multiple error messages to the report. Meant to be called outside of rules.
	 * @param {LintProblem[]} descriptors The descriptors for the error messages.
	 * @returns {LintMessage[]} The created problem objects.
	 */
	addErrors(descriptors) {
		return descriptors.map(descriptor => this.addError(descriptor));
	}
	 ...
	 */
	addFatal(descriptor) {
		const message = createLintingProblem(descriptor, 2);
		message.fatal = true;
	 ...
	 * @param {LintProblem[]} descriptors The descriptors for the fatal error messages.
	 * @returns {LintMessage[]} The created problem objects.
	 */
	addFatals(descriptors) {
		return descriptors.map(descriptor => this.addFatal(descriptor));
	}
	...
	addWarning(descriptor) {
		this.messages.push(createLintingProblem(descriptor, 1));
		return this.messages.at(-1);
	}

	/**
	 * Adds multiple warning messages to the report. Meant to be called outside of rules.
	 * @param {LintProblem[]} descriptors The descriptors for the warning messages.
	 * @returns {LintMessage[]} The created problem objects.
	 */
	addWarnings(descriptors) {
		return descriptors.map(descriptor => this.addWarning(descriptor));
	}
}
