...
function addProblemIfSameSeverityAndOptions(config, loc, problems, ruleId, ruleOptions, ruleOptionsInline, severity) {
    ...
    const existingConfig = asArray(existingConfigRaw);
    ...
    if (existingConfig.length === 1 && ruleOptions.length === 1) {
        message = `"${ruleId}" is already configured to ${existingSeverity}.`;
    } else if (!containsDifferentProperty(ruleOptions.slice(1), existingConfig.slice(1))) {
        message = ruleOptionsInline.length === 1
            ? `"${ruleId}" is already configured to ${existingSeverity}.`
            : `"${ruleId}" is already configured to ${existingSeverity} with the same options.`;
    }
 ...
 * @param {SourceCode} sourceCode The SourceCode object to get comments from.
 * @param {function(string): {create: Function}} ruleMapper A map from rule IDs to defined rules
 * @param {StringSeverity} reportUnusedInlineConfigs Whether to report on unused inline config comments.
 * @param {string|null} warnInlineConfig If a string then it should warn directive comments as disabled. The string value is the config name what the setting came from.
 * @param {ConfigData} config Provided config.
 * @returns {{configuredRules: Object, enabledGlobals: {value:string,comment:Token}[], exportedVariables: Object, problems: LintMessage[], disableDirectives: DisableDirective[]}}
 * A collection of the directive comments that were found, along with any problems that occurred when parsing
 */
function getDirectiveComments(sourceCode, ruleMapper, reportUnusedInlineConfigs, warnInlineConfig, config) {
                ...
                if (parseResult.ok) {
                    Object.keys(parseResult.config).forEach(name => {
                        ...
                        if (reportUnusedInlineConfigs !== "off") {
                            addProblemIfSameSeverityAndOptions(config, loc, problems, name, ruleOptions, ruleOptionsInline, reportUnusedInlineConfigs);
                        }
...
function normalizeVerifyOptions(providedOptions, config) {
    ...
    let reportUnusedInlineConfigs = providedOptions.reportUnusedInlineConfigs;

    if (typeof reportUnusedInlineConfigs === "boolean") {
        reportUnusedInlineConfigs = reportUnusedInlineConfigs ? "error" : "off";
    }
    if (typeof reportUnusedInlineConfigs !== "string") {
        if (typeof linterOptions.reportUnusedInlineConfigs === "boolean") {
            reportUnusedInlineConfigs = linterOptions.reportUnusedInlineConfigs ? "warn" : "off";
        } else {
            reportUnusedInlineConfigs = linterOptions.reportUnusedInlineConfigs === void 0 ? "off" : normalizeSeverityToString(linterOptions.reportUnusedInlineConfigs);
        }
    }
...
function getRuleOptions(ruleConfig) {
    return asArray(ruleConfig).slice(1);
}
...
class Linter {
    ...
    #eslintrcVerifyWithoutProcessors(file, providedConfig, providedOptions) {
        ...
        const commentDirectives = options.allowInlineConfig
            ? getDirectiveComments(sourceCode, ruleId => getRule(slots, ruleId), options.reportUnusedInlineConfigs, options.warnInlineConfig, config)
                        ...
                        Object.keys(inlineConfig.rules).forEach(ruleId => {
                            ...
                            try {
                                ...
                                if (
                                    ...
                                    /*
                                     * If rule was already configured
                                     */
