 ...
 * @property {Object} [processorOptions] the options to pass to the preprocess
...
class Linter {
    ...
    _verifyWithConfigArray(textOrSourceCode, configArray, options) {
        ...
        if (processor) {
            ...
            const { processorOptions } = config;
            const disableFixes = options.disableFixes || !supportsAutofix;

            return this._verifyWithProcessor(
                textOrSourceCode,
                config,
                { ...options, disableFixes, postprocess, preprocess, processorOptions },
                configArray
            );
        }
        return this._verifyWithoutProcessors(textOrSourceCode, config, options);
    ...
    _verifyWithProcessor(textOrSourceCode, config, options, configForRecursive) {
        ...
        const messageLists = preprocess(text, filenameToExpose, processorOptions).map((block, i) => {
        ...
        return postprocess(messageLists, filenameToExpose, processorOptions);
    }
