 ...
 * @returns {number} normalized ECMAScript version
 */
function normalizeEcmaVersionForLanguageOptions(parser, ecmaVersion) {
    if ((parser[parserSymbol] || parser) === espree) {
        if (ecmaVersion === "latest") {
...
function parse(text, languageOptions, filePath) {
    ...
    // RuleTester wraps Espree and places a symbol on the wrapper to identify it
    const parserIsEspree = parser === espree || parser[parserSymbol] === espree;
    ...
    const sharedTraversalContext = Object.freeze(
        Object.assign(
            Object.create(BASE_TRAVERSAL_CONTEXT),
            {
                ...
                parserOptions: {
                    ecmaVersion: normalizeEcmaVersion(languageOptions.parser, languageOptions.ecmaVersion),
    ...
    _verifyWithFlatConfigArrayAndWithoutProcessors(textOrSourceCode, providedConfig, providedOptions) {
        ...
        try {
            lintingProblems = runRules(
                ...
                configuredRules,
                ruleId => getRuleFromConfig(ruleId, config),
                "TODO: Create backcompat parser name",
                languageOptions,
                settings,
                options.filename,
                options.disableFixes,
