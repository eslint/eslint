 ...
 * @param {boolean} resetTimes Whether to reset times
 * @returns {void}
 */
function storeTime(time, timeOpts, slots, resetTimes = false) {
    const { filename, type, key } = timeOpts;

    if (!slots.times) {
        slots.times = { [filename]: { passes: [{ [type]: { total: time } }] } };
    } else {
        if (!slots.times[filename]) {
            slots.times[filename] = { passes: [{ [type]: { total: time } }] };
        }

        const passIndex = slots.fixPasses ? slots.fixPasses : slots.times[filename].passes.length - 1;

        if (passIndex > slots.times[filename].passes.length - 1) {
            slots.times[filename].passes.push({ [type]: { total: 0 } });
        }

        if (typeof slots.times[filename].passes[passIndex][type] === "undefined") {
            if (type === "rules") {
                slots.times[filename].passes[passIndex][type] = {};
            } else {
                slots.times[filename].passes[passIndex][type] = { total: 0 };
            }
        }

        if (key) {
            if (typeof slots.times[filename].passes[passIndex][type][key] === "undefined" || resetTimes) {
                slots.times[filename].passes[passIndex][type][key] = { total: 0 };
            }
            slots.times[filename].passes[passIndex][type][key].total += time;
        } else {
            if (type !== "rules") {
                slots.times[filename].passes[passIndex][type].total = time;
            }
        }
    }
}
...
function runRules(sourceCode, configuredRules, ruleMapper, parserName, languageOptions, settings, filename, disableFixes, cwd, physicalFilename, ruleFilter,
    ...
    Object.keys(configuredRules).forEach(ruleId => {
        ...
        if (stats) {
            storeTime(ruleListenersReturn.tdiff, { filename, type: "rules", key: ruleId }, slots, slots.resetTimes === filename);
        }
        ...
        function addRuleErrorHandler(ruleListener) {
            return function ruleErrorHandler(...listenerArgs) {
                try {
                    ...
                    if (stats) {
                        storeTime(ruleListenerReturn.tdiff, { filename, type: "rules", key: ruleId }, slots, slots.resetTimes === filename);
                    }
...
class Linter {
    ...
    _verifyWithoutProcessors(textOrSourceCode, providedConfig, providedOptions) {
        ...
        if (!slots.lastSourceCode) {
            ...
            if (options.stats) {
                ...
                const timeOpts = { type: "parse", filename: options.filename };

                slots.resetTimes = !slots.fixPasses && slots?.times && slots?.times[options.filename] ? options.filename : false;
    ...
    getTimes(filename) {
        return internalSlotsMap.get(this).times[filename] ?? { passes: [] };
    }
    ...
    verifyAndFix(text, config, options) {
        ...
        const filename = options?.filename ?? "<text>";
        ...
        do {
            ...
            if (stats) {
                ...
                if (fixedResult.fixed) {
                    ...
                    storeTime(time, { filename, type: "fix" }, slots);
                    slots.fixPasses = fixPasses;
                } else {
                    storeTime(0, { filename, type: "fix" }, slots);
                }
            }
            ...
            if (stats) {
                ...
                const passIndex = slots.times[filename].passes.length - 1;

                slots.times[filename].passes[passIndex].total = tTotal;
            }
