 ...
 * @property {SuppressedLintMessage[]|null} lastSuppressedMessages The `SuppressedLintMessage[]` instance that the last `verify()` call produced.
...
function extractDirectiveComment(value) {
    const commentParts = value.split(/\s-{2,}\s/u);
    const directive = commentParts[0].trim();
    const justification = commentParts.length > 1 ? commentParts[1].trim() : "";
...
class Linter {
    ...
    _verifyWithoutProcessors(textOrSourceCode, providedConfig, providedOptions) {
        ...
        const { messages, suppressedMessages } = applyDisableDirectives({
