...
function storeTime(time, timeOpts, slots) {
    ...
    const passIndex = slots.fixPasses ? slots.fixPasses : slots.times.passes.length - 1;

    if (passIndex > slots.times.passes.length - 1 || !slots.times.passes[passIndex]) {
        slots.times.passes.push({ [type]: { total: 0 } });
    }

    if (typeof slots.times.passes[passIndex][type] === "undefined") {
        slots.times.passes[passIndex][type] = type === "rules" ? {} : { total: 0 };
    }

    if (key) {
        slots.times.passes[passIndex][type][key] = slots.times.passes[passIndex][type][key] || { total: 0 };
...
function runRules(sourceCode, configuredRules, ruleMapper, parserName, languageOptions, settings, filename, disableFixes, cwd, physicalFilename, ruleFilter,
    ...
    Object.keys(configuredRules).forEach(ruleId => {
        ...
        if (stats) {
            storeTime(ruleListenersReturn.tdiff, { filename, type: "rules", key: ruleId }, slots);
        }
        ...
        function addRuleErrorHandler(ruleListener) {
            return function ruleErrorHandler(...listenerArgs) {
                try {
                    ...
                    if (stats) {
                        storeTime(ruleListenerReturn.tdiff, { filename, type: "rules", key: ruleId }, slots);
                    }
