...
class Linter {
    ...
    _verifyWithoutProcessors(textOrSourceCode, providedConfig, providedOptions) {
        ...
        if (typeof config.parser === "object" && config.parser !== null) {
            ...
            jsparser = config.parser.definition;
            ...
            jsparser = slots.parserMap.get(config.parser);
        }
        ...
        const parserOptions = resolveParserOptions(jsparser, config.parserOptions || {}, enabledEnvs);
        ...
        if (!slots.lastSourceCode) {
            ...
            const parseResult = parser.parseSync(file, {
                language: jslang,
                languageOptions
            });

            if (typeof parseResult.time === "number") {
                const timeOpts = { type: "parse" };

                storeTime(parseResult.time, timeOpts, slots);
            }
    ...
    verify(textOrSourceCode, config, filenameOrOptions) {
        ...
        const slots = internalSlotsMap.get(this);
        const { configType, cwd } = slots;

        const options = typeof filenameOrOptions === "string"
            ? { filename: filenameOrOptions }
            : filenameOrOptions || {};

        const session = new Session({ persistent: false, cwd, timingEnabled: options.stats });
    ...
    _verifyWithFlatConfigArrayAndWithoutProcessors(textOrSourceCode, providedConfig, providedOptions) {
        ...
        if (!slots.lastSourceCode) {
            ...
            const parseResult = parser.parseSync(file, config);

            if (typeof parseResult.time === "number") {
                storeTime(parseResult.time, { type: "parse" }, slots);
            }
    ...
    verifyAndFix(text, config, options) {
        ...
        const session = new Session({
            persistent: false,
            cwd: internalSlotsMap.get(this).cwd,
            timingEnabled: stats
        });
