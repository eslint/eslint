...
class SourceCode extends TokenStore {
    ...
    getJSDocComment(node) {
         ...
         * @returns {Token|null} The comment token if found, null if not.
         * @private
         */
        const findJSDocComment = nodeToCheck => {
            const tokenBefore = this.getTokenBefore(nodeToCheck, { includeComments: true });

            if (
                astUtils.isCommentToken(tokenBefore) &&
                tokenBefore.type === "Block" &&
                tokenBefore.value.charAt(0) === "*" &&
                nodeToCheck.loc.start.line - tokenBefore.loc.end.line <= 1
            ) {
                return tokenBefore;
            }
        ...
        switch (node.type) {
            ...
            case "FunctionExpression":
                if (parent.type !== "CallExpression" && parent.type !== "NewExpression") {
                    ...
                    return (parent && parent.type !== "FunctionDeclaration" && parent.type !== "Program")
                        ? findJSDocComment(parent)
                        : null;
                }
                if (this.getCommentsBefore(node).length) {
                    return findJSDocComment(node);
                }
