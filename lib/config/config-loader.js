...
class ConfigLoader {
	 ...
	 * @returns {Promise<{configFilePath:string|undefined,defaultBasePath:string}>} Location information for
	 *      the config file.
	 */
	 ...
	 * @param {string} defaultBasePath The directory that contains the config file, or `cwd`.
	 * @returns {Promise<FlatConfigArray>} The config array for `eslint`.
	 */
	async #calculateConfigArray(configFilePath, defaultBasePath) {
	...
	async loadConfigArrayForDirectory(dirPath) {
		...
		const { configFilePath, defaultBasePath } =
			await this.#locateConfigFileToUse(absoluteDirPath);
		...
		return this.#calculateConfigArray(configFilePath, defaultBasePath);
	}
	...
	static async locateConfigFileToUse({
		...
		if (typeof useConfigFile === "string") {
			...
			if (configFilePath) {
				defaultBasePath = path.dirname(configFilePath);
			}
		}
	...
	static async calculateConfigArray(
		...
		const basePath = optionBasePath ?? defaultBasePath;
