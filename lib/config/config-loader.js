...
/**
 * Checks if native TypeScript support is enabled in the current Node.js process.
 *
 * This function determines if the `typescript` feature is present in the `process.features` object
...
function isNativeTypeScriptSupportEnabled() {
    return (
        /* eslint-disable n/no-unsupported-features/node-builtins -- it's still an experimental feature. */
        process.features.typescript === "strip" ||
        process.features.typescript === "transform"
    );
    /* eslint-enable n/no-unsupported-features/node-builtins -- enabling*/
}
 ...
 */
async function loadTypeScriptConfigFile(filePath, fileURL, mtime) {
 ...
 * @param {string} filePath The file path of the module to import.
 * @param {URL} fileURL The file URL to load.
 * @param {number} mtime The last modified timestamp of the file.
 * @returns {Promise<any>} - A promise that resolves to the imported module.
 */
async function dynamicImport(filePath, fileURL, mtime) {
    ...
    if (isTS) {
        if (hasUnstableNativeNodeJsTSConfigFlag) {
            if (isNativeTypeScriptSupportEnabled()) {
                return await dynamicImport(filePath, fileURL, mtime);
            }

            throw new Error(
                "The unstable_native_nodejs_ts_config flag is enabled, but native TypeScript support is not enabled in the current Node.js process. You need to either enable native TypeScript support by passing --experimental-strip-types or disable the unstable_native_nodejs_ts_config flag."
            );
        }

        if (!isDeno && !isBun) {
            return await loadTypeScriptConfigFile(filePath, fileURL, mtime);
        }
    }
...
class ConfigLoader {
	...
	static async calculateConfigArray(configFilePath, basePath, options) {
		...
		if (configFilePath) {
			...
			const fileConfig = await loadConfigFile(configFilePath, hasUnstableNativeNodeJsTSConfigFlag);
