...
class ConfigLoader {
    ...
    async #locateConfigFileToUse(fromDirectory) {
        ...
        const result = await locateConfigFileToUse({
...
class LegacyConfigLoader extends ConfigLoader {
    ...
    #locateConfigFileToUse() {
        if (!this.#configFilePathPromise) {
            this.#configFilePathPromise = locateConfigFileToUse({
                useConfigFile: this.#options.configFile,
                cwd: this.#options.cwd,
                allowTS: this.#options.allowTS
            });
        ...
        return this.#configFilePathPromise;
    }
    ...
    async #calculateConfigArray(configFilePath, basePath) {
        ...
        if (!this.#configArrayPromise) {
            this.#configArrayPromise = calculateConfigArray(configFilePath, basePath, this.#options);
        }

        // Cache the config array for this instance. This is primarely needed for the sync `getCachedConfigArrayForPath` method.
        this.#configArray = await this.#configArrayPromise;
