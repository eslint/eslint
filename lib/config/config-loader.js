...
function isNativeTypeScriptSupportEnabled() {
    return (
        /* eslint-disable n/no-unsupported-features/node-builtins -- disabling for now */
 ...
 *
 * This function uses `createJiti` to create a JITI (Just-In-Time Interpreter) instance
 * with specific options to disable module caching and interop default. It then uses
 * this JITI instance to import the module from the provided file path.
 ...
 */
async function dynamicImport(filePath, mtime) {
...
async function loadConfigFile(filePath, hasUnstableNativeNodeJsTSConfigFlag) {
    ...
    if (isTS) {
        if (
            isNativeTypeScriptSupportEnabled() &&
            hasUnstableNativeNodeJsTSConfigFlag
        ) {
            return await dynamicImport(filePath, mtime);
        }
