 ...
 * @property {string|undefined} [basePath] The directory containing the files to be linted. If not specified, it will be determined based on the config file location or `cwd`.
...
class ConfigLoader {
	 ...
	 * @returns {Promise<{configFilePath:string|undefined,referenceLocation:string}>} Location information for
	 *      the config file.
	 */
	 ...
	 * @param {string} referenceLocation The directory that contains the config file, or `cwd`.
	 * @returns {Promise<FlatConfigArray>} The config array for `eslint`.
	 */
	async #calculateConfigArray(configFilePath, referenceLocation) {
	...
	async loadConfigArrayForDirectory(dirPath) {
		...
		const { configFilePath, referenceLocation } =
			await this.#locateConfigFileToUse(absoluteDirPath);

		debug(
			'Using config file "%s" and reference location "%s"',
			configFilePath,
			referenceLocation,
		);
		return this.#calculateConfigArray(configFilePath, referenceLocation);
	}
	...
	static async locateConfigFileToUse({
		...
		if (typeof useConfigFile === "string") {
			...
			if (configFilePath) {
				referenceLocation = path.dirname(configFilePath);
			}
		}
	 ...
	 * @returns {Promise<FlatConfigArray>} The config array for `eslint`.
	 */
	static async calculateConfigArray(
		configFilePath,
		referenceLocation,
		options,
	) {
		...
		const basePath = optionBasePath ?? referenceLocation;

		debug(
			'Calculating config array from config file "%s" with base path "%s" and reference path "%s"',
			configFilePath,
			basePath,
			referenceLocation,
		);
