...
function assertConfigurationExists(configFilePath, loaderOptions) {
    ...
    if (!configFilePath && useConfigFile !== false && !overrideConfig) {
...
class ConfigLoader {
    ...
    async loadConfigArrayForFile(filePath) {
        ...
        const absoluteDirPath = path.resolve(this.#options.cwd, dirPath);
        const configFilePath = await this.findConfigFileForDirectory(absoluteDirPath);

        assertConfigurationExists(configFilePath, this.#options);

        return this.loadConfigArrayForDirectory(dirPath);
    }
