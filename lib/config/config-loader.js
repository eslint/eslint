 ...
 *      the config file.
 */
async function locateConfigFileToUse({ useConfigFile, cwd, fromDirectory = cwd, allowTS }) {
...
class ConfigLoader {
    ...
    async #locateConfigFileToUse(fromDirectory) {
        ...
        const resultPromise = locateConfigFileToUse({
        ...
        // ensure `locateConfigFileToUse` is called only once for `fromDirectory`
    ...
    async #calculateConfigArray(configFilePath, basePath) {
        ...
        const configsPromise = calculateConfigArray(configFilePath, basePath, this.#options);

        // ensure `calculateConfigArray` is called only once for `configFilePath`
    ...
    getCachedConfigArrayForPath(fileOrDirPath) {
        ...
        if (!configArray || typeof configArray.then === "function") {
            throw new Error(`Config array for ${fileOrDirPath} has not yet been calculated`);
        }
...
class LegacyConfigLoader extends ConfigLoader {
    ...
    #locateConfigFileToUse() {
        if (!this.#configFilePath) {
            this.#configFilePath = locateConfigFileToUse({
                useConfigFile: this.#options.configFile,
                cwd: this.#options.cwd,
                allowTS: this.#options.allowTS
            });
    ...
    async #calculateConfigArray(configFilePath, basePath) {
        ...
        // ensure `calculateConfigArray` is called only once
        this.#configArray = calculateConfigArray(configFilePath, basePath, this.#options);
    ...
    getCachedConfigArrayForPath(dirPath) {
        ...
        if (typeof this.#configArray.then === "function") {
            throw new Error(`Config array for ${dirPath} has not yet been calculated`);
        }
