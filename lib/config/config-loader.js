...
function assertConfigurationExists(configFilePath, loaderOptions) {
    ...
    /*
     * Note: The CLI always passes and `overrideConfig` regardless of whether
     * there are any flags that need it. So checking if `overrideConfig`
     * is defined is not a good indicator of whether or not a config file
     * has been specified.
     *
     * The symbol is used to uniquely identify `overrideConfig` as being
     * passed from the CLI.
     */
    if (!configFilePath && useConfigFile !== false && (!overrideConfig || overrideConfig[Symbol.for("eslint:from-cli")])) {
...
class ConfigLoader {
    ...
    /**
     * The default configuration array for the current working directory.
    ...
    /**
     * Returns a configuration array with the default configuration for the
     * current working directory. This does not load anything from disk.
     * @returns {Promise<FlatConfigArray>} The default configuration array for the current working directory.
     */
    async loadDefaultConfigArray() {

        debug("Calculating default config for directory");

        if (this.#defaultConfigArray) {
            return this.#defaultConfigArray;
        }

        this.#defaultConfigArray = await calculateConfigArray(void 0, this.#options.cwd, this.#options);
