...
class ConfigLoader {
	 ...
	 * This method is exposed internally for testing purposes.
	 * @param {string} configFilePath The absolute path to the config file to use if not overridden.
	 * @param {string} configBasePath The directory that contains the config file, or `cwd`.
	 * @param {ConfigLoaderOptions} options The options to use when loading configuration files.
	 * @returns {Promise<FlatConfigArray>} The config array for `eslint`.
	 */
	static async calculateConfigArray(configFilePath, configBasePath, options) {
		const {
			cwd,
		...
		const basePath = optionBasePath ?? configBasePath;

		debug(
			'Calculating config array from config file "%s" with base path "%s"',
			configFilePath,
			basePath,
		);
