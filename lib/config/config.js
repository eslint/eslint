...
/**
 * Builds a map from each registered plugin name to its `meta.namespace` value.
 * Only includes entries where `meta.namespace` is a non-empty string.
 * @param {Object} plugins The plugins object (registered name → plugin).
 * @returns {Map<string, string>} Map of registered plugin name to namespace.
 */
function buildPluginNamespaceMap(plugins) {
	...
	for (const [name, plugin] of Object.entries(plugins)) {
		if (plugin?.meta?.namespace) {
			map.set(name, plugin.meta.namespace);
		}
	}
...
function doesRuleSupportLanguage(ruleLangs, configLanguageName, plugins) {
	...
	for (const langEntry of ruleLangs) {
		...
		if (ruleLangPart === "*") {
			// Direct plugin name match
			if (configPluginName === rulePluginPart) {
				return true;
			}

			// Namespace match: the plugin registered as configPluginName has namespace rulePluginPart
