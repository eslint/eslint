 ...
 */
function doesRuleSupportLanguage(ruleLangs, configLanguageName, validPluginNames) {
...
class Config {
	...
	validateRulesConfig(rulesConfig) {
		...
		const { pluginName: configPluginName } =
			splitPluginIdentifier(normalizedLanguageName);
		const configPlugin =
			this.plugins[configPluginName] ??
			(configPluginName === "js" ? this.plugins["@"] : undefined);
