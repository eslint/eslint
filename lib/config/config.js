 ...
 * @param {string} configLanguageName The language name from the config (e.g., "@/js").
...
function doesRuleSupportLanguage(ruleLangs, configLanguageName, plugins) {
	...
	for (const langEntry of ruleLangs) {
		...
		// Special case: "js/js" matches "@/js"
		if (langEntry === "js/js" && configLanguageName === "@/js") {
			return true;
		}
		...
		if (ruleLangPart === "*") {
			...
			// Special case: "js/*" matches any "@" language
			if (rulePluginPart === "js" && configPluginName === "@") {
				return true;
			}

			// Namespace match: find plugin registered under meta.namespace === rulePluginPart
			for (const [namespace, plugin] of Object.entries(plugins)) {
				if (
					plugin?.meta?.namespace === rulePluginPart &&
					namespace === configPluginName
				) {
					return true;
				}
			}
		} else {
			// Namespace-based match: find plugin with meta.namespace === rulePluginPart
			for (const [namespace, plugin] of Object.entries(plugins)) {
				if (
					plugin?.meta?.namespace === rulePluginPart &&
					`${namespace}/${ruleLangPart}` === configLanguageName
				) {
					return true;
				}
			}
...
class Config {
	...
	validateRulesConfig(rulesConfig) {
		...
		if (unsupportedLanguageRules.length > 0) {
			// Normalize "@/" prefix to "js/" for user-facing messages
			const displayLanguageName = this.#languageName.startsWith("@/")
				? `js/${this.#languageName.slice(2)}`
				: this.#languageName;

			throw new TypeError(
				`Key "rules": The following rules do not support the language "${displayLanguageName}":\n${unsupportedLanguageRules.map(ruleId => `\t- "${ruleId}"`).join("\n")}`,
			);
		}
	}
