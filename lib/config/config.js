...
/**
 * Formats a value for display in rule-options validation error messages.
 * RegExp is shown as "/pattern/flags" instead of "{}".
 * @param {any} value The value that failed validation.
 * @returns {string} A string suitable for inclusion in an error message.
 */
function formatValueForErrorMessage(value) {
...
/**
 * Recursively adds `plainObjectOnly: true` to every node in a schema that has
 * `type: "object"`. Mutates the schema in place. Used so that RegExp (and other
 * non-plain objects) fail validation when a rule expects a plain object.
 * @param {Object} schema A JSON Schema object (will be mutated).
 * @returns {void}
 */
function addPlainObjectOnlyToSchema(schema) {
	if (!schema || typeof schema !== "object") {
		return;
	}

	if (schema.type === "object") {
		schema.plainObjectOnly = true;
	}
		...
		if (Array.isArray(value)) {
			for (const item of value) {
				if (typeof item === "object" && item !== null) {
					addPlainObjectOnlyToSchema(item);
				}
			}
		} else if (typeof value === "object" && value !== null) {
			addPlainObjectOnlyToSchema(value);
		}
	}
	...
	if (schema.properties && typeof schema.properties === "object") {
		for (const prop of Object.values(schema.properties)) {
			if (typeof prop === "object" && prop !== null) {
				addPlainObjectOnlyToSchema(prop);
			}
		}
	}
...
function getOrCreateValidator(rule, ruleId) {
	if (!validators.has(rule)) {
		try {
			let schema = getRuleOptionsSchema(rule);

			if (schema) {
				const { ruleName } = parseRuleId(ruleId);

				if (ruleName === "no-restricted-imports") {
					schema = JSON.parse(JSON.stringify(schema));
					addPlainObjectOnlyToSchema(schema);
				}
						...
						`Key "rules": Key "${ruleId}":\n${validateRule.errors
							.map(error => {
								if (
									...
									return `\tValue ${formatValueForErrorMessage(error.data)} ${error.message}.\n\t\tUnexpected property "${error.params.additionalProperty}". Expected properties: ${expectedProperties.join(", ")}.\n`;
								}

								if (error.keyword === "plainObjectOnly") {
									return `\tValue ${formatValueForErrorMessage(error.data)} must be a plain object. RegExp and other non-plain objects are not allowed.\n`;
								}
