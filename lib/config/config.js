...
/**
 * The error type when a rule's options fail schema validation.
 * Used so that RuleTester can recognize and test schema validation failures
 * via the messageForTest property (helper-text-free message).
 */
class SchemaValidationError extends Error {
	/**
	 * Creates a new instance.
	 * @param {string} message Full error message (includes Key "rules": etc.).
	 * @param {string} messageForTest Message for RuleTester assertions (no helper text).
	 */
	constructor(message, messageForTest) {
		super(message);
...
/**
 * Formats an error from the schema validation.
		...
		for (const [ruleId, ruleOptions] of Object.entries(rulesConfig)) {
			...
			if (validateRule) {
				...
				if (validateRule.errors) {
					const messageForTest = validateRule.errors
						.map(formatError)
						.join("")
						.trim();
					const fullMessage = `Key "rules": Key "${ruleId}":\n${messageForTest}`;
