...
/**
 * Builds a map from each registered plugin name (normalized: `"@"` → `"js"`)
 * to its effective namespace. The namespace is `meta.namespace` when present,
 * otherwise it falls back to the normalized plugin name itself.
 * @param {Object} plugins The plugins object (registered name → plugin).
 * @returns {Map<string, string>} Map of normalized plugin name to namespace.
 */
function buildPluginNamespaceMap(plugins) {
	const map = new Map();

	for (const [name, plugin] of Object.entries(plugins)) {
		const key = name === "@" ? "js" : name;
		const value = plugin?.meta?.namespace || key;
 ...
 * @param {Map<string, string>} pluginNamespaceMap Precomputed map of normalized plugin name to namespace.
 * @returns {boolean} True if the rule supports the language, false otherwise.
 */
function doesRuleSupportLanguage(ruleLangs, configLanguageName, pluginNamespaceMap) {
	// If no languages specified, works with all languages (backward compatible)
	if (!ruleLangs || !Array.isArray(ruleLangs)) {
		return true;
	}

	const { pluginName: configPluginName } =
	...
	for (const langEntry of ruleLangs) {
		...
		// "plugin/*" wildcard - matches any language from that plugin
		if (ruleLangPart === "*") {
			// Namespace match (or direct name match via fallback when no meta.namespace)
			if (pluginNamespaceMap.get(configPluginName) === rulePluginPart) {
				return true;
			}
		} else {
			// Namespace-based match: the plugin registered as configPluginName has namespace rulePluginPart
			if (
				pluginNamespaceMap.get(configPluginName) === rulePluginPart &&
				`${configPluginName}/${ruleLangPart}` === configLanguageName
			) {
				return true;
			}
		}
...
class Config {
	...
	validateRulesConfig(rulesConfig) {
		...
		const normalizedLanguageName = normalizeLanguageName(this.#languageName);
		const pluginNamespaceMap = buildPluginNamespaceMap(this.plugins);
