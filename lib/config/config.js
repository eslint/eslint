...
class Config {
	 ...
	 * @returns {RuleDefinition} The rule definition from the plugin.
	 */
	...
	/**
	 * Gets the severity of a rule by its ID.
	 * @param {string} ruleId The ID of the rule to get the severity for.
	 * @returns {number} The numeric severity of the rule, or 0 if the rule is not found.
	 */
	getRuleNumericSeverity(ruleId) {
		const ruleConfig = this.rules[ruleId];

		if (!ruleConfig) {
			return 0;
		}

		// the normalized rule config always has a numeric severity
		return ruleConfig[0];
	}
	...
	#normalizeRulesConfig() {
		for (const [ruleId, originalConfig] of Object.entries(this.rules)) {
			...
			// normalize severity
			ruleConfig[0] = severities.get(
				typeof ruleConfig[0] === "string"
					? ruleConfig[0].toLowerCase()
					: ruleConfig[0],
			);
	 ...
	 *      parts of the ruleId;
	 */
	static parseRuleId(ruleId) {
		return parseRuleId(ruleId);
	}
