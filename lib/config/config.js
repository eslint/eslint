...
const ajv = ajvImport({ strictSchema: false });
...
/**
 * Cache for the result of `getRuleOptionsSchema`. Keyed by rule object so
 * repeated calls for the same rule always return the same (identity-equal)
 * schema wrapper. This is necessary for Ajv's compile/validateSchema caches,
 * which key by object identity, to work correctly across test cases.
 * @type {WeakMap<object, Object|null>}
 */
const ruleOptionsSchemas = new WeakMap();
...
function getRuleOptionsSchema(rule) {
	if (ruleOptionsSchemas.has(rule)) {
		return ruleOptionsSchemas.get(rule);
	}

	let schema;

	if (!rule.meta) {
		schema = { ...noOptionsSchema }; // default if `meta.schema` is not specified
	} else {
		const ruleSchema = rule.meta.schema;

		if (typeof ruleSchema === "undefined") {
			schema = { ...noOptionsSchema }; // default if `meta.schema` is not specified
		} else if (ruleSchema === false) {
			schema = null;
		} else if (typeof ruleSchema !== "object" || ruleSchema === null) {
			throw new TypeError("Rule's `meta.schema` must be an array or object");
		} else if (Array.isArray(ruleSchema)) {
			if (ruleSchema.length) {
				schema = {
					type: "array",
					items: ruleSchema,
					minItems: 0,
	...
	ruleOptionsSchemas.set(rule, schema);
	return schema;
}
