 ...
 * @param {string} [relativeTo] The path to resolve relative to.
 * @param {ConfigCache} [configCache] the config cache
 * @returns {Object} the parsed config object (empty object if there was a parse error)
 * @private
 */
function load(filePath, relativeTo, configCache) {
    const resolvedPath = resolve(filePath, relativeTo);

    if (typeof configCache !== "undefined") {
        const cachedConfig = configCache.getConfig(resolvedPath.filePath);
    ...
    const config = loadFromDisk(resolvedPath);

    if (config && typeof configCache !== "undefined") {
        config.filePath = resolvedPath.filePath;
        config.baseDirectory = path.dirname(resolvedPath.filePath);
        configCache.setConfig(resolvedPath.filePath, config);
    }
