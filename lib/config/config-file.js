...
function applyExtends(config, configContext, filePath) {
    ...
    return extendsList.reduceRight((previousValue, extendedConfigReference) => {
        try {
 ...
 *     or the absolute path to a config file.
 * @private
 */
function resolve(extendedConfigReference, referencedFromPath, pluginRootPath) {
    ...
    if (extendedConfigReference.startsWith("plugin:")) {
        ...
        return {
            filePath: relativeModuleResolver(normalizedPluginName, path.join(pluginRootPath, "placeholder.js")),
            configName,
            configFullName
        };
    }
 ...
 * @param {string} configPath The path where `extends` and parsers should be resolved from (usually the path where the config was located from)
 * @param {string} configDisplayName The display name of the config, for use in error messages
 * @returns {Object} the config object with extends applied if present, or the passed config if not
 * @private
 */
function loadObject(configObject, configContext, configPath, configDisplayName) {
    ...
    if (config.parser) {
        config.parser = relativeModuleResolver(config.parser, configPath);
    ...
    validator.validate(config, configDisplayName, ruleMap.get.bind(ruleMap), configContext.linterContext.environments);
     ...
     */
    if (config.extends) {
        return applyExtends(config, configContext, configPath);
    }
...
function loadFromDisk(configInfo, configContext) {
    ...
    if (config) {
        return loadObject(config, configContext, configInfo.filePath, configInfo.configFullName);
    }
...
function load(extendedConfigReference, configContext, referencedFromPath) {
    ...
    if (config) {
        ...
        // FIXME: remove baseDirectory property
