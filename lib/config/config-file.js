...
function resolve(filePath, relativeTo) {
    ...
    if (filePath.startsWith("plugin:")) {
        ...
        if (relativeTo !== "") {
            debug(`Using relative path for loading plugin "${pluginName}"`);
        ...
        return {
            filePath: require.resolve(normalizedPackageName, { paths: resolutionPaths }),
            configName,
            configFullName
        };
    }
...
function load(filePath, configContext, relativeTo) {
    ...
    debug(`Resolved ${ filePath } to ${ JSON.stringify(resolvedPath) }`);
