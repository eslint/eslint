    ...
    validate(config) {
        ...
        for (const ruleId of Object.keys(config.rules)) {
            ...
            if (validateRule) {
                ...
                /*
                 * Because rule schemas can specify a minimum number of items
                 * required in a rule option, a configuration such as
                 * foo: "warn" might be invalid because it lacks the expected
                 * options. To catch that case, we validate against an empty
                 * array if the rule options aren't already an array.
                 */
                const ruleOptions = Array.isArray(config.rules[ruleId])
                    ? config.rules[ruleId].slice(1)
                    : [];

                validateRule(ruleOptions);
