...
const pluginsSchema = {
	...
	validate(value) {
		...
		for (const key of Object.keys(value)) {
			...
			// validate meta.languages for each rule in this plugin
			const rules = value[key].rules;

			if (rules && typeof rules === "object") {
				for (const [ruleName, rule] of Object.entries(rules)) {
					if (rule && typeof rule === "object" && rule.meta) {
						const { languages } = rule.meta;

						if (languages !== undefined) {
							if (!Array.isArray(languages)) {
								throw new TypeError(
									`Key "${key}": Key "rules": Rule "${ruleName}": Key "meta": Key "languages": Expected an array.`,
								);
							}
							...
							for (const lang of languages) {
								if (typeof lang !== "string") {
									throw new TypeError(
										`Key "${key}": Key "rules": Rule "${ruleName}": Key "meta": Key "languages": Expected each element to be a string.`,
									);
								}
							}
						}
