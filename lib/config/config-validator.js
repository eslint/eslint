...
function getRuleOptionsSchema(rule) {
    ...
    // return null if rule is not defined.
    if (!rule) {
        return null;
    }
...
function validate(config, source, ruleMapper, envContext) {
    ...
    // validate config.rules, config.envs, and overrides.
    const configs = config.overrides ? [config, ...config.overrides] : [config];

    for (const configItem of configs) {
        validateRules(configItem.rules, source, ruleMapper);
        validateEnvironment(configItem.env, source, envContext);
    }
}
