...
function validateRuleSchema(id, localOptions, rulesContext) {
    ...
    if (validateRule) {
        ...
        if (validateRule.errors) {
            // @todo This needs to changed to include context of the error, i.e.
            // instead of 'data should NOT have more than 0 items' it needs to say
            // 'data should NOT have more than 0 items (dataPath: "", data: ["extra"])'
            // or something along the lines. Best to discuss with the ESLint team.
            throw new Error(ajv.errorsText(validateRule.errors));
        }
    }
}
...
function formatErrors(errors) {
    return errors.map(error => {
        // @todo All of these errors can be moved to ajv-i18n or abstracted using a custom Ajv error handler.
        ...
        if (/^should be/.test(error.message) && error.schema) {
            const formattedField = error.dataPath.slice(1);
            const formattedExpectedType = Array.isArray(error.schema) ? error.schema.join('/') : error.schema;
            const formattedValue = JSON.stringify(error.data);

            return `Property "${formattedField}" is the wrong type (expected ${error.schema} but got \`${formattedValue}\`)`;
        }
