...
function hasESLintVersionConflict(answers) {
    ...
    /*
     * Get the version of ESLint currently being used. This is intentionally called via
     * `module.exports` to make it easier to stub a different ESLint version in tests.
     */
    const eslintVersion = module.exports.getESLintVersion();
    ...
    answers.eslintVersion = eslintVersion;
    answers.requiredESLintVersionRange = requiredESLintVersionRange;

    // Check the version.
    if (semver.satisfies(eslintVersion, requiredESLintVersionRange)) {
...
function promptUser() {
    ...
    return inquirer.prompt([
        ...
        },
        {
            ...
            name: "installESLint",
            message(answers) {
                const verb = semver.ltr(answers.eslintVersion, answers.requiredESLintVersionRange)
                    ? "upgrade"
                    : "downgrade";

                return `The style guide "${answers.styleguide}" requires eslint@${answers.requiredESLintVersionRange}. You are currently using eslint@${answers.eslintVersion}.\n  Do you want to ${verb}?`;
            },
            default: true,
            when(answers) {
        ...
        if (earlyAnswers.source === "guide") {
            ...
            if (earlyAnswers.installESLint === false && !semver.satisfies(earlyAnswers.eslintVersion, earlyAnswers.requiredESLintVersionRange)) {
