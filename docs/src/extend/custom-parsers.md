...
- `scopeManager` can be a [ScopeManager](./scope-manager-interface) object or `null`. Custom parsers can use customized scope analysis for experimental/enhancement syntaxes. If this property is omitted or `null`, ESLint's JavaScript language support falls back to a default `ScopeManager` created by [eslint-scope](https://github.com/eslint/js/tree/main/packages/eslint-scope) when it creates the `SourceCode` object.
