...
- `getText(node, beforeCount, afterCount)`: Returns the source code for the given node. If `node` is omitted or falsy (such as `undefined`, `null`, or an empty string), the entire source text is returned (see the [dedicated section](#accessing-the-source-text)).
