...
The options available and the expected syntax for `parserOptions` is the same as those used in [configuration](../use/configuring/language-options#specifying-parser-options).
...
2. Create a [configuration file](../use/configuring/) and specify your rule ID error level under the `rules` key. Your rule will not run unless it has a value of `1` or `2` in the configuration file.
