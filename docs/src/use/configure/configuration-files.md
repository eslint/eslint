...
* `ignores` - An array of glob patterns indicating the files that the configuration object should not apply to. If not specified, the configuration object applies to all files matched by `files`. [global ignores](#globally-ignoring-files-with-ignores) is what you need if you want to apply the same `ignores` rule to every configuration objects.
