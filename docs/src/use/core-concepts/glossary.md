...
Each node may have any number of properties, including properties that store children nodes.

The AST format used by ESLint is the standard [ESTree](#estree) format.
...
ESLint will apply all fixes in a [report](#report) when run with the `--fix` flag.
Fixes may also be triggered in common editor extensions.
...
ESLint ships with several built-in reporters, including `json`, `stylish` (default), and `tap`.
...
Common formatters in the ecosystem include [Prettier](https://prettier.io) or [dprint](https://dprint.dev).

Note that although ESLint is a [linter](#linter) rather than a formatter, ESLint still currently ships with a collection of deprecated [formatting rules](#formatting-rule).
Formatting rules will be removed in a later major version.
...
Each node has a node type representing its type of syntax.
...
For more information on configuring parsers, see [Plugins > Specify a Processor](../configure/plugins#specify-a-processor).
...
A piece of logic that checks [AST](#abstract-syntax-tree-ast) for the presence of a code defect.

ESLint provides a large collections of rules that check for common JavaScript code issues.
Many more lint rules may be loaded in by [plugins](#plugin).
...
A description of how to search for [nodes](#node) within an [AST](#abstract-syntax-tree-ast).
...
Tooling that analyzes source code without building or running it.

[Linters](#linter) such as ESLint, [formatters](#formatter-tool), and [type checkers](#type-checker) are examples of static analysis.

Static analysis is different from _dynamic_ analysis, which is tooling that builds and runs source code.
...
ESLint's built-in stylistic rules are feature frozen frozen: except for supporting new ECMAScript versions, they won't receive new features.
...
An optional augmentation to a [rule](#rule) [violation](#violation) that describes how one way to manually adjust the code to improve.

Suggestions are not generally "safe" to apply automatically: they cause code behavior changes.
ESLint will only apply suggestions when run via an integration such as an editor extension.
