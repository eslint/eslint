...
- In the `eslint/eslint` repository, submit two PRs that add `'/docs/v9\.x'` to the `exclude` list in `docs/lychee.toml`. One PR should target the `main` branch and the other the `v9.x-dev` branch. The PRs should be reviewed and approved in time, but merged right before ESLint 10.0.0 is released.
