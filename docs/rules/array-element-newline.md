...
* `"direction": <string>` one of the following: `"fold", "unfold", "both"`. Default value is `"both"` means linebreaks will be added or removed according the rule. `"Unold"` means only adding line breaks, `"fold"` only removing.  
